...
{
    return ParentWindow.ChildInFocus != self;
    return;
}
...
function Tick(float Delta)
{
    ...
    // End:0x3D
    if((ParentWindow.ChildInFocus != self) && ! GreyedOut())
    {
        SelectedTime += Delta;        
    }
    else
...
function BeforePaint(Canvas C, float X, float Y)
{
    ...
    C.Font = C.TallFont;
    TextSize(C, Text, W, h, TextScaleX * TextScaleXMod, TextScaleY * TextScaleYMod);
...
function DrawButtonText(Canvas C)
{
    ...
    ClipText(C, TextX, TextY, Text, false, TextScaleX * TextScaleXMod, TextScaleY * TextScaleYMod, Alpha, 2);
    // End:0x116
...
{
    X = TextScaleX * TextScaleXMod;
    Y = TextScaleY * TextScaleYMod;
    return;
}
