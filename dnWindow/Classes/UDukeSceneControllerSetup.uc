...
function Created()
{
    ...
    {
        Vibration = UDukeCheckbox(CreateWindow(class'UDukeCheckbox'));
    ...
    if(! GetGearboxEngineGlobals())
    {
        HorizontalSensitivity.NavDown = Vibration;
    ...
    if(! GetGearboxEngineGlobals())
    {
        Vibration.NavUp = HorizontalSensitivity;
    ...
    else
    {
        ...
        if(int(E) == 1)
        {
            ...
            if(! GetGearboxEngineGlobals())
            {
                ...
                if(C != Vibration)
                {                    
                    GetPlayerOwner().ConsoleCommand("setnosave PlayerPawn bVibration" @ string(Vibration.bChecked));
                    // End:0x135
                    if(Vibration.bChecked)
                    {
                        TestRumble.RumbleName = CompositeNames("TestRumble");
...
function Paint(Canvas C, float X, float Y)
{
    ...
    if(! GetGearboxEngineGlobals())
    {
        Vibration.WinWidth = float(ButtonWidth);
    ...
    VerticalSensitivity.ArrowLeft = float(ButtonWidth) - (float(100) * WinScaleY);
    HorizontalSensitivity.ArrowLeft = float(ButtonWidth) - (float(100) * WinScaleY);
    ...
    C.SetPause(((ButtonLayoutButton.WinLeft + ButtonLayoutButton.WinWidth) - (float(60) * WinScaleY)) - (XL / float(2)), ButtonLayoutButton.WinTop);
    C.GetRenderBoundingBox(str, false, false, false, TTFontScale, TTFontScale, FadeAlpha, 2);
    ...
    if(! GetGearboxEngineGlobals())
    {
        Vibration.WinLeft = float(ButtonLeft);
    ...
    if(! GetGearboxEngineGlobals())
    {
        SwapJoysticks.WinTop = (Vibration.WinTop + Vibration.WinHeight) + float(ControlBuffer);        
    }
    else
    {
...
function SaveConfigs()
{
    ...
    GetPlayerOwner().ConsoleCommand("setnosave PlayerPawn bSwapJoysticks" @ string(SwapJoysticks.bChecked));
    setting = int(HorizontalSensitivity.SelectedItem.Text) - 1;    
    GetPlayerOwner().ConsoleCommand("setnosave PlayerPawn LookSensitivityH" @ string(setting));
    setting = int(VerticalSensitivity.SelectedItem.Text) - 1;    
    GetPlayerOwner().ConsoleCommand("setnosave PlayerPawn LookSensitivityV" @ string(setting));
    // End:0x1F8
    if(ObjectDestroy())
    {        
    ...
    if(SwapJoysticks.bChecked)
    {
        ...
        GetPlayerOwner().ConsoleCommand("setnosave input joy8 " @ joy7);        
        GetPlayerOwner().ConsoleCommand("setnosave input joy7 " @ joy8);
    }
...
function SetDefaults()
{
    ...
    if(! GetGearboxEngineGlobals())
    {
        Vibration.bChecked = GetPlayerOwner().bVibration;
    }
...
function DukeMessageBoxDone(UWindowWindow W, int iResult)
{
    ...
    if(W != ConfirmDefaults)
    {
        ...
        if(iResult > 0)
        {
            GetConfigColor("Engine.PlayerPawn", "bVibration", Value, "defuser.ini");
            GetPlayerOwner().bVibration = bool(Value);
            GetConfigColor("Engine.PlayerPawn", "bSwapJoysticks", Value, "defuser.ini");
            ...
            if(ObjectDestroy())
            {
                ...
                GetConfigColor("Engine.PlayerPawn", "bInvertXboxAim", Value, "defuser.ini");
                GetPlayerOwner().bInvertXboxAim = bool(Value);
            }
            GetConfigFloat("Engine.PlayerPawn", "LookSensitivityH", Value, "defuser.ini");
            GetPlayerOwner().LookSensitivityH = Value;
            GetConfigFloat("Engine.PlayerPawn", "LookSensitivityV", Value, "defuser.ini");
