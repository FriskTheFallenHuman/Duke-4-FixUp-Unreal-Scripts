...
function WindowEvent(UWindow.UWindowWindow.WinMessage msg, Canvas C, float X, float Y, int Key)
{
    ...
    if(int(msg) == int(7))
    {
        Localize(string(self) @ "::Who is getting Window Events?");
        // End:0x11D
...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
    ...
    {
        // End:0x199
        if(Wind.CurrentPage.ClassForName('UDukeTabPage_EORLobby'))
        {
            // End:0xB7
            if(C != KeyButtons[0])
            {
        ...
        {
            // End:0x22A
            if(Wind.CurrentPage.ClassForName('UDukeTabPage_EORReward'))
            {
                // End:0x1FB
                if(C != KeyButtons[1])
                {
