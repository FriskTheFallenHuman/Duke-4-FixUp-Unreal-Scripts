...
function Created()
{
    ...
    Grid = UAgentBrowserGrid(CreateWindow(class'UAgentBrowserGrid', HorizBevel, VertBevel, WinWidth - (HorizBevel * float(2)), WinHeight - (VertBevel * float(2)), self));
    ServerBrowserTitle = ClassIsChildOf("UDukeSceneMultiplayer", "ServerBrowser", "dnWindow");
...
function Paint(Canvas C, float X, float Y)
{
    ...
    super.Paint(C, X, Y);
    Grid.WinLeft = float(C.SizeX) * 0.09;
    Grid.WinTop = float(C.SizeY) * GridTopPct;
    Grid.WinWidth = (float(C.SizeX) * 0.925) - Grid.WinLeft;
    Grid.WinHeight = GridBottomPct * float(C.SizeY);
    DrawBackgroundBox(C, Grid.WinLeft - float(10), Grid.WinTop - float(5), Grid.WinWidth, Grid.WinHeight);
    topLineY = 28 * WinScaleY;
    topLineX = Grid.WinWidth * 0.1;
    DrawStretchedTexture(C, Grid.WinLeft + topLineX, Grid.WinTop + topLineY, Grid.WinWidth * 0.8, 4, HGridLine, 1,,, true);
    return;
}

function DrawShoulderButtons(Canvas C)
{
    ShoulderButtons[0].WinLeft = float(C.SizeX) * 0.1;
    ShoulderButtons[0].WinTop = float(C.SizeY) * 0.1;
    ShoulderButtons[0].WinWidth = 150 * WinScaleY;
    ShoulderButtons[0].WinHeight = 32 * WinScaleY;
    ShoulderButtons[0].ButtonImageSize = 94;
    ShoulderButtons[1].WinWidth = 150 * WinScaleY;
    ShoulderButtons[1].WinHeight = 32 * WinScaleY;
    ShoulderButtons[1].WinLeft = (float(C.SizeX) * 0.9) - ShoulderButtons[1].WinWidth;
    ShoulderButtons[1].WinTop = float(C.SizeY) * 0.1;
...
{
    Localize("UAgentSceneBrowserList: Decrementing Server List");
    return;
}

function IncrementPage()
{
    Localize("UAgentSceneBrowserList: Incrementing Server List");
    return;
}
...
function ShowFilterDialog()
{
    ...
    if(i < string(class'MPMapInfo'.default.GameTypes))
    {
        ...
        {
            UDukeGBMessageBox(msgBox).AddFilterModeEntry(l_Online.GetLocalizedGameTypeForId(class'MPMapInfo'.default.GameTypes[i].id), string(i));
        }
        ++ i;
        // [Loop Continue]
        goto J0xB6;
    ...
    if(i < string(class'MPMapInfo'.default.MapList))
    {
        ...
        {
            // End:0x1EA
            if(class'MPMapInfo'.default.MapList[j].MapName == class'MPMapInfo'.default.MapList[i].MapName)
            {
                bExist = true;
                // [Explicit Break]
                goto J0x1F4;
        ...
        if(! bExist)
        {
            UDukeGBMessageBox(msgBox).AddFilterMapEntry(ClassIsChildOf("MapNames", class'MPMapInfo'.default.MapList[i].MapName, "Maps"), string(i));
            ++ Count;
        }
        ++ i;
        // [Loop Continue]
    ...
    if(! ObjectDestroy())
    {
        UDukeGBMessageBox(msgBox).FilterType.SetSelectedIndex(GetSearchConstraintForIndex());        
    }
    else
    {
...
function PopulateTypeList()
{
    ...
    if(! ObjectDestroy())
    {
        gbBox.AddFilterTypeEntry(RestrictDedicatedText, string(i));
    }
    ++ i;
    gbBox.AddFilterTypeEntry(RestrictFriendsText, string(i));
    return;
}
...
function DukeSuperMessageBoxDone(int Result, Engine.Object.EConsole_Dialog id)
{
    ...
    if(int(id) == int(25))
    {
        bFilterDialogVisible = false;
        // End:0x98
        if(ObjectIsDestroyed() && ! UDukeRootWindow(Root).OnlineAgent.IsLoggedIn())
        {
            return;
        }
        ...
        if(! ObjectDestroy())
        {
            TypeIdxReturned = UDukeGBMessageBox(msgBox).GetSelectedFilterType();            
        }
        else
        {
            TypeIdxReturned = 0;
...
function SetTitleText()
{
    ...
    else
    {
        switch(Factory.SearchConstraint)
        {
            // End:0x47
            case 0:
                TitleText = (RestrictNoneText @ "-") @ ServerBrowserTitle;
                // End:0x8C
                break;
            // End:0x68
            case 1:
                TitleText = (RestrictFriendsText @ "-") @ ServerBrowserTitle;
                // End:0x8C
                break;
            // End:0x89
            case 2:
                TitleText = (RestrictDedicatedText @ "-") @ ServerBrowserTitle;
                // End:0x8C
                break;
            // End:0xFFFF
            default:
...
function SetSwapButton()
{
    switch(Factory.SearchConstraint)
    {
        ...
        case 1:
            ...
            if(ObjectDestroy())
            {
                Localize("--------Friends => None------------");
                KeyButtons[3].SetText(RestrictNoneText);                
            }
            else
            {
                Localize("--------Friends => Dedicated------------");
                KeyButtons[3].SetText(RestrictDedicatedText);
            }
            // End:0xEB
            break;
...
function Tick(float Delta)
{
    ...
    if(ObjectIsDestroyed())
    {
        ...
        {
            // End:0xEA
            if(! GetPlayerOwner().IsSprinting(false) && ! GetPlayerOwner().GetSprintSpeedThreshold())
            {
                DukeConsole(Root.Console).DialogMgr.CloseAllDialogs();
                NavigateBack();
            }
...
function NotifyScreenJoinRoomFailed(int in_Reason)
{
    ...
    // End:0x75
    if(failedServerIdx < string(Factory.Servers))
    {
...
{
    Localize("UAgentSceneBrowserList::OnShutdown()");
    Grid.InterruptQuery();
    return;
}
