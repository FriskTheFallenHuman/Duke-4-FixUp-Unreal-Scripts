...
{
    ServerPing = GetPlayerOwner().GetStaticMeshInstance().EmptyTouchClasses(class'UDukeServerPing');
...
function PingDone(bool bInitial, bool bJustThisServer, bool bSuccess, bool bNoSort)
{
    ...
    else
    {
        ...
        {
            Localize("Unsorted PingDone lost as it's not in ping list!");
        }
    }
    // End:0x1B5
    if(Sentinel == none)
...
function PingNext(bool bInitial, bool bNoSort)
{
    ...
    else
    {
        ...
        if(TotalPinging < MaxSimultaneousPing)
        {
            ...
            {
                // End:0x178
                if((((! l.bHidden && ! l.bPinging) && ! l.bPinged) && ! bInitial || ! l.bNoInitalPing) && TotalPinging < MaxSimultaneousPing)
                {
                    ++ TotalPinging;
