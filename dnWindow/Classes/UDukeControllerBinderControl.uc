...
function DrawKey(Canvas C, float X, float Y, int i)
{
    ...
    DrawStretchedTexture(C, X, Y, KeyHeight, KeyHeight, XboxButtonTex[BoundKeys[i] - int(Root.Console.200)], 1);
    return;
}
...
function SetKey(int KeyNo)
{
    ...
    {
        OldKeyName = GetPlayerOwner().ConsoleCommand("KEYNAME" @ string(BoundKeys[SelectedIndex]));
        KeyName = GetPlayerOwner().ConsoleCommand("KEYNAME " $ string(KeyNo));
        // End:0x118
        if(OldKeyName != "")
        {            
            GetPlayerOwner().ConsoleCommand(("set input" @ OldKeyName) @ "");
            // End:0x118
            if(KeyName != "")
            {
                Alias = GetPlayerOwner().ConsoleCommand("KEYBINDING " $ KeyName);
                ...
                if(Alias != "")
                {                    
                    GetPlayerOwner().ConsoleCommand(("set input" @ OldKeyName) @ Alias);
                }
            }
        }
        // End:0x158
