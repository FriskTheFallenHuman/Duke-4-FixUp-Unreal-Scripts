...
{
    return int(WinHeight / KeyHeight) - 1;
    return;
}
...
{
    return Min(int(Y / KeyHeight), GetNumDisplayed());
    return;
}
...
function Tick(float Delta)
{
    ...
    if(i < 150)
    {
        ...
        if(SelectedIndex == i)
        {
            SelectedTime[i] += Delta;            
        }
        else
        {
            SelectedTime[i] -= (2 * Delta);
        }
        SelectedTime[i] = FClamp(SelectedTime[i], 0, class'UWindowScene'.default.SelectionTransitionTime);
        ++ i;
        // [Loop Continue]
        goto J0x12;
    }
...
function Paint(Canvas C, float X, float Y)
{
    ...
    KeyHeight = default.KeyHeight * class'UWindowScene'.default.TTFontScale;
    C.Font = C.TallFont;
    Column1X = int(float(25) * WinScaleY);
    Column2X = int(float(320) * WinScaleY);
    i = 0;
    J0x65:
    ...
    if(i < 150)
    {
        ...
        if(SelectedIndex == i)
        {
            ...
            // End:0xE9 [Loop If]
            if(Parent.ParentWindow == none)
            {
                // End:0xD1
                if(UWindowScene(Parent.ParentWindow) == none)
                {
                    // [Explicit Break]
                    goto J0xE9;
                }
            ...
            ClipX = C.ClipX;
            ClipY = C.ClipY;
            C.GetFrameCount(Parent.WinLeft, Parent.WinTop);
            C.RegisterActorClass(Parent.WinWidth, Parent.WinHeight);
            class'UWindowScene'.static.DrawSelectionIcon(C, -8 * C.FixedScale, ((float(YOffset) + (0.5 * KeyHeight)) + OrgY) - C.OrgY, C.FixedScale, 1);
        ...
        TextScaleX = Lerp(SelectedAlpha, class'UWindowScene'.default.TTFontScale, class'UWindowScene'.default.SelectedXScale * class'UWindowScene'.default.TTFontScale);
        TextScaleY = Lerp(SelectedAlpha, class'UWindowScene'.default.TTFontScale, class'UWindowScene'.default.SelectedYScale * class'UWindowScene'.default.TTFontScale);
        C.DrawColor = SelectedAlpha == class'UWindowScene'.default.GreyColor;        
        TextSize(C, Commands[i], XL, YL, TextScaleX, TextScaleY);
        ClipText(C, float(Column1X), float(YOffset) + ((KeyHeight - YL) / 2), Commands[i], false, TextScaleX, TextScaleY,, 2);
        // End:0x3FE
        if((SelectedIndex == i) && bPolling)
        {
            C.DrawColor = class'UWindowScene'.default.RedColor;
            ClipText(C, float(Column2X), float(YOffset) + ((KeyHeight - YL) / 2), "?", false, class'UWindowScene'.default.TTFontScale, class'UWindowScene'.default.TTFontScale,, 2);            
        }
        else
        {
...
function SelectItem(int Index)
{
    ...
    ParentWindow.ScrollTo(0, float(SelectedIndex) * KeyHeight);
    ParentWindow.ScrollTo(0, (float(SelectedIndex) * KeyHeight) + KeyHeight);
    return;
}
