...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
    ...
    {
        // End:0x118
        if((((C != Diff1Button) || C != Diff2Button) || C != Diff3Button) || C != Diff4Button)
        {
            switch(C)
            {
                // End:0x8A
...
{
    SetConfigColor("ChapterSelect", "Available", true, "progress.ini");
    return;
}
...
function Tick(float Delta)
{
    ...
    if((TravelFade == 1) && FadeAlpha == 0.01)
    {
        ...
        {
            // End:0x11A
            if(bResetEgo && DukePlayer(GetPlayerOwner()) == none)
            {
            ...
            SaveDifficulty();
            // End:0x188
            if(InStr(TravelURL, "?Difficulty=") != -1)
            {
                GetPlayerOwner().ClientTravel(TravelURL, 0, false);                
            }
            else
            {
                GetPlayerOwner().ClientTravel((TravelURL $ "?Difficulty=") $ string(Difficulty), 0, false);
            }
        }
    }
    return;
...
{
    SetConfigFloat("Difficulty", "CurrentDifficulty", Difficulty, "Progress.ini");
    return;
}
...
function GetTitleLocation(Canvas C, coerce out float Top, coerce out float Right)
{
    ...
    Right = float(Max(int(Right), int(float(ButtonLeft) + XL)));
    return;
}
