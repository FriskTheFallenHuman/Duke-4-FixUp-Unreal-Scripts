...
function Paint(Canvas C, float X, float Y)
{
    ...
    {
        C.DrawColor.R = byte(Clamp(int(C.DrawColor.R) + 100, 0, 255));
        C.DrawColor.G = byte(Clamp(int(C.DrawColor.G) + 100, 0, 255));
        C.DrawColor.B = byte(Clamp(int(C.DrawColor.B) + 100, 0, 255));
        TextColor.R = 255;
        TextColor.G = 255;
        TextColor.B = 0;        
    }
    ...
    // End:0x1B3
    if(LabelWidth == float(0))
    {
        LabelWidth = WinWidth;
    }
    // End:0x1CA
    if(LabelHeight == float(0))
    {
        LabelHeight = WinHeight;
    }
    C.DrawColor = TextColor;
    C.DrawColor.R = byte(float(C.DrawColor.R) * FadeFactor);
    C.DrawColor.G = byte(float(C.DrawColor.G) * FadeFactor);
    C.DrawColor.B = byte(float(C.DrawColor.B) * FadeFactor);
    TextSize(C, Text, Wx, Hy);
    // End:0x2C1
    if(bLeftJustify)
    {
        ClipText(C, xOffset, (LabelHeight - Hy) / float(2), Text);        
    }
    else
    {
        ClipText(C, (LabelWidth - Wx) / float(2), (LabelHeight - Hy) / float(2), Text);
    }
    C.DrawColor.R = 255;
    C.DrawColor.G = 255;
    C.DrawColor.B = 255;
