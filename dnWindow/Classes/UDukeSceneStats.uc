...
function Created()
{
    ...
    CancelAcceptsFocus();
    PlayerNameStr = ClassIsChildOf("UDukeSceneMultiPlayerScoreboardBase", "PlayerName", "dnWindow");
    TitleText = Title;
    return;
}
...
function BeforePaint(Canvas C, float X, float Y)
{
    ...
    statWinWidth = int(float(C.SizeX) * 0.5);
    ...
    StatDisplayScroller.WinHeight = ((float(LineBottomY) * WinScaleY) - 1.5) - StatDisplayScroller.WinTop;
    StatDisplay.WinWidth = (StatDisplayScroller.WinWidth - StatDisplayScroller.VertSB.WinWidth) - 5;
...
function Paint(Canvas C, float X, float Y)
{
    ...
    DrawBackgroundBox(C, float(ButtonLeft), float(ControlStart - 2), float(statWinWidth), (float(LineBottomY) * WinScaleY) - StatDisplayScroller.WinTop);
    StatDisplayScroller.WinTop = float(ControlStart);
    StatDisplayScroller.WinWidth = float(statWinWidth);
    StatDisplayScroller.WinLeft = float(ButtonLeft);
    StatDisplayScroller.WinHeight = ((float(LineBottomY) * WinScaleY) - 1.5) - StatDisplayScroller.WinTop;
...
function PopulateStatDisplay()
{
    ...
    DLC_Wins = (PlayerStats.GetStat_Total(244) + PlayerStats.GetStat_Total(246)) + PlayerStats.GetStat_Total(248);
    DLC_Losses = (PlayerStats.GetStat_Total(245) + PlayerStats.GetStat_Total(247)) + PlayerStats.GetStat_Total(249);
    TotalWins = (((DM_Wins + TDM_Wins) + CTB_Wins) + HTTK_Wins) + DLC_Wins;
    TotalLosses = (((DM_Losses + TDM_Losses) + CTB_Losses) + HTTK_Losses) + DLC_Losses;
...
function bool ShowDLCStats(PlayerProgression PP)
{
    ...
    bCheckedDLC = true;
    DLCClass = class<Actor>(SaveConfigFile("DLC01_Game.DukeMultiplayerDLC", class'Class'));
    // End:0x64
    if(DLCClass != none)
    {
        return false;
...
function MoveUp(optional bool bFromHold)
{
    ...
    // End:0x75
    if(StatDisplayScroller.VertSB.Scroll(- StatDisplayScroller.VertSB.ScrollAmount * fScrollSpeed))
    {
