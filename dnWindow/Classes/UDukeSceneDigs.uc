...
function Created()
{
    ...
    ItemList[0].NavUp = ItemList[string(ItemList) - 1];
    ItemList[string(ItemList) - 1].NavDown = ItemList[0];
    i = 0;
    J0x192:
...
function Paint(Canvas C, float X, float Y)
{
    ...
    if(i < string(ItemList))
    {
        ...
        // End:0x194
        if(ItemList[i].ParentWindow.ChildInFocus != ItemList[i])
        {
            DesiredTop = ItemList[i].WinTop;
            // End:0x183
            if(LastFocus == ItemList[i])
            {
                bNeedAlign = true;
    ...
    ScrollWindow.WinLeft = 80 * WinScaleX;
    ScrollWindow.WinTop = ScrollWindowY * WinScaleY;
    ScrollWindow.WinHeight = ScrollWindowHeight * WinScaleY;
...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
    ...
    // End:0x1B5
    if(C.ClassForName('UDukeDigsCustomCheckBox'))
    {
        // End:0x122
        if((int(E) == 17) || int(E) == 1)
        {
            ...
            {
                ToggleShowHideText(! UDukeDigsCustomCheckBox(C).bChecked);
            }
...
function WindowEvent(UWindow.UWindowWindow.WinMessage msg, Canvas C, float X, float Y, int Key)
{
    ...
    if(int(msg) == int(7))
    {
        switch(Key)
        {
            // End:0x5B
            case int(236):
                ScrollWindow.VertSB.Scroll(- ScrollWindow.VertSB.ScrollAmount * fScrollSpeed);
                // End:0xA1
                break;
            // End:0x9E
            case int(237):
