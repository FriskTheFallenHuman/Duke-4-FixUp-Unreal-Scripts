...
function Paint(Canvas C, float X, float Y)
{
    ...
    C.Font = C.TallFont;
    NewScale = class'UWindowScene'.default.TTFontScale * NewTextScale;
    TextSize(C, NewTag, NewXL, NewYL, NewScale, NewScale);
    // End:0xBF
    if(int(ItemState) == int(1))
    {
        C.DrawColor = NewTextColor;
        ClipText(C, TextX, (WinHeight - NewYL) / 2, NewTag,, NewScale, NewScale, Alpha, 2);
    }
    ...
    if(int(ItemState) == int(2))
    {
        ...
        DrawStretchedTextureSegment(C, CheckX, (WinHeight - LockHeight) / float(2), LockWidth * class'UWindowScene'.default.WinScaleX, LockHeight * class'UWindowScene'.default.WinScaleY, float(LockRegion.X), float(LockRegion.Y), float(LockRegion.W), float(LockRegion.h), LockIcon, Alpha,,,,, true);        
    }
    else
    {
