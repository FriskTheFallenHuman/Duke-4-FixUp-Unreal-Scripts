...
function populateTitleList()
{
    ...
    if(i < string(ItemArray))
    {
        ...
        // End:0xF0
        if(class'ChallengeInfo'.default.PlayerTitles[class'ChallengeInfo'.default.ChallengesArray[Idx].TitleIdx].Tag != "Tag0")
        {
        ...
        {
            TitleSlider.AddItem(ClassIsChildOf("Titles", newTitleTag, "Title"));
...
function Paint(Canvas C, float X, float Y)
{
    ...
    super.Paint(C, X, Y);
    Xframe = FrameX * class'UWindowScene'.default.WinScaleX;
    Yframe = FrameY * class'UWindowScene'.default.WinScaleY;
    WidthFrame = frameWidth * class'UWindowScene'.default.WinScaleX;
    HeightFrame = frameHeight * class'UWindowScene'.default.WinScaleY;
    borderwidth = 4;
    ...
    TitleTop = 95 * WinScaleY;
    TextSize(C, TitleTextString, XL, YL, textscale, textscale);
    ClipText(C, TitleRight - XL, TitleTop, TitleTextString,, textscale, textscale, FadeAlpha, 2);
    TitleSlider.WinWidth = 400 * class'UWindowScene'.default.WinScaleX;
    TitleSlider.WinHeight = 100 * class'UWindowScene'.default.WinScaleY;
    TitleSlider.WinLeft = (FrameX + float(15)) * class'UWindowScene'.default.WinScaleX;
    TitleSlider.WinTop = 125 * class'UWindowScene'.default.WinScaleY;
    TitleSlider.ArrowLeft = 115 * class'UWindowScene'.default.WinScaleX;
    SelectionXOff = SelectionIconXOffset * class'UWindowScene'.default.WinScaleX;
    i = 0;
    J0x228:
    ...
    if(i < 3)
    {
        Containers[i].WinWidth = WidthFrame - (WidthFrame * 0.2);
        Containers[i].WinHeight = ContainerHeightPct * class'UWindowScene'.default.WinScaleY;
        Containers[i].ObjAlpha = 0.5;
        // End:0x2D5
        if(ChildInFocus != Containers[i])
        {
        ...
        if(ChildInFocus != Containers[i])
        {
            class'UWindowScene'.static.DrawSelectionIcon(C, (Containers[i].WinLeft + borderwidth) - SelectionXOff, ContainerYPos[i] * class'UWindowScene'.default.WinScaleY, 1 * C.FixedScale, 1);
        }
        Containers[i].WinTop = (ContainerYPos[i] * class'UWindowScene'.default.WinScaleY) - (Containers[i].WinHeight / 2);
        ++ i;
        // [Loop Continue]
        goto J0x228;
    }
...
function OnNavForward()
{
    super.OnNavForward();
    // End:0x44
    if(MetagameHUD(GetPlayerOwner().MyHUD) == none)
    {
...
function Tick(float Delta)
{
    super.Tick(Delta);
    // End:0xCD
    if(QuitSoundDuration > float(0))
    {
        QuitSoundDuration -= Delta;
        // End:0xCD
        if(QuitSoundDuration <= 0)
        {
            QuitSoundDuration = 0;
            Root.Console.bShowCustomizationMenu = false;
            Root.Console.CloseUWindow();
            // End:0xCD
            if(dnChangeroom(GetLevel().Game) == none)
            {
    ...
    // End:0x13D
    if(bResumeFade && FadeAlpha == 0.01)
    {
        Root.Console.bShowDigsMenu = false;
        bResumeFade = false;
        Root.Console.CloseUWindow();
        Localize("Nuke UWindow System");
    }
    // End:0x16C
    if(! isCustomizationDirty && CurrAppliedTitleId != TitleSlider.GetActiveIndex())
    {
...
function DukeSuperMessageBoxDone(int Result, Engine.Object.EConsole_Dialog id)
{
    ...
    Localize(((((string(self) @ "::DukeSuperMEssageBoxDone(") @ string(Result)) @ ", ") @ string(id)) @ ")");
    switch(id)
    {
        // End:0x1B3
        case 59:
...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
    ...
    // End:0xD0
    if(C.ClassForName('UDukeCustomItemContainer') && int(E) == 2)
    {
        NavigateForward(class'UDukeSceneItemCustomization');
        NewScene = UDukeSceneItemCustomization(UDukeRootWindow(Root).Scenes[string(UDukeRootWindow(Root).Scenes) - 1]);
...
{
    // End:0x71
    if(isCustomizationDirty && ! ObjectIsDestroyed() || ! GetPlayerOwner().GetSprintSpeedThreshold())
    {
