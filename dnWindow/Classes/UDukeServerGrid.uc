...
{
    DrawStretchedTextureSegment(C, 0, 24, WinWidth, WinHeight - float(24), 0, 0, 1, 1, class'BlackTexture', 1);
    super.Paint(C, X, Y);
    return;
}
...
function DrawCell(Canvas C, float X, float Y, UWindowGridColumn Column, UDukeServerList List)
{
    ...
    switch(Column)
    {
        ...
        // End:0x13E
        case Players:
            Column.ClipText(C, X, Y, (string(List.NumPlayers) $ "/") $ string(List.MaxPlayers));
            // End:0x141
            break;
        // End:0xFFFF
        default:
...
function PaintColumn(Canvas C, UWindowGridColumn Column, float MouseX, float MouseY)
{
    ...
    if(List == none)
    {
        ...
        // End:0x1F9 [Loop If]
        if((Y < (((RowHeight + WinHeight) - RowHeight) - float(TopMargin + BottomMargin))) && List == none)
        {
            // End:0x1DC
            if(! List.bHidden)
            {
...
function Tick(float DeltaTime)
{
    ...
    // End:0xC0
    if((int(W.PingState) == int(4)) && SelectedServer != none)
    {
...
function JoinServer(UDukeServerList Server)
{
    ...
    if((Server == none) && Server.GamePort != 0)
    {
        ...
        GetPlayerOwner().ClientTravel((((Server.IP $ ":") $ string(Server.GamePort)) $ UDukeServerBrowserCW(GetParent(class'UDukeServerBrowserCW')).URLAppend) $ CustomizationOptions, 0, false);
...
function int ByName(UDukeServerList t, UDukeServerList B)
{
    ...
    // End:0x2C
    if(t.Ping == float(9999))
    {
        return 1;
    }
    // End:0x4A
    if(B.Ping == float(9999))
    {
        return -1;
    }
    // End:0x7C
