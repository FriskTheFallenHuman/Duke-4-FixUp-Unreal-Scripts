...
function Real_DrawWindowFrame(UWindowFramedWindow W, Canvas C, Texture t, Texture T3, bool bGlow)
{
    ...
    {
        R = FrameT;
        W.DrawStretchedTextureSegment(C, float(FrameTL.W), 0, (W.WinWidth - float(FrameTL.W)) - float(FrameTR.W), float(FrameT.h), float(FrameT.X), float(FrameT.Y), float(FrameT.W), float(FrameT.h), UseT, 1);        
    }
    else
    {
        C.Font = C.TallFont;
        ...
        TitleOver = TitleEnd - float(107);
        // End:0x22C
        if(TitleOver > float(0))
        {
            TitleOver += (float(5) - (TitleOver % float(5)));
            R = FrameT;
        ...
        TrueRestLength = ((W.WinWidth - float(FrameTL.W)) - float(FrameTR.W)) - TitleOver;
        RestLength = TrueRestLength;
        // End:0x389
        if(RestLength > float(0))
        {
            RestLength -= (RestLength % float(5));
            R = FrameT2;
            W.DrawHorizTiledPieces(C, float(FrameTL.W) + TitleOver, 0, RestLength, float(FrameT2.h), R, UseT, 1);
            // End:0x389
            if((TrueRestLength - RestLength) > float(0))
            {
                R = FrameT3;
                W.DrawStretchedTextureSegment(C, (float(FrameTL.W) + TitleOver) + RestLength, 0, TrueRestLength - RestLength, float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), UseT, 1);
            }
        }
    }
    R = FrameTR;
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), 0, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), UseT, 1);
    // End:0x759
    if(W.bStatusBar)
    {
        R = FrameL;
        W.DrawStretchedTextureSegment(C, 0, float(FrameTL.h), float(R.W), (W.WinHeight - float(FrameTL.h)) - float(FrameSBL.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        R = FrameR;
        W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), float(FrameTL.h), float(R.W), (W.WinHeight - float(FrameTR.h)) - float(FrameSBR.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        R = FrameSBL;
        W.DrawStretchedTextureSegment(C, 0, W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), T3, 1);
        R = FrameSB;
        W.DrawStretchedTextureSegment(C, float(FrameSBL.W), W.WinHeight - float(R.h), (W.WinWidth - float(FrameSBL.W)) - float(FrameSBR.W), float(FrameSB.h), float(FrameSB.X), float(FrameSB.Y), float(FrameSB.W), float(FrameSB.h), T3, 1);
        R = FrameSBR;
        W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), T3, 1);        
    }
    else
    {
        R = FrameL;
        W.DrawStretchedTextureSegment(C, 0, float(FrameTL.h), float(R.W), (W.WinHeight - float(FrameTL.h)) - float(FrameBL.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        R = FrameR;
        W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), float(FrameTL.h), float(R.W), (W.WinHeight - float(FrameTR.h)) - float(FrameBR.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        R = FrameBL;
        W.DrawStretchedTextureSegment(C, 0, W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        R = FrameB;
        W.DrawStretchedTextureSegment(C, float(FrameBL.W), W.WinHeight - float(R.h), (W.WinWidth - float(FrameBL.W)) - float(FrameBR.W), float(FrameB.h), float(FrameB.X), float(FrameB.Y), float(FrameB.W), float(FrameB.h), t, 1);
        R = FrameBR;
        W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    }
    return;
}
...
function DrawTitleAndStatus(UWindowFramedWindow W, Canvas C)
{
    ...
    if(W.bStatusBar)
    {
        ...
        W.ClipText(C, 28, W.WinHeight - float(43), W.StatusBarText);
    }
    return;
}
...
function Tab_DrawTab(UWindowTabControlTabArea Tab, Canvas C, bool bActiveTab, bool bLeftmostTab, float X, float Y, float W, float h, string Text, bool bShowText)
{
    ...
    if(bActiveTab)
    {
        ...
        R = TabMid;
        R = TabRight;
        Tab.DrawStretchedTextureSegment(C, (X + W) - float(R.W), Y, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        C.DrawColor = GetTextColor(Tab);
        // End:0x1FD
        if(bShowText)
        {
            Tab.TextSize(C, Text, tW, tH);
            Tab.ClipText(C, X + ((W - tW) / float(2)), Y + ((float(TabMid.h) - tH) / float(2)), Text, true);
        }        
    }
    else
    {
        ...
        Tab.DrawStretchedTextureSegment(C, X, Y, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        R = TabMidUp;
        Tab.DrawStretchedTextureSegment(C, X + float(TabLeftUp.W), Y, (W - float(TabLeftUp.W)) - float(TabLeftUp.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        R = TabRightUp;
        Tab.DrawStretchedTextureSegment(C, (X + W) - float(R.W), Y, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        C.DrawColor = GetTextColor(Tab);
...
function Tab_SetTabPageSize(UWindowPageControl W, UWindowPageWindow P)
{
    ...
    P.SetSize(W.WinWidth - float(4), (W.WinHeight - (W.TabArea.WinHeight - float(TabMid.h))) - float(6));
    return;
}
...
function Editbox_SetupSizes(UWindowEditControl W, Canvas C)
{
    ...
    switch(W.Align)
    {
        ...
        case 1:
            W.EditAreaDrawX = 0;
            W.TextX = (W.WinWidth - tW) - float(EditBoxTR.W);
            // End:0x1D3
            break;
        // End:0x1D0
        case 2:
            W.EditAreaDrawX = (W.WinWidth - W.EditBoxWidth) / float(2);
            W.TextX = ((W.WinWidth - tW) - float(EditBoxTR.W)) / float(2);
            // End:0x1D3
            break;
        // End:0xFFFF
        default:
    ...
    W.WinHeight = float(EditBoxTL.h + EditBoxBL.h) + tH;
    ...
    W.EditBox.WinTop = float(EditBoxT.h - 1);
    W.EditBox.WinWidth = (W.WinWidth - float(EditBoxTL.W)) - float(EditBoxTR.W);
    W.EditBox.WinHeight = (W.WinHeight - float(EditBoxT.h)) - float(EditBoxB.h);
    W.EditBox.TextY = (W.EditBox.WinHeight - tH) / float(2);
...
function Editbox_Draw(UWindowEditControl W, Canvas C)
{
    ...
    W.DrawStretchedTextureSegment(C, float(EditBoxTL.W), 0, (W.WinWidth - float(EditBoxTL.W)) - float(EditBoxTR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x1E9
    if(W.EditBox.bHasKeyboardFocus)
    {
        R = EditBoxFocusTR;        
    ...
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), 0, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x2A1
    if(W.EditBox.bHasKeyboardFocus)
    {
        R = EditBoxFocusL;        
    ...
    W.DrawStretchedTextureSegment(C, 0, float(EditBoxTL.h), float(R.W), ((W.WinHeight - float(EditBoxTL.h)) - float(EditBoxBL.h)) - float(4), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x36B
    if(W.EditBox.bHasKeyboardFocus)
    {
        R = EditBoxFocusR;        
    ...
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(EditBoxR.W), float(EditBoxTR.h), float(R.W), ((W.WinHeight - float(EditBoxTR.h)) - float(EditBoxBR.h)) - float(4), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x44C
    if(W.EditBox.bHasKeyboardFocus)
    {
        R = EditBoxFocusBL;        
    ...
    W.DrawStretchedTextureSegment(C, 0, (W.WinHeight - float(R.h)) - float(4), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x509
    if(W.EditBox.bHasKeyboardFocus)
    {
        R = EditBoxFocusB;        
    ...
    W.DrawStretchedTextureSegment(C, float(EditBoxBL.W), (W.WinHeight - float(R.h)) - float(4), (W.WinWidth - float(EditBoxBL.W)) - float(EditBoxBR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x5EA
    if(W.EditBox.bHasKeyboardFocus)
    {
        R = EditBoxFocusBR;        
    ...
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), (W.WinHeight - float(R.h)) - float(4), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = EditBoxFill;
    W.DrawStretchedTextureSegment(C, float(EditBoxTL.W), float(EditBoxTL.h), (W.WinWidth - float(EditBoxTL.W)) - float(EditBoxTR.W), ((W.WinHeight - float(EditBoxTL.h)) - float(EditBoxTL.h)) - float(4), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    C.DrawColor = GetTextColor(W);
    C.Style = W.GetPlayerOwner().3;
    C.Font = C.TallFont;
...
function DrawFullBorder(UWindowWindow W, Canvas C, int borderwidth)
{
    ...
    W.DrawStretchedTexture(C, W.WinWidth - float(borderwidth), 0, float(borderwidth), W.WinHeight, t, 1);
    W.DrawStretchedTexture(C, 0, W.WinHeight - float(borderwidth), W.WinWidth, float(borderwidth), t, 1);
    return;
}
...
function DrawBorder(UWindowWindow W, Canvas C, int X, int Y, int Width, int Height, int borderwidth)
{
    ...
    W.DrawStretchedTexture(C, float(Width - borderwidth), 0, float(borderwidth), float(Height), t, 1);
    W.DrawStretchedTexture(C, 0, float(Height - borderwidth), float(Width), float(borderwidth), t, 1);
    return;
}
...
function Bevel_DrawSimpleBevel(UWindowWindow W, Canvas C, int X, int Y, int Width, int Height, float Alpha)
{
    ...
    t = W.GetLookAndFeelTexture();
    R = SimpleBevelTL;
    W.DrawStretchedTextureSegment(C, float(X - SimpleBevelTL.W), float(Y - SimpleBevelTL.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, Alpha);
    R = SimpleBevelT;
    W.DrawStretchedTextureSegment(C, float(X), float(Y - SimpleBevelTL.h), float(Width), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, Alpha);
    R = SimpleBevelTR;
    W.DrawStretchedTextureSegment(C, float(X + Width), float(Y - SimpleBevelTR.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, Alpha);
    R = SimpleBevelL;
    W.DrawStretchedTextureSegment(C, float(X - SimpleBevelL.W), float(Y), float(R.W), float(Height), float(R.X), float(R.Y), float(R.W), float(R.h), t, Alpha);
    R = SimpleBevelR;
    W.DrawStretchedTextureSegment(C, float(X + Width), float(Y), float(R.W), float(Height), float(R.X), float(R.Y), float(R.W), float(R.h), t, Alpha);
    R = SimpleBevelBL;
    W.DrawStretchedTextureSegment(C, float(X - SimpleBevelBL.W), float(Y + Height), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, Alpha);
    R = SimpleBevelB;
    W.DrawStretchedTextureSegment(C, float(X), float(Y + Height), float(Width), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, Alpha);
    R = SimpleBevelBR;
    W.DrawStretchedTextureSegment(C, float(X + Width), float(Y + Height), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, Alpha);
    R = SimpleBevelFill;
...
function Bevel_DrawTabBevel(UWindowWindow W, Canvas C, int X, int Y, int Width, int Height)
{
    ...
    t = W.GetLookAndFeelTexture();
    R = SimpleBevelTR;
    W.DrawStretchedTextureSegment(C, float(X - SimpleBevelTL.W), float(Y - SimpleBevelTL.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelTL;
    W.DrawStretchedTextureSegment(C, float(X + Width), float(Y - SimpleBevelTR.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelL;
    W.DrawStretchedTextureSegment(C, float((X - SimpleBevelL.W) + 5), float(Y), float(R.W), float(Height), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelR;
    W.DrawStretchedTextureSegment(C, float((X + Width) - 5), float(Y), float(R.W), float(Height), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelBL;
    W.DrawStretchedTextureSegment(C, float((X - SimpleBevelBL.W) + 5), float(Y + Height), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelB;
    W.DrawStretchedTextureSegment(C, float(X + 5), float(Y + Height), float(Width - 10), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelBR;
    W.DrawStretchedTextureSegment(C, float((X + Width) - 5), float(Y + Height), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelFill;
...
function Bevel_DrawSplitHeaderedBevel(UWindowWindow W, Canvas C, int X, int Y, int Width, int Height, string Header1, string Header2)
{
    ...
    MiddleLen = float(((Width - BevelHeaderSplit.W) - BevelHeaderTL.W) - BevelHeaderTR.W) / float(2);
    R = BevelHeaderT;
    W.DrawStretchedTextureSegment(C, float(X + BevelHeaderTL.W), float(Y), MiddleLen, float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelHeaderSplit;
    W.DrawStretchedTextureSegment(C, float(X + BevelHeaderTL.W) + MiddleLen, float(Y), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelHeaderT;
    W.DrawStretchedTextureSegment(C, float((X + BevelHeaderTL.W) + BevelHeaderSplit.W) + MiddleLen, float(Y), MiddleLen, float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelHeaderTR;
    W.DrawStretchedTextureSegment(C, float((X + Width) - R.W), float(Y), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelTL;
    W.DrawStretchedTextureSegment(C, float(X), float(Y + BevelHeaderTL.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelT;
    W.DrawStretchedTextureSegment(C, float(X + BevelTL.W), float(Y + BevelHeaderTL.h), float((Width - BevelTL.W) - BevelTR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelTL;
    W.DrawStretchedTextureSegment(C, float((X + Width) - R.W), float(Y + BevelHeaderTR.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelL;
    W.DrawStretchedTextureSegment(C, float(X), float(Y + BevelHeaderTL.h), float(R.W), float((Height - BevelHeaderT.h) - SimpleBevelB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelR;
    W.DrawStretchedTextureSegment(C, float((X + Width) - R.W), float(Y + BevelHeaderTR.h), float(R.W), float((Height - BevelHeaderT.h) - SimpleBevelB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelBL;
    W.DrawStretchedTextureSegment(C, float(X), float((((Y + BevelHeaderTL.h) + Height) - BevelHeaderT.h) - SimpleBevelB.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelB;
    W.DrawStretchedTextureSegment(C, float(X + SimpleBevelBL.W), float((((Y + BevelHeaderTL.h) + Height) - BevelHeaderT.h) - SimpleBevelB.h), float((Width - SimpleBevelBL.W) - SimpleBevelBR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelBR;
    W.DrawStretchedTextureSegment(C, float((X + Width) - R.W), float((((Y + BevelHeaderTL.h) + Height) - BevelHeaderT.h) - SimpleBevelB.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelFill;
    W.DrawStretchedTextureSegment(C, float(X + SimpleBevelL.W), float(Y + BevelHeaderTL.h), float((Width - SimpleBevelL.W) - SimpleBevelR.W), float((Height - BevelHeaderT.h) - SimpleBevelB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelSplit;
    W.DrawVertTiledPieces(C, float(((X + 1) + SimpleBevelL.W) + ((((Width - SimpleBevelL.W) - SimpleBevelR.W) - R.W) / 2)), float(Y + BevelHeaderTL.h), float(R.W), float((Height - BevelHeaderT.h) - 1), R, t, 1);
    C.DrawColor = GetTextColor(W);
    C.Font = C.BlockFontSmall;
    W.TextSize(C, Header1, XL, YL);
    W.ClipText(C, float(X + BevelHeaderTL.W), float(Y) + ((float(BevelHeaderTL.h) - YL) / float(2)), Header1, true);
    C.Font = C.BlockFontSmall;
    W.TextSize(C, Header2, XL, YL);
    W.ClipText(C, (float((X + BevelHeaderTL.W) + BevelHeaderSplit.W) + MiddleLen) + float(BevelHeaderT.W), float(Y) + ((float(BevelHeaderTL.h) - YL) / float(2)), Header2, true);
    return;
}
...
function int Bevel_GetSplitRight(int Width)
{
    ...
    MiddleLen = float(((Width - BevelHeaderSplit.W) - BevelHeaderTL.W) - BevelHeaderTR.W) / float(2);
    return int((float(BevelHeaderTL.W + BevelHeaderSplit.W) + MiddleLen) + float(BevelHeaderT.W));
    return;
}
...
function Grid_SizeGrid(UWindowGrid W)
{
    ...
    W.ClientArea.WinWidth = W.WinWidth - float(SBPosIndicator.W);
    // End:0x134
    if(W.bShowHorizSB)
    {
        W.ClientArea.WinHeight = W.WinHeight - float(SBPosIndicator.h);        
    }
    else
    {
    ...
    W.VertSB.SetSize(float(SBPosIndicatorBevel.W), (W.WinHeight - W.VertSB.WinTop) - float(4));
    W.VertSB.WinLeft = (W.WinWidth - float(SBPosIndicatorBevel.W)) - float(4);
    Offset = 0;
    colColumn = W.FirstColumn;
    J0x234:
...
function Grid_DrawGrid(UWindowGrid W, Canvas C)
{
    ...
    colColumn = W.FirstColumn;
    DrawWidth = colColumn.WinWidth - float(BevelHeaderSplit.W);
    J0x158:
    ...
    if(colColumn == none)
    {
        ...
        {
            DrawWidth = (W.WinWidth - Offset) - float(BevelHeaderTR.W);
        }
        R = BevelHeaderT;
        ...
        if(colColumn.NextColumn == none)
        {
            ...
            Offset += float(BevelHeaderSplit.W);
        }
        colColumn = colColumn.NextColumn;
        // End:0x300
        if(colColumn == none)
    ...
    R = BevelHeaderTR;
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(BevelHeaderTR.W), 0, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelTL;
    W.DrawStretchedTextureSegment(C, 0, float(BevelHeaderTL.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelT;
    W.DrawStretchedTextureSegment(C, float(BevelTL.W), float(BevelHeaderTL.h), (W.WinWidth - float(BevelTL.W)) - float(BevelTR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = BevelTL;
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), float(BevelHeaderTR.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelL;
    W.DrawStretchedTextureSegment(C, 0, float(BevelHeaderTL.h), float(R.W), (W.WinHeight - float(BevelHeaderT.h)) - float(SimpleBevelB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelR;
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), float(BevelHeaderTR.h), float(R.W), (W.WinHeight - float(BevelHeaderT.h)) - float(SimpleBevelB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelBL;
    W.DrawStretchedTextureSegment(C, 0, ((float(BevelHeaderTL.h) + W.WinHeight) - float(BevelHeaderT.h)) - float(SimpleBevelB.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelB;
    W.DrawStretchedTextureSegment(C, float(SimpleBevelBL.W), ((float(BevelHeaderTL.h) + W.WinHeight) - float(BevelHeaderT.h)) - float(SimpleBevelB.h), (W.WinWidth - float(SimpleBevelBL.W)) - float(SimpleBevelBR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelBR;
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), ((float(BevelHeaderTL.h) + W.WinHeight) - float(BevelHeaderT.h)) - float(SimpleBevelB.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = SimpleBevelFill;
    W.DrawStretchedTextureSegment(C, float(SimpleBevelL.W), float(BevelHeaderTL.h), (W.WinWidth - float(SimpleBevelL.W)) - float(SimpleBevelR.W), (W.WinHeight - float(BevelHeaderT.h)) - float(SimpleBevelB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, W.FillAlpha,,,,, true);
    colColumn = W.FirstColumn;
    Offset = colColumn.WinWidth;
    J0x9B1:
    ...
    if(colColumn == none)
    {
        ...
        {
            R = BevelSplit;
            W.DrawVertTiledPieces(C, Offset, float(BevelHeaderT.h), float(R.W), ((W.WinHeight - float(BevelHeaderT.h)) - float(BevelSplitB.h)) + float(1), R, t, 1);
            R = BevelSplitB;
            W.DrawStretchedTextureSegment(C, Offset, ((float(BevelHeaderTL.h) + W.WinHeight) - float(BevelHeaderT.h)) - float(BevelSplitB.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        }
        colColumn = colColumn.NextColumn;
        // End:0xB28
        if(colColumn == none)
...
function Grid_DrawBrowserGrid(UWindowGrid W, Canvas C)
{
    ...
    if(colColumn == none)
    {
        Bevel_DrawSimpleBevel(W, C, int(colColumn.WinLeft + float(6)), 6, int(colColumn.WinWidth - float(12)), 10, 1);
        colColumn = colColumn.NextColumn;
        // [Loop Continue]
        goto J0x15;
    }
...
function Button_AutoSize(UWindowSmallButton W, Canvas C)
{
    ...
    W.WinWidth = float(ButtonUpTL.W + ButtonUpBL.W) + XL;
    // End:0xC5
    if(W.WinWidth < float(36))
    {
        W.WinWidth = 36;
    }
    // End:0x109
    if(W.Font == 6)
    {
        W.WinHeight = float((ButtonUpTL.h + ButtonUpBL.h) - 4);        
    }
    else
    {
        W.WinHeight = float(ButtonUpTL.h + ButtonUpBL.h);
    }
    return;
}
...
function Button_DrawSmallButton(UWindowSmallButton W, Canvas C, bool IsFocused)
{
    ...
    W.DrawStretchedTextureSegment(C, 0, 0, float(R.W), float(R.h) - Offset, float(R.X), float(R.Y), float(R.W), float(R.h) - Offset, t, 1);
    // End:0x190
    if(W.bMouseDown)
    {
        R = ButtonDownT;        
    ...
    W.DrawStretchedTextureSegment(C, float(ButtonUpTL.W), 0, (W.WinWidth - float(ButtonUpTL.W)) - float(ButtonUpTR.W), float(R.h) - Offset, float(R.X), float(R.Y), float(R.W), float(R.h) - Offset, t, 1);
    // End:0x259
    if(W.bMouseDown)
    {
        R = ButtonDownTR;        
    ...
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), 0, float(R.W), float(R.h) - Offset, float(R.X), float(R.Y), float(R.W), float(R.h) - Offset, t, 1);
    // End:0x315
    if(W.bMouseDown)
    {
        R = ButtonDownBL;        
    ...
    W.DrawStretchedTextureSegment(C, 0, W.WinHeight - float(R.h), float(R.W), float(R.h) - Offset, float(R.X), float(R.Y) + Offset, float(R.W), float(R.h), t, 1);
    // End:0x3D1
    if(W.bMouseDown)
    {
        R = ButtonDownB;        
    ...
    W.DrawStretchedTextureSegment(C, float(ButtonUpBL.W), W.WinHeight - float(R.h), (W.WinWidth - float(ButtonUpBL.W)) - float(ButtonUpBR.W), float(R.h) - Offset, float(R.X), float(R.Y) + Offset, float(R.W), float(R.h), t, 1);
    // End:0x4B1
    if(W.bMouseDown)
    {
        R = ButtonDownBR;        
    ...
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), W.WinHeight - float(R.h), float(R.W), float(R.h) - Offset, float(R.X), float(R.Y) + Offset, float(R.W), float(R.h), t, 1);
    return;
}
...
function SB_VSetSize(UWindowVScrollbar W, out float ThumbY, out float ThumbHeight)
{
    ...
    MoveHeight = W.WinHeight - Magic1;
    Denom = (W.MaxPos + W.MaxVisible) - W.MinPos;
    ThumbY = (W.pos - W.MinPos) / Denom;
    ThumbY *= MoveHeight;
    ThumbHeight = W.MaxVisible / Denom;
    ThumbHeight *= MoveHeight;
    // End:0x13C
    if(ThumbHeight < float(SBPosIndicatorBevelT.h * 2))
    {
        ThumbHeight = float(SBPosIndicatorBevelT.h * 2);
    }
    ThumbY += (float(SBPosIndicatorBevelT.h * 2) + Magic2);
...
function SB_VDraw(UWindowVScrollbar W, Canvas C)
{
    ...
    W.DrawStretchedTextureSegment(C, 0, 0, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = VSlideMid;
    W.DrawStretchedTextureSegment(C, 0, float(VSlideTop.h), float(R.W), (W.WinHeight - float(VSlideBot.h)) - float(VSlideTop.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = VSlideBot;
    W.DrawStretchedTextureSegment(C, 0, W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x3F6
    if(! W.bDisabled)
    {
        ...
        R = SBPosIndicatorM;
        W.DrawStretchedTextureSegment(C, 0, ThumbY + float(SBPosIndicatorT.h), float(R.W), ThumbHeight - float(SBPosIndicatorB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
        R = SBPosIndicatorB;
        W.DrawStretchedTextureSegment(C, 0, (ThumbY + ThumbHeight) - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    }
    // End:0x489
    ...
    {
        R = VScrollDownDown;
        W.DrawStretchedTextureSegment(C, 1, (W.WinHeight - float(SBDownUp.h)) + float(1), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    }
    return;
}
...
function SB_VDrawSmall(UWindowVScrollbar W, Canvas C)
{
    ...
    W.DrawStretchedTextureSegment(C, 0, 0, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = VSlideSmallMid;
    W.DrawStretchedTextureSegment(C, 0, float(VSlideSmallTop.h), float(R.W), (W.WinHeight - float(VSlideSmallBot.h)) - float(VSlideSmallTop.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = VSlideSmallBot;
    W.DrawStretchedTextureSegment(C, 0, W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x20A
    if(! W.bDisabled)
    ...
    if(W.DownButton.bMouseDown)
    {
        ...
        W.DrawStretchedTextureSegment(C, 1, W.WinHeight - float(SBDownUp.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    }
    return;
}
...
function SB_VDrawBevel(UWindowVScrollbar W, Canvas C)
{
    ...
    // End:0x28
    if(W.WinHeight < (float(VSlideBevelTop.h) * 2.8))
    {
        return;
    }
    C.DrawColor = GetGUIColor(W);
    ...
    W.DrawStretchedTextureSegment(C, 0, 0, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = VSlideBevelMid;
    W.DrawStretchedTextureSegment(C, 0, float(VSlideBevelTop.h), float(R.W), (W.WinHeight - float(VSlideBevelBot.h)) - float(VSlideBevelTop.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = VSlideBevelBot;
    W.DrawStretchedTextureSegment(C, 0, W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x3D1
    if(! W.bDisabled)
    {
        ...
        W.DrawStretchedTextureSegment(C, 0, ThumbY + float(SBPosIndicatorBevelT.h), float(R.W), ThumbHeight - float(SBPosIndicatorBevelB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    ...
    if(W.DownButton.bMouseDown)
    {
        ...
        W.DrawStretchedTextureSegment(C, 4, (W.WinHeight - float(SBDownUp.h)) - float(2), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    }
    return;
}
...
function HSlider_AutoSize(UWindowHSliderControl W, Canvas C)
{
    ...
    W.SliderTrackWidth = ((W.SliderWidth - W.TrackWidth) - float(HSliderL.W)) - float(HSliderR.W);
    W.SliderTrackX = W.SliderDrawX + float(HSliderL.W);
    W.TrackStart = (W.SliderTrackX + float(HSliderThumb.W / 2)) + ((((W.SliderWidth - W.TrackWidth) - float(HSliderL.W)) - float(HSliderR.W)) * ((W.Value - W.MinValue) / (W.MaxValue - W.MinValue)));
    return;
}
...
function HSlider_Draw(UWindowHSliderControl W, Canvas C, bool IsFocused)
{
    ...
    R = HSlider;
    W.DrawStretchedTextureSegment(C, W.SliderDrawX + float(HSliderL.W), W.SliderDrawY, (W.SliderWidth - float(HSliderL.W)) - float(HSliderR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = HSliderR;
    W.DrawStretchedTextureSegment(C, (W.SliderDrawX + W.SliderWidth) - float(R.W), W.SliderDrawY, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = HSliderThumb;
    W.DrawStretchedTextureSegment(C, W.TrackStart - float(R.W / 2), W.SliderDrawY, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    C.Font = C.BlockFontSmall;
    C.DrawColor = GetTextColor(W);
    ...
    W.ClipText(C, (W.SliderDrawX + W.SliderWidth) + float(2), (W.WinHeight - YL) / float(2), W.ValueString);
...
function Combo_SetupSizes(UWindowComboControl W, Canvas C)
{
    ...
    W.WinHeight = float(ComboUpTL.h + ComboUpBL.h);
    W.EditBoxWidth = (W.WinWidth - float(ComboUpTR.W)) - float(20);
    switch(W.Align)
    {
        // End:0x104
        case 0:
    ...
    W.TextY = (W.WinHeight - YL) / float(2);
    ...
    W.Button.WinLeft = W.WinWidth - float(ComboUpTR.W);
    W.Button.WinTop = 0;
...
function Combo_Draw(UWindowComboControl W, Canvas C)
{
    ...
    R = ComboUpT;
    W.DrawStretchedTextureSegment(C, W.EditAreaDrawX + float(ButtonUpTL.W), 0, (W.WinWidth - float(ButtonUpTL.W)) - float(ButtonUpTR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x1A4
    if(W.bListVisible)
    {
        R = ComboDownTR;        
    ...
    W.DrawStretchedTextureSegment(C, W.EditAreaDrawX, W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = ComboUpB;
    W.DrawStretchedTextureSegment(C, W.EditAreaDrawX + float(ButtonUpBL.W), W.WinHeight - float(R.h), (W.WinWidth - float(ButtonUpBL.W)) - float(ButtonUpBR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    // End:0x3CC
    if(W.bListVisible)
    {
        R = ComboDownBR;        
...
{
    W.WinHeight += float(ComboDropBL.h);
    W.WinWidth = W.Owner.WinWidth;
    ListX = W.Owner.EditAreaDrawX;
    ListY = 21;
    W.VertSB.WinLeft = (W.WinWidth - float(SBPosIndicator.W)) - float(8);
    W.VertSB.WinTop = 1;
    W.VertSB.WinWidth = float(SBPosIndicator.W);
    W.VertSB.WinHeight = W.WinHeight - float(ComboDropBR.h);
    return;
}
...
function ComboList_DrawBackground(UWindowComboList W, Canvas C)
{
    ...
    t = W.GetLookAndFeelTexture();
    R = ComboDropL;
    W.DrawStretchedTextureSegment(C, 0, 0, float(R.W), W.WinHeight - float(ComboDropBL.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = ComboDropR;
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), 0, float(R.W), W.WinHeight - float(ComboDropBR.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = ComboDropBL;
    W.DrawStretchedTextureSegment(C, 0, W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = ComboDropB;
    W.DrawStretchedTextureSegment(C, float(ComboDropBL.W), W.WinHeight - float(R.h), (W.WinWidth - float(ComboDropBL.W)) - float(ComboDropBR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = ComboDropBR;
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    R = ComboFill;
    W.DrawStretchedTextureSegment(C, float(ComboDropL.W), 0, (W.WinWidth - float(ComboDropL.W)) - float(ComboDropR.W), W.WinHeight - float(ComboDropB.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, 1);
    C.Style = W.GetPlayerOwner().1;
    return;
}
...
function ComboList_DrawItem(UWindowComboList Combo, Canvas C, float X, float Y, float W, float h, string Text, bool bSelected)
{
    ...
    Combo.ClippingRegion.W = int(Combo.WinWidth - float(ComboDropL.W * 2));
    C.Font = C.TallFont;
...
function MessageBox_AutoSize(UWindowMessageBox W, Canvas C)
{
    ...
    W.SetSize(WinLen, (W.WinHeight - float(R.h)) + ClientBox.GetHeight(C));
    W.WinLeft = float(int((W.Root.WinWidth - W.WinWidth) / float(2)));
    W.WinTop = float(int((W.Root.WinHeight - W.WinHeight) / float(2)));
    return;
}
...
{
    return W.GetPlayerOwner().PlaySound(GameStartSound);
    return;
}

function Menu_DrawPulldownMenuBackground(UWindowPulldownMenu W, Canvas C)
{
    Bevel_DrawSimpleBevel(W, C, int(Pulldown_HBorder), int(Pulldown_VBorder), int(W.WinWidth - (float(2) * Pulldown_HBorder)), int(W.WinHeight - (float(2) * Pulldown_VBorder)), 1);
    return;
}
...
function Menu_DrawPulldownMenuItem(UWindowPulldownMenu m, UWindowPulldownMenuItem Item, Canvas C, float X, float Y, float W, float h, bool bSelected)
{
    ...
    {
        m.DrawClippedTexture(C, (X + W) - float(9), Y + float(3), class'MenuSubArrow');
    }
