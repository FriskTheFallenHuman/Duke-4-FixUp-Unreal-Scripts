...
function Created()
{
    super.Created();
    RowHeight = 15;
    AddColumn(RuleText, 0.6 * WinWidth);
    AddColumn(ValueText, 0.4 * WinWidth);
    return;
}
    ...
    if((float(Y) < (((RowHeight + WinHeight) - RowHeight) - float(TopMargin + BottomMargin))) && l == none)
    {
        ...
        if(float(Skipped) >= VertSB.pos)
        {
            switch(Column.ColumnNum)
            {
                // End:0x1E3
                case 0:
                    Column.ClipText(C, 6, float(Y + TopMargin), l.Rule);
                    // End:0x223
                    break;
                // End:0x220
                case 1:
                    Column.ClipText(C, 6, float(Y + TopMargin), l.Value);
                    // End:0x223
                    break;
                // End:0xFFFF
                default:
