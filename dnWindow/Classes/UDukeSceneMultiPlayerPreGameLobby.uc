...
function SetupButtons(Canvas C, out float YOffset)
{
    ...
    if(int(GetPlayerOwner().Level.NetMode) == int(NM_ListenServer))
    {
        ...
        if(! GetPlayerOwner().Level.Game.MatchStarted())
        {
            ...
            YOffset += (StartGameButton.WinHeight + float(ControlBuffer));            
        }
        else
        {
            ...
            YOffset += (RestartGameButton.WinHeight + float(ControlBuffer));
            ...
            YOffset += (NewGameButton.WinHeight + float(ControlBuffer));
        }        
    }
    else
    {
    ...
    YOffset += (DisconnectButton.WinHeight + float(ControlBuffer));
    return;
}
...
function Paint(Canvas C, float MouseX, float MouseY)
{
    ...
    ControlStart = int(float(75) * WinScaleY);
    ButtonLeft = int(float(75) * WinScaleY);
    ...
    xOffset = float(ButtonLeft) + (float(20) * WinScaleY);
    YOffset += YL;
    ...
    if(GRI.RoundTimeLimit > 0)
    {
        ...
        if(S == "")
        {
            S = RoundTimeLimit @ TimeString;            
        }
        else
        {
            S = ((S @ "/") @ RoundTimeLimit) @ TimeString;
        }
    }
    // End:0x453
    if(GRI.TimeLimit > 0)
    ...
    if(GRI.NumPlayers > 0)
    {
        S = string(GRI.NumPlayers) @ "PLAYERS";
    }
    // End:0x5A0
    if(GRI.NumSpectators > 0)
    {
        ...
        if(S == "")
        {
            S = string(GRI.NumSpectators) @ "Spectators";            
        }
        else
        {
            S = ((S @ "/") @ string(GRI.NumSpectators)) @ "Spectators";
        }
    }
    // End:0x658
    if((GRI.MinPlayers - GRI.NumPlayers) > 0)
    {
        // End:0x610
        if(S == "")
        {
            S = string(GRI.MinPlayers - GRI.NumPlayers) @ "Players needed";            
        }
        else
        {
            S = ((S @ "/") @ string(GRI.MinPlayers - GRI.NumPlayers)) @ "Players needed";
        }
    }
    // End:0x6E9
    if(S != "")
    ...
    PlayerList.WinLeft = float(ButtonLeft + ButtonWidth) + (float(25) * WinScaleY);
    PlayerList.WinWidth = (WinWidth - float(ButtonLeft)) - PlayerList.WinLeft;
    PlayerList.WinTop = float(ControlStart);
    PlayerList.WinHeight = 640 * WinScaleY;
    C.Font = OldFont;
    return;
}
...
function DrawPlayersList(Canvas C, float Padding)
{
    ...
    S = "TEST";
    TextSize(C, S, XL, YL, class'UWindowScene'.default.TTFontScale, class'UWindowScene'.default.TTFontScale);
    xOffset = float((ButtonLeft + ButtonWidth) + 25);
    YOffset = float(ControlStart);
    LookAndFeel.Bevel_DrawSimpleBevel(self, C, int(xOffset), int(YOffset), int((WinWidth - xOffset) - float(ButtonLeft)), int(float(17) * (YL + Padding)), 1);
    GRI = dnDeathmatchGameReplicationInfo(GetPlayerOwner().GameReplicationInfo);
    // End:0xDE
    if(GRI != none)
    {
    ...
    if(GRI.NumPlayers > 0)
    {
        ...
        if(i < string(GRI.PRIArray))
        {
            ...
            if(GRI.bTeamGame)
            {
                ...
                {
                    S = (Left(GRI.PRIArray[i].TeamName, 1) @ "-") @ S;
                    C.DrawColor = UDukeLookAndFeel(LookAndFeel).TeamColor[int(GRI.PRIArray[i].Team)];
                    DrawStretchedTexture(C, xOffset, YOffset, WinWidth - (xOffset * 2), YL, class'WhiteTexture');
                }
            }
            // End:0x25E
