...
{
    // End:0x1F
    if(! GetPlayerOwner().KGetRotationVector(bSaveGameMapFound, SaveDataStatus))
    {
        return false;
    }
    // End:0x41
    if(InStr(GetPlayerOwner().KPopState(), "DLC") != -1)
    {
        return false;
    }
    return true;
...
function UpdateProgressFlags()
{
    ...
    Localize("OnNavForward:bContineGameVisible:" @ string(bContinueGameVisible));
    Localize("OnNavForward:bChapterSelectVisible:" @ string(bChapterSelectVisible));
    return;
}
...
function OnNavForward()
{
    ...
    UpdateProgressFlags();
    // End:0x2D
    if(! bContinueGameVisible && ! bChapterSelectVisible)
    {
        NewGamePushed();        
    }
    else
...
function DukeSuperMessageBoxDone(int Result, Engine.Object.EConsole_Dialog id)
{
    ...
    if(GetGearboxEngineGlobals())
    {
        ...
        if(int(id) == int(13))
        {
            ...
            {
                Root.Console.PS3MsgDialog(ClassIsChildOf("DukeDialogBoxManager", "CorruptSaveNoOverwriteText", "dnwindow"));
            }
        }
    }
    return;
...
function ContinueGamePushed()
{
    ...
    {
        GetPlayerOwner().IsMenuActive(true);
    }
    return;
}
...
function Tick(float Delta)
{
    ...
    if((TravelFade == 1) && FadeAlpha == 0.01)
    {
        ...
        GetConfigFloat("Difficulty", "CurrentDifficulty", Difficulty, "Progress.ini");
        GetPlayerOwner().LoadGame(2, 0, Difficulty);
    }
    // End:0x113
    if((ObjectIsDestroyed() && UDukeRootWindow(Root).bSaveExists) && GetPlayerOwner().GetSprintSpeedThreshold())
    {
