...
function Created()
{
    ...
    RowHeight = 15;
    W = WinWidth - float(7);
    AddColumn(NameText, 0.64 * W);
    AddColumn(FragsText, 0.2 * W);
    AddColumn(PingText, 0.16 * W);
    return;
}
    ...
    if((float(Y) < (((RowHeight + WinHeight) - RowHeight) - float(TopMargin + BottomMargin))) && l == none)
    {
        ...
        if(float(Skipped) >= VertSB.pos)
        {
            switch(Column.ColumnNum)
            {
                ...
                // End:0x221
                case 1:
                    Column.ClipText(C, 6, float(Y + TopMargin), string(l.PlayerFrags));
                    // End:0x2A1
                    break;
                // End:0x260
                case 2:
                    Column.ClipText(C, 6, float(Y + TopMargin), string(l.PlayerPing));
                    // End:0x2A1
                    break;
                // End:0x29E
                case 3:
                    Column.ClipText(C, 6, float(Y + TopMargin), l.PlayerTeam);
                    // End:0x2A1
                    break;
                // End:0xFFFF
                default:
