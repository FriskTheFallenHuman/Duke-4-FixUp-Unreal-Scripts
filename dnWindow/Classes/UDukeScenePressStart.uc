...
function Paint(Canvas C, float X, float Y)
{
    ...
    PressStartY = 534 * WinScaleY;
    ESRBY = 594 * WinScaleY;
    CopyrightY = 620 * WinScaleY;
    ...
    if(GetPlayerOwner().GetSpeakerType())
    {
        ...
        PressStartTextPosX = CenterX - (XL / float(2));
        // End:0x129
        if(PressStartTextPosX < (WinWidth * 0.15))
        {
            PressStartTextPosX = WinWidth * 0.15;
        }
    ...
    FontScale = 0.75 * TTFontScale;
    C.DrawColor = GetPlayerOwner().NewColorBytes(139, 139, 139);
    TextSize(C, CopyrightText, XL, YL, FontScale, FontScale);
    ClipText(C, CenterX - (XL / float(2)), CopyrightY, CopyrightText,, FontScale, FontScale,, 2);
    return;
}
...
function KeyDown(int Key, float X, float Y)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // End:0x141
                if((! bWaitForSignIn && Key == int(P.204)) || Key == int(P.210))
                {
                    // End:0x141
                    if(GetPlayerOwner().SetSpeakerType(false))
                    {
                        bWaitForSignIn = true;
                    }
                }
...
function PressedStart(optional bool bSkipInputDelay)
{
    ...
    // End:0x36
    if(! bSkipInputDelay && iInputDelay > 0)
    {
        return;
    }
...
function Tick(float Delta)
{
    ...
    if(bGlowRising)
    {
        GlowColor = GlowColor + (GlowRate * Delta);
        // End:0x4D
        if(GlowColor > float(255))
        {
            bGlowRising = false;
            GlowColor = 255;
        }        
    ...
    else
    {
        GlowColor = GlowColor - (GlowRate * Delta);
        // End:0x8B
        if(GlowColor < default.GlowColor)
        {
            bGlowRising = true;
    ...
    else
    {
        ...
        {
            // End:0xFC
            if((ModalWindow != none) && GetPlayerOwner().SetSpeakerType(! bSignInShown))
            {
                // End:0xF4
                if(bSignInShown || GetPlayerOwner().WantsToSprint(false))
                {
                    PressedStart(true);
                }
                bSignInShown = true;
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // End:0x194
                    if((((ModalWindow != none) && GetPlayerOwner().SetSpeakerType(false)) && ! bReturned) && string(DukeConsole(Root.Console).DialogMgr.DialogStack) == 0)
                    {
                        PressedStart(true);
                    }
                }
...
function OnNavForward()
{
    ...
    bWaitForSignIn = false;
    bDontTick = false;
    GetPlayerOwner().Native_GetDisplayMode(true);
    // End:0x45
    if(ObjectIsDestroyed())
    {
