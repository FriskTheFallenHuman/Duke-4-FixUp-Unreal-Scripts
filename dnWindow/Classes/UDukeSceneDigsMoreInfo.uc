...
function OnNavForward()
{
    ...
    {
        // End:0xC6
        if(MyDigsPlayer.LookHitDigsItem.ChallengeTag != class'ChallengeInfo'.default.ChallengesArray[i].SearchTag)
        {
...
{
    super.Paint(C, X, Y);
    DisplayWindow.WinLeft = float(C.SizeX) * XPos;
    DisplayWindow.WinTop = float(C.SizeY) * YPos;
    DisplayWindow.WinWidth = float(C.SizeX) * WidthPct;
    DisplayWindow.WinHeight = float(C.SizeY) * HeightPct;
    return;
}
