...
function Created()
{
    ...
    if(i < string(Entries))
    {
        Entries[i].Date = ClassIsChildOf("UDukeSceneTimeline", ("Entries[" $ string(i)) $ "].Date", "dnWindow");
        Entries[i].Info = ClassIsChildOf("UDukeSceneTimeline", ("Entries[" $ string(i)) $ "].Info", "dnWindow");
        ListSlider.AddItem(" ");
        ++ i;
        // [Loop Continue]
        goto J0x0D;
...
function DrawMenuContent(Canvas C, float Alpha, Region ClipRegion)
{
    ...
    Top = float(ClipRegion.Y) + (0.15 * float(ClipRegion.h));
    // End:0x69
    if(ObjectDestroy())
    {
        ClipRegion.X += int(float(ClipRegion.W) * 0.05);
        ClipRegion.W *= 0.9;
    }
    C.DrawColor = OrangeColor;
    C.SetPause(float(ClipRegion.X), Top);
    C.SetClampMode(WhiteMaterial, float(ClipRegion.W), 2, 0, 0, 1, 1, 0, 0, 0, false, Alpha);
    MinValue = ((float(Entries[0].Year) * 366) + (float(Entries[0].Month) * 31)) + float(Entries[0].Day);
    ValueRange = (((float(Entries[string(Entries) - 1].Year) * 366) + (float(Entries[string(Entries) - 1].Month) * 31)) + float(Entries[string(Entries) - 1].Day)) - MinValue;
    i = string(Entries) - 1;
    J0x192:
    ...
    if(i >= 0)
    {
        Value = ((((float(Entries[i].Year) * 366) + (float(Entries[i].Month) * 31)) + float(Entries[i].Day)) - MinValue) / ValueRange;
        C.SetPause(float(Ceil(float(ClipRegion.X) + (Value * float(ClipRegion.W)))), Top - 3);
        C.SetClampMode(WhiteMaterial, 1, 8, 0, 0, 1, 1, 0, 0, 0, false, Alpha);
        // End:0x3BD
        if(i == Index)
        {
            class'UWindowScene'.static.DrawSelectionIcon(C, (float(ClipRegion.X) + (Value * float(ClipRegion.W))) - (((1.5 * float(default.SpinningNuke.DrawStatic())) / float(2)) * C.FixedScale), Top - (0.1 * float(ClipRegion.h)), 1.5 * C.FixedScale, Alpha);
            DrawStretchedTextureSegment(C, float(ClipRegion.X) + ((Value - 0.005) * float(ClipRegion.W)), Top - (0.05 * float(ClipRegion.h)), 0.01 * float(ClipRegion.W), 0.04 * float(ClipRegion.h), 417, 136, 11, 19, class'Backdrop', Alpha, true, true);
            C.DrawColor = OrangeColor;
        }
        // End:0x45A
        if(i == 0)
        {
            C.SetPause(float(Ceil(float(ClipRegion.X) + ((Value - 0.015) * float(ClipRegion.W)))), Top + (0.05 * float(ClipRegion.h)));
            C.GetRenderBoundingBox(string(Entries[i].Year), false, false, false, TTFontScale * 0.8, TTFontScale * 0.8, Alpha, 2);
        }
        // End:0x4FF
    ...
    Left = float(ClipRegion.X) + (0.2 * float(ClipRegion.W));
    Top = (float(ClipRegion.Y) + (0.2 * float(ClipRegion.h))) + 2;
    Width = 0.6 * float(ClipRegion.W);
    Height = ((0.7 * float(ClipRegion.h)) - 4) - YL;
    // End:0x5E4
    if((Index >= 0) && Index < string(Entries))
    {
    ...
    Left = float(ClipRegion.X) + (0.1 * float(ClipRegion.W));
    Top = (float(ClipRegion.Y) + (0.2 * float(ClipRegion.h))) + (4 * C.FixedScale);
    C.Font = C.TallFont;
    C.DrawColor = GreyColor;
    C.SetClip(Date, XL, YL, TTFontScale, TTFontScale);
    C.SetPause(float(ClipRegion.X) + (0.5 * (float(ClipRegion.W) - XL)), Top);
    C.GetRenderBoundingBox(Date, false, false, false, TTFontScale, TTFontScale, Alpha, 2);
    Top += YL;
    Width = 0.8 * float(ClipRegion.W);
    Height = ((0.7 * float(ClipRegion.h)) - (4 * C.FixedScale)) - YL;
    C.bCenter = true;
    C.RegisterActorClass(Left + Width, Top + Height);
    C.SetPause(Left, Top);
    C.Style = 0;
    YL = C.GetRenderBoundingBox(Text, false, true, true, TTFontScale, TTFontScale, Alpha, 2);
    Top += (0.5 * (Height - YL));
    C.SetPause(Left, Top);
    C.Style = 1;
    C.GetRenderBoundingBox(Text, false, true, true, TTFontScale, TTFontScale, Alpha, 2);
    C.RegisterActorClass(float(C.SizeX), float(C.SizeY));
    C.bCenter = false;
    return;
}
