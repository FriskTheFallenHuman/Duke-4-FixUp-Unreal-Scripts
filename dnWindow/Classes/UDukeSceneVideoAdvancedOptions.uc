...
function Created()
{
    ...
    if(! ObjectDestroy())
    {
        FOVSetting = UDukeListSlider(CreateWindow(class'UDukeListSlider'));
        i = FOVMin;
        J0x3A5:
    ...
    if(! ObjectDestroy())
    {
        PostEffectsCheck.NavDown = FOVSetting;
...
function SetDefaults()
{
    ...
    ShadowDetail.SetSelectedIndex(int(GetPlayerOwner().ConsoleCommand("get ini:Engine.Engine.ViewportManager ShadowMapDetail")) - 1);
    // End:0x128
    if(! bool(GetPlayerOwner().ConsoleCommand("get ini:Engine.Engine.ViewportManager AllowStencilShadows")))
    {
        ShadowsEnabled.SetSelectedIndex(0);        
    }
    else
    {
        // End:0x190
        ...
        {
            ShadowsEnabled.SetSelectedIndex(1);            
        }
        else
        {
    ...
    if(! ObjectDestroy())
    {
        ...
        FOVvalue = Clamp(FOVvalue, FOVMin, FOVMax);
        // End:0x3F7
...
{
    GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager TextureDetail " $ TextureDetail.SelectedItem.Value);
    return;
}

function ShadowDetailChanged()
{
    GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager ShadowMapDetail " $ string(int(ShadowDetail.SelectedItem.Value)));
    return;
}
...
{
    GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager PerPixelMotionBlur " $ string(MotionBlurCheck.bChecked));
    return;
}
...
function AntialiasingChanged()
{
    switch(Antialiasing.GetSelectedIndex())
    {
        // End:0xAD
        case 2:            
            GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager AllowFXAA " $ string(true));            
            GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager AllowFSAA " $ string(true));
            // End:0x1DB
            break;
        // End:0x146
        case 1:            
            GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager AllowFXAA " $ string(false));            
            GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager AllowFSAA " $ string(true));
            // End:0x1DB
            break;
        // End:0xFFFF
        default:            
            GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager AllowFXAA " $ string(false));            
            GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager AllowFSAA " $ string(false));
            break;
    }
    return;
}
...
{
    GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager AllowFilmGrain " $ string(FilmGrainCheck.bChecked));
    return;
}

function PostEffectsChanged()
{
    GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager AllowPostEffects " $ string(PostEffectsCheck.bChecked));
    return;
}

function FOVChanged()
{
    SetConfigFloat("Engine.PlayerPawn", "DefaultFOV", int(FOVSetting.SelectedItem.Value), "User.ini");
    // End:0x83
    if(bShowFOVWarning)
    {
...
function Paint(Canvas C, float X, float Y)
{
    ...
    TextureDetail.ArrowLeft = float(ButtonWidth) - (float(250) * WinScaleY);
    ShadowDetail.ArrowLeft = float(ButtonWidth) - (float(250) * WinScaleY);
    ShadowsEnabled.ArrowLeft = float(ButtonWidth) - (float(250) * WinScaleY);
    Antialiasing.ArrowLeft = float(ButtonWidth) - (float(250) * WinScaleY);
    ...
    if(! ObjectDestroy())
    {
        FOVSetting.ArrowLeft = float(ButtonWidth) - (float(250) * WinScaleY);
...
function NavigateBack()
{
    ...
    GetPlayerOwner().DefaultFOVShrunk = float(FOVvalue - 5);
    SaveConfig("User.ini");
    super.NavigateBack();
    return;
}
...
function defaultFOVSetting()
{
    ...
    FOVvalue = Clamp(FOVvalue, FOVMin, FOVMax);
    SetConfigFloat("Engine.PlayerPawn", "DefaultFOV", FOVvalue, "User.ini");
