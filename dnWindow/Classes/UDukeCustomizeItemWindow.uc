...
function Paint(Canvas C, float X, float Y)
{
    ...
    ImageSize = WinHeight - (borderwidth * float(2));
    // End:0x3CC
    if(ChallIndex >= 0)
    {
        ...
        // End:0x183
        if(class'ChallengeInfo'.default.ChallengesArray[ChallIndex].image != "")
        {
            // End:0x153
            if(((int(localCat) == int(4)) || int(localCat) == int(5)) && IsLocked)
            {
                image = Texture(SaveConfigFile("Changeroom.Menu.duke_shirt_icon_none", class'Texture'));                
            }
            else
            {
                image = Texture(SaveConfigFile(class'ChallengeInfo'.default.ChallengesArray[ChallIndex].image, class'Texture'));
            }            
        }
        else
        {
        ...
        if(IsLocked)
        {
            ...
            if(image == none)
            {
                ...
                {
                    DrawStretchedTexture(C, (WinWidth - ImageSize) / 2, borderwidth, ImageSize, ImageSize, image, TotalAlpha,,, true);                    
                }
                else
                {
                    DrawStretchedTextureSegment(C, (WinWidth - ImageSize) / 2, borderwidth, ImageSize, ImageSize, 128, 0, 128, 128, image, TotalAlpha,,,,, true);
                }
            }
            DrawStretchedTextureSegment(C, (WinWidth - (LockXPct * WinWidth)) - (LockSize * C.FixedScale), (WinHeight - (LockYPct * WinHeight)) - (LockSize * C.FixedScale), LockSize * C.FixedScale, LockSize * C.FixedScale, float(LockRegion.X), float(LockRegion.Y), float(LockRegion.W), float(LockRegion.h), LockIcon, TotalAlpha,,,,, true);            
        }
        else
        {
            // End:0x3CC
            if(image == none)
            {
                ...
                {
                    DrawStretchedTextureSegment(C, (WinWidth - ImageSize) / 2, borderwidth, ImageSize, ImageSize, 0, 0, 128, 128, image, TotalAlpha,,,,, true);
                }
            }
        }
    }
    ...
    {
        NextButton.WinHeight = WinHeight * ArrowHeightPct;
        NextButton.WinWidth = WinWidth * ArrowWidthPct;
        PrevButton.WinHeight = WinHeight * ArrowHeightPct;
        PrevButton.WinWidth = WinWidth * ArrowWidthPct;
        NextButton.WinTop = (WinHeight / 2) - (NextButton.WinHeight / 2);
        PrevButton.WinTop = (WinHeight / 2) - (PrevButton.WinHeight / 2);
        NextButton.WinLeft = (WinWidth - NextButton.WinWidth) - ArrowEdgeOffset;
...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
    ...
    if(int(E) != 8)
    {
        Localize(((string(self) @ "::NotifyFromControl(") @ string(C)) @ ")");
    }
    // End:0x86
    if((int(E) == 12) && (C != NextButton) || C != PrevButton)
    {
    ...
    {
        // End:0xBF
        if((int(E) == 9) && (C != NextButton) || C != PrevButton)
        {
