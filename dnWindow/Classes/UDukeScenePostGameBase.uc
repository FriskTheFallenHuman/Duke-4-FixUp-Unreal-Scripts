...
function Paint(Canvas C, float X, float Y)
{
    ...
    {
        ScoreSpacing = (float(TimerBkg.DrawStatic()) * 0.25) * WinScaleX;
        Y = float(C.SizeY) * 0.075;
        iTeam = 1;
        ...
        C.SetClip(string(GRI.Teams[iTeam].Score), XL, YL, TeamScoreSize, TeamScoreSize);
        C.SetPause((((C.ClipX - ScoreSpacing) / 2) - XL) - (ScoreOffset * WinScaleX), Y * WinScaleY);
        C.GetRenderBoundingBox(string(GRI.Teams[iTeam].Score),,,, TeamScoreSize, TeamScoreSize,, 1);
        iTeam = 0;
        C.DrawColor = TeamColor[iTeam];
        // End:0x2B5
        ...
        C.SetClip(string(GRI.Teams[iTeam].Score), XL, YL, TeamScoreSize, TeamScoreSize);
        C.SetPause(((C.ClipX + ScoreSpacing) / 2) + (ScoreOffset * WinScaleX), Y * WinScaleY);
        C.GetRenderBoundingBox(string(GRI.Teams[iTeam].Score),,,, TeamScoreSize, TeamScoreSize,, 1);        
    }
    else
    {
    ...
    ClipText(C, (WinWidth - XL) / 2, (FillTopEdge - YL) - float(4), WinString,,,, 2);
    return;
}
