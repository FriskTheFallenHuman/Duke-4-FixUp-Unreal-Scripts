...
function BeforePaint(Canvas C, float X, float Y)
{
    ...
    ColumnHeadingHeight += 2;
    ColumnHeadingHeight *= HeaderPaddingMultiplier;
    return;
}
...
function Paint(Canvas C, float X, float Y)
{
    ...
    DrawStretchedTextureSegment(C, WinWidth * 0.475, YL, float(SortingArrowRegion.W) * ScaleX, float(SortingArrowRegion.h) * ScaleY, float(SortingArrowRegion.X), float(SortingArrowRegion.Y), float(SortingArrowRegion.W), float(SortingArrowRegion.h), SortingArrowTexture, 1,,,,, true);
    // End:0x1EA
    if(bDrawDivider)
    {
        DrawStretchedTexture(C, WinWidth * 0.95, WinHeight * 0.1, 4, WinHeight * 0.8, VGridLine, 1,,, true);
    }
    return;
}
