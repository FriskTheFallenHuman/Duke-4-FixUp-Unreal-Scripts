...
{
    super.Created();
    ExpGainStr = ClassIsChildOf("MetagameHUD", "ExpGainStr", "dngame");
...
{
    borderwidth = 5;
    FillLeft = FillLeftPct * float(C.SizeX);
    FillTop = FillTopPct * float(C.SizeY);
    FillWidth = FillWidthPct * float(C.SizeX);
    FillHeight = FillHeightPct * float(C.SizeY);
...
function DrawBackground(Canvas C)
{
    ...
    C.Style = 1;
    DrawStretchedTexture(C, FillLeft - borderwidth, FillTop - borderwidth, FillWidth + borderwidth, borderwidth, class'WhiteTexture');
    DrawStretchedTexture(C, FillLeft - borderwidth, FillTop, borderwidth, FillHeight, class'WhiteTexture');
    DrawStretchedTexture(C, (FillLeft + FillWidth) - borderwidth, FillTop, borderwidth, FillHeight, class'WhiteTexture');
    DrawStretchedTexture(C, FillLeft - borderwidth, FillTop + FillHeight, FillWidth + borderwidth, borderwidth, class'WhiteTexture');
    return;
}
...
function SetTitle()
{
    ...
    if(DukeMultiPlayer(GetPlayerOwner()).PlayerProgress.HasLeveledUp())
    {
        ...
        SubTitleStr = FormatTimeString(LevelUpSubTitle, FormatArr);        
    }
    else
    {
        // End:0x97
...
function Paint(Canvas C, float X, float Y)
{
    ...
    super.Paint(C, X, Y);
    CDFontScale = TTFontScale * CDTextScale;
    TitleFontScale = TTFontScale * TitleTextScale;
    SubtitleFontScale = TTFontScale * SubTitleTextScale;
    OtherFontScale = TTFontScale * ChallTextScale;
    ChallItemFontScale = TTFontScale * ChallItemTextScale;
    ...
    ClipText(C, FillHalf - (TitleXL / 2), YOffset, TitleStr,, TitleFontScale, TitleFontScale, 1, 2);
    YOffset += TitleYL;
    TextSize(C, SubTitleStr, SubtitleXL, SubtitleYL, SubtitleFontScale, SubtitleFontScale);
    ClipText(C, FillHalf - (SubtitleXL / 2), YOffset, SubTitleStr,, SubtitleFontScale, SubtitleFontScale, 1, 2);
    YOffset += SubtitleYL;
    YOffset += BarSpacing;
    C.Style = 8;
    DrawStretchedTexture(C, FillLeft + (FillWidth * LineXIndentPct), YOffset, (FillWidth * (1 - LineXIndentPct)) - FillLeft, 8, HorizBarTexture, 1);
    YOffset += (BarSpacing * 2);
    C.Style = 1;
    TextSize(C, ChallengesCompleted, XL, YL, OtherFontScale, OtherFontScale);
    ClipText(C, FillLeft + (FillWidth * ChallTitleXPct), YOffset, ChallengesCompleted,, OtherFontScale, OtherFontScale, 1, 2);
    TextSize(C, ChallengeXP, XL, YL, OtherFontScale, OtherFontScale);
    ClipText(C, FillLeft + (FillWidth * ChallXPTitleXPct), YOffset, ChallengeXP,, OtherFontScale, OtherFontScale, 1, 2);
    TextSize(C, ItemsUnlocked, XL, YL, OtherFontScale, OtherFontScale);
    ClipText(C, FillLeft + (FillWidth * RewardTitleXPct), YOffset, ItemsUnlocked,, OtherFontScale, OtherFontScale, 1, 2);
    YOffset += YL;
    ...
    // End:0x53E [Loop If]
    if(i < Min(NumEntries, BaseEntry + 6))
    {
        TextSize(C, NewChallenges[i].LocdName, XL, YL, ChallItemFontScale, ChallItemFontScale);
        ClipText(C, FillLeft + (FillWidth * ChallItemXPct), YOffset, NewChallenges[i].LocdName,, ChallItemFontScale, ChallItemFontScale, 1, 2);
        XP[0] = string(NewChallenges[i].XPValue);
        ClipText(C, FillLeft + (FillWidth * ChallXPXPct), YOffset, FormatTimeString(ExpGainStr, XP),, ChallItemFontScale, ChallItemFontScale, 1, 2);
        // End:0x4D5
        if(NewChallenges[i].Reward != "")
        {
            ClipText(C, FillLeft + (FillWidth * RewardItemXPct), YOffset, NewChallenges[i].Reward,, ChallItemFontScale, ChallItemFontScale, 1, 2);            
        }
        else
        {
            ClipText(C, FillLeft + (FillWidth * RewardItemXPct), YOffset, NoRewardText,, ChallItemFontScale, ChallItemFontScale, 1, 2);
        }
        YOffset += YL;
        ++ ItemCount;
        // End:0x534
    ...
    ProgressBar.WinWidth = FillLeft + (FillWidth * ProgressBarWidthPct);
    ProgressBar.WinLeft = FillHalf - (ProgressBar.WinWidth / 2);
    ProgressBar.WinHeight = FillHeight * ProgressBarHeightPct;
    ...
    YOffset += ProgressBar.WinHeight;
    TextSize(C, CombatXP, XL, YL, OtherFontScale, OtherFontScale);
    ClipText(C, (FillHalf - (FillWidth * XPCenterXOffset)) - XL, YOffset, CombatXP,, OtherFontScale, OtherFontScale, 1, 2);
    ClipText(C, FillHalf + (FillWidth * XPCenterXOffset), YOffset, CombatXPStr,, OtherFontScale, OtherFontScale, 1, 2);
    YOffset += YL;
    TextSize(C, ChallengeXP, XL, YL, OtherFontScale, OtherFontScale);
    ClipText(C, (FillHalf - (FillWidth * XPCenterXOffset)) - XL, YOffset, ChallengeXP,, OtherFontScale, OtherFontScale, 1, 2);
    ClipText(C, FillHalf + (FillWidth * XPCenterXOffset), YOffset, ChallengeXPStr,, OtherFontScale, OtherFontScale, 1, 2);
    YOffset += YL;
    TextSize(C, MatchXP, XL, YL, OtherFontScale, OtherFontScale);
    ClipText(C, (FillHalf - (FillWidth * XPCenterXOffset)) - XL, YOffset, MatchXP,, OtherFontScale, OtherFontScale, 1, 2);
    C.DrawColor = class'UWindowScene'.default.OrangeColor;
    ClipText(C, FillHalf + (FillWidth * XPCenterXOffset), YOffset, MatchXPStr,, OtherFontScale, OtherFontScale, 1, 2);
    C.DrawColor = class'UWindowScene'.default.GreyColor;
    // End:0xAA0
    if(MPGameReplicationInfo(GetPlayerOwner().GameReplicationInfo).EORCountTime > 0)
    {
        C.DrawColor = WhiteColor;
        S = CountDownStr $ GenerateColorFromChar(int(float(MPGameReplicationInfo(GetPlayerOwner().GameReplicationInfo).EORCountTime) + 0.999));
        TextSize(C, S, CDXL, CDYL, CDFontScale, CDFontScale);
        ClipText(C, (float(C.SizeX) * CDXPos) - CDXL, (float(C.SizeY) * CDYPos) - CDYL, S,, CDFontScale, CDFontScale, 1, 2);
    }
    return;
}
...
function OnNavForward()
{
    ...
    ChallengeXP = 0;
    XPFormater = ClassIsChildOf("UDukeChallengeEntry", "XPFormater", "dnwindow");
    super.OnNavForward();
    ...
    if(i < NumEntries)
    {
        ...
        {
            NewChallenges[i].LocdName = ClassIsChildOf("ChallengeNames", InfoStruct.HelpText, "dnWindow");
            NewChallenges[i].Reward = ClassIsChildOf("ChallengeNames", InfoStruct.Name, "dnWindow");            
        }
        else
        {
            NewChallenges[i].LocdName = ClassIsChildOf("ChallengeNames", InfoStruct.Name, "dnWindow");
            NewChallenges[i].Reward = ClassIsChildOf("ChallengeNames", InfoStruct.HelpText, "dnWindow");
        }
    ...
    FormatArr[0] = string(MatchXP - ChallengeXP);
    CombatXPStr = FormatTimeString(XPFormater, FormatArr);
    FormatArr[0] = string(ChallengeXP);
    ChallengeXPStr = FormatTimeString(XPFormater, FormatArr);
    FormatArr[0] = string(MatchXP);
    MatchXPStr = FormatTimeString(XPFormater, FormatArr);
    curLvlStr = ClassIsChildOf("MetagameHUD", "LevelLabel", "dngame");
    nextLvlStr = curLvlStr;
    FormatArr[0] = string(DukeMultiPlayer(GetPlayerOwner()).PlayerProgress.GetLevel());
    curLvlStr = FormatTimeString(curLvlStr, FormatArr);
    FormatArr[0] = string(DukeMultiPlayer(GetPlayerOwner()).PlayerProgress.GetLevel() + 1);
    nextLvlStr = FormatTimeString(nextLvlStr, FormatArr);
    return;
}
