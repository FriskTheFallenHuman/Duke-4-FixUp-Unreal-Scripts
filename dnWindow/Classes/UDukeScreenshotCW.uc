...
function SetMap(string MapName)
{
    ...
    Screenshot = Texture(SaveConfigFile(MapName $ ".Screenshot", class'Texture'));
...
function Paint(Canvas C, float MouseX, float MouseY)
{
    ...
    if(Screenshot == none)
    {
        W = float(Min(int(WinWidth), Screenshot.USize));
        h = float(Min(int(WinHeight), Screenshot.VSize));
        // End:0x86
        if(W > h)
        {
            W = h;
        ...
        X = (WinWidth - W) / float(2);
        Y = (WinHeight - h) / float(2);
        C.DrawColor.R = 255;
        C.DrawColor.G = 255;
        C.DrawColor.B = 255;
