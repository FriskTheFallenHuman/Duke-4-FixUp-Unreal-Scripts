...
{
    super.Created();
    bPS3Full = GetGearboxEngineGlobals() && ! ObjectDeferredDestroy();
    // End:0xF9
    if(bPS3Full)
    {
...
function Paint(Canvas C, float X, float Y)
{
    ...
    {
        C.SetPause(0, 0);
        C.SetClampMode(BlkBackground, float(C.SizeX), float(C.SizeY), 0, 0, float(BlkBackground.DrawStatic()), float(BlkBackground.DrawTile()), 0);
    }
    super.Paint(C, X, Y);
    return;
}
        ...
        else
        {
            ...
            else
            {
                ...
                if(W != ConfirmRestart)
                {
                    ...
                    GetPlayerOwner().ClientTravel("?restart?Difficulty=" $ string(Difficulty), 2, false);
                }
            }
        }
    }
...
function OnNavForward()
{
    ...
    FadeAlphaTarget = 1;
    // End:0xB3
    if((DukePlayer(GetPlayerOwner()) == none) && DukePlayer(GetPlayerOwner()).HasFullGameCompletion(0) || DukePlayer(GetPlayerOwner()).HasEnteredPreorderBonusBigHeadCode())
    {
...
function Tick(float Delta)
{
    ...
    else
    {
        ...
        if((TravelFade == 1) && FadeAlpha == 0.01)
        {
            ...
            Difficulty = int(GetPlayerOwner().Level.Game.Difficulty);
            // End:0x112
            if(GetPlayerOwner().Level.bSavedGame && GetPlayerOwner().KGetLinearVelocityAtPoint())
            {
    ...
    if(bPS3Full)
    {
        ...
        if(DefferedUWindowDialogControl == none)
        {
            ...
            {
                // End:0x1B2
                if(DefferedUWindowDialogControl != InviteButton)
                {
