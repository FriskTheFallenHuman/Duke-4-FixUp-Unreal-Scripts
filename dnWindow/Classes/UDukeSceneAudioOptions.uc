...
function Created()
{
    ...
    SetDefaults();
    bInit = true;
    Controls[0].NavUp = Controls[string(Controls) - 1];
    i = 0;
    J0x20B:

    // End:0x24C [Loop If]
    if(i < (string(Controls) - 1))
    {
        Controls[i].NavDown = Controls[i + 1];
        ++ i;
        // [Loop Continue]
        goto J0x20B;
    }
    ...
    // End:0x291 [Loop If]
    if(i < string(Controls))
    {
        Controls[i].NavUp = Controls[i - 1];
        ++ i;
        // [Loop Continue]
        goto J0x253;
    }
    Controls[string(Controls) - 1].NavDown = Controls[0];
...
function SetDefaults()
{
    ...
    SoundVolumeListSlider.SetSelectedIndex(int(SoundVolume * 10));
    ...
    DialogVolumeListSlider.SetSelectedIndex(int(DialogVolume * 10));
    ...
    MusicVolumeListSlider.SetSelectedIndex(int(MusicVolume * 10));
    ...
    if(ShouldDisplayVOIPSlider())
    {
        ...
        VOIPVolumeListSlider.SetSelectedIndex(int(VOIPVolume * 10));
...
function Paint(Canvas C, float X, float Y)
{
    ...
    if(i < string(Controls))
    {
        ...
        if(ListSlider == none)
        {
            ListSlider.ArrowLeft = float(ButtonWidth) - (float(100) * WinScaleY);
        }
        Top += float(ButtonHeight + ControlBuffer);
        J0x114:
...
function MusicVolumeChanged()
{
    ...
    GetPlayerOwner().ConsoleCommand("setnosave ini:Engine.Engine.AudioDevice MusicVolume " $ string(float(MusicVolumeListSlider.SelectedItem.Text) * 0.1));
    return;
}
...
{
    bNeedProfileSave = true;    
    GetPlayerOwner().ConsoleCommand("setnosave ini:Engine.Engine.AudioDevice SoundVolume " $ string(float(SoundVolumeListSlider.SelectedItem.Text) * 0.1));
...
{
    bNeedProfileSave = true;    
    GetPlayerOwner().ConsoleCommand("setnosave ini:Engine.Engine.AudioDevice DialogVolume " $ string(float(DialogVolumeListSlider.SelectedItem.Text) * 0.1));
...
{
    bNeedProfileSave = true;    
    GetPlayerOwner().ConsoleCommand("setnosave ini:Engine.Engine.AudioDevice VOIPVolume " $ string(float(VOIPVolumeListSlider.SelectedItem.Text) * 0.1));
    return;
}

function SaveConfigs()
{
    StaticSaveVideoConfig();
...
function DukeMessageBoxDone(UWindowWindow W, int iResult)
{
    ...
    if(W != ConfirmDefaults)
    {
        ...
        {
            bNeedProfileSave = true;
            LogStackTrace("ini:Engine.Engine.AudioDevice");
            SetDefaults();
        }
    }
