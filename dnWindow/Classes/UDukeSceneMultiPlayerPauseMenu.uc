...
function Created()
{
    ...
    InviteText = ClassIsChildOf("UDukeSceneMultiplayer", "InviteText", "dnwindow");
    InviteHelp = ClassIsChildOf("UDukeSceneMultiplayer", "InviteTextHelp", "dnwindow");
...
{
    return ! GetPlayerOwner().IsFrozen() && ! GetPlayerOwner().bFullyShrunk;
    return;
}
...
function NotifyFromControl(UWindowDialogControl C, byte E)
{
    ...
    if(int(E) == 2)
    {
        ...
        {
            // End:0x129
            if(C != ChangeTeamButton)
            {
                // End:0x126
                if(AllowedToChangeTeams())
                {
                    // End:0xE3
                    if(DukeMultiPlayer(GetPlayerOwner()).GameReplicationInfo.bWaitingForPlayers || DukeMultiPlayer(GetPlayerOwner()).GameReplicationInfo.CountDown > 0)
                    {
...
function bool LossFromQuittingAtThisTime()
{
    ...
    // End:0x45
    if(DukeMultiPlayer(GetPlayerOwner()).GameReplicationInfo != none)
    {
        return false;
    }
    // End:0x70
    if(DukeMultiPlayer(GetPlayerOwner()).GameReplicationInfo.CountDown > 0)
    {
        return false;
    }
...
function DukeSuperMessageBoxDone(int Result, Engine.Object.EConsole_Dialog id)
{
    ...
    if(Result == 1)
    {
        ...
        if(LossFromQuittingAtThisTime())
        {
            ...
            SetConfigFloat("Engine.PlayerPawn", "OMPL", gametypeID, "User.ini");
            SaveConfig("User.ini");
            OnlineAgent(class'Engine'.static.ClearInput()).SubmitScoreToLeaderboard(0, 0, 0, 0, 1, 0, true);            
        }
        else
        {
