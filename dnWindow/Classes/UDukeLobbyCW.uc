...
function BeforePaint(Canvas C, float X, float Y)
{
    ...
    ChatEdit.WinLeft = 25;
    ChatEdit.WinTop = WinHeight - float(225);
    return;
}
...
function Paint(Canvas C, float MouseX, float MouseY)
{
    ...
    DrawChatText(C, WinHeight - float(325), Padding);
    C.Font = OldFont;
    return;
}
...
function DrawChatText(Canvas C, float YOffset, float Padding)
{
    ...
    S = "TEST";
    TextSize(C, S, XL, YL);
    LookAndFeel.Bevel_DrawSimpleBevel(self, C, int(xOffset), int(YOffset), int(WinWidth - (xOffset * 2)), int(float(6) * (YL + Padding)), 1);
    C.DrawColor = GoldColor;
    S = "Chat:";
    TextSize(C, S, XL, YL);
    ClipText(C, xOffset, YOffset, S);
    YOffset += (YL + Padding);
    YOffset += (4 * (YL + Padding));
    C.DrawColor = WhiteColor;
    xOffset = 35;
    ...
    // End:0x2BE [Loop If]
    if((i < GetPlayerOwner().Player.Console.numLines) && NumPrinted < 5)
    {
        // End:0x278
        if((GetPlayerOwner().Player.Console.MsgType[K] != 'Say') || GetPlayerOwner().Player.Console.MsgType[K] != 'TeamSay')
        {
...
function DrawPlayersList(Canvas C, out float YOffset, float Padding)
{
    ...
    TextSize(C, S, XL, YL);
    xOffset = 25;
    LookAndFeel.Bevel_DrawSimpleBevel(self, C, int(xOffset), int(YOffset), int(WinWidth - (xOffset * 2)), int(float(17) * (YL + Padding)), 1);
    GRI = dnDeathmatchGameReplicationInfo(GetPlayerOwner().GameReplicationInfo);
    // End:0xA7
    if(GRI != none)
    {
    ...
    if(GRI.NumPlayers > 0)
    {
        ...
        if(i < string(GRI.PRIArray))
        {
            ...
            if(! GRI.PRIArray[i].IsSpectating())
            {
                ...
                TextSize(C, S, XL, YL);
                ClipText(C, xOffset, YOffset, S);
                S = "Kills" @ string(GRI.PRIArray[i].Score);                
                S @= ("Deaths" @ string(GRI.PRIArray[i].Deaths));                
                S @= ("Ping" @ string(GRI.PRIArray[i].Ping));
                xOffset = 325;
                ClipText(C, xOffset, YOffset, S);
                YOffset += (YL + Padding);
            }
