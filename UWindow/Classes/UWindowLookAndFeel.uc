...
function DrawBevelByTilingHor(UWindowWindow W, Canvas C, float DestX, float DestY, float DestWidth, float DestHeight, Region R, Texture t, int iBevelSizeLeft, int iBevelSizeRight)
{
    ...
    W.DrawStretchedTextureSegment(C, (DestX + DestWidth) - float(iBevelSizeRight), DestY, float(iBevelSizeRight), DestHeight, float((R.X + R.W) - iBevelSizeRight), float(R.Y), float(iBevelSizeRight), float(R.h), t);
    regRemainder.X = R.X + iBevelSizeLeft;
    regRemainder.Y = R.Y;
    regRemainder.W = R.W - (iBevelSizeLeft + iBevelSizeRight);
    regRemainder.h = R.h;
    W.DrawHorizTiledPieces(C, DestX + float(iBevelSizeLeft), DestY, DestWidth - float(iBevelSizeLeft + iBevelSizeRight), DestHeight, regRemainder, t);
    return;
}
...
function DrawBevelByTilingVert(UWindowWindow W, Canvas C, float DestX, float DestY, float DestWidth, float DestHeight, Region R, Texture t, int iBevelSizeTop, int iBevelSizeBottom)
{
    ...
    W.DrawStretchedTextureSegment(C, DestX, (DestY + DestHeight) - float(iBevelSizeBottom), DestWidth, float(iBevelSizeBottom), float(R.X), float((R.Y + R.h) - iBevelSizeBottom), float(R.W), float(iBevelSizeBottom), t);
    regRemainder.X = R.X;
    regRemainder.Y = R.Y + iBevelSizeTop;
    regRemainder.W = R.W;
    regRemainder.h = R.h - (iBevelSizeTop + iBevelSizeBottom);
    W.DrawVertTiledPieces(C, DestX, DestY + float(iBevelSizeTop), DestWidth, DestHeight - float(iBevelSizeTop + iBevelSizeBottom), regRemainder, t);
    return;
}
...
function Checkbox_SetupSizes(UWindowCheckbox W, Canvas C)
{
    ...
    W.WinHeight = float(Max(int(tH + float(1)), 32));
    switch(W.Align)
    {
        // End:0xA0
        case 0:
            W.ImageX = W.WinWidth - float(32);
            W.TextX = 0;
            // End:0x142
            break;
        // End:0xE3
        ...
        // End:0x13F
        case 2:
            W.ImageX = (W.WinWidth - float(32)) / float(2);
    ...
    W.ImageY = (W.WinHeight - float(32)) / float(2);
...
function Combo_SetupSizes(UWindowComboControl W, Canvas C)
{
    ...
    C.Font = W.Root.GetFont(W.Font, C);
    W.TextSize(C, W.Text, tW, tH);
    W.WinHeight = float((ComboBtnUp.h + MiscBevelT[2].h) + MiscBevelB[2].h);
    switch(W.Align)
    {
        // End:0x104
        case 0:
