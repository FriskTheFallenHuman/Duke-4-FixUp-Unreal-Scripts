...
{
    return Root.Console.Viewport.Actor.GetStaticMeshInstance();
    return;
}
...
function RMouseUp(float X, float Y)
{
    ...
    {
        // End:0x83
        if(((! bIgnoreRDoubleClick && Abs(X - RClickX) <= float(1)) && Abs(Y - RClickY) <= float(1)) && GetLevel().TimeSeconds < (RClickTime + 0.6))
        {
            RDoubleClick(X, Y);
            RClickTime = 0;            
        }
...
function MMouseUp(float X, float Y)
{
    ...
    {
        // End:0x81
        if(((! bIgnoreMDoubleClick && Abs(X - MClickX) <= float(1)) && (Y - MClickY) <= float(1)) && GetLevel().TimeSeconds < (MClickTime + 0.6))
        {
            MDoubleClick(X, Y);
            MClickTime = 0;            
        }
...
function LMouseUp(float X, float Y)
{
    ...
    {
        // End:0x81
        if(((! bIgnoreLDoubleClick && Abs(X - ClickX) <= float(1)) && (Y - ClickY) <= float(1)) && GetLevel().TimeSeconds < (ClickTime + 0.6))
        {
            DoubleClick(X, Y);
            ClickTime = 0;            
        }
...
{
    // End:0x53
    if((NavRepeatTimeout != float(0)) && NavRepeatTimeout <= GetLevel().TimeSeconds)
    {
        NavRepeatTimeout = 0;
...
final function PaintClients(Canvas C, float X, float Y)
{
    ...
    if(Child == none)
    {
        ...
        if(bUWindowActive || Child.bLeaveOnscreen)
        {
            ...
            {
                Child.WindowEvent(10, C, X - Child.WinLeft, Y - Child.WinTop, 0);
...
function UWindowWindow FindWindowUnder(float X, float Y)
{
    ...
    if(Child == none)
    {
        ...
        {
            // End:0x1E7
            if(((((((Child.WinWidth > float(0)) && Child.WinHeight > float(0)) && X >= Child.WinLeft) && X <= (Child.WinLeft + Child.WinWidth)) && Y >= Child.WinTop) && Y <= (Child.WinTop + Child.WinHeight)) && ! Child.CheckMousePassThrough(X - Child.WinLeft, Y - Child.WinTop))
            {
...
function bool MessageClients(UWindowWindow.WinMessage msg, Canvas C, float X, float Y, int Key)
{
    ...
    {
        // End:0xE3
        if((((X >= ModalWindow.WinLeft) && X <= (ModalWindow.WinLeft + ModalWindow.WinWidth)) && Y >= ModalWindow.WinTop) && Y <= (ModalWindow.WinTop + ModalWindow.WinHeight))
        {
        ...
        else
        {
            ...
            if(Window == none)
            {
                ...
                if(Window.NextSiblingWindow != Child)
                {
                    ...
                    // End:0x182
                    if(Window.NextSiblingWindow == none)
                    {
...
final function UWindowWindow CreateWindow(class<UWindowWindow> WndClass, optional float X, optional float Y, optional float W, optional float h, optional UWindowWindow OwnerW, optional bool bUnique, optional name ObjectName)
{
    ...
    // End:0x7D
    if(NameForString(ObjectName, 'None'))
    {
...
{
    DrawStretchedTextureSegment(C, X, Y, float(Tex.DrawStatic()), float(Tex.DrawTile()), 0, 0, float(Tex.DrawStatic()), float(Tex.DrawTile()), Tex, fAlpha, bNoColorSet,,, bBilinear, bTranslucent2);
    return;
}
...
final function DrawStretchedTexture(Canvas C, float X, float Y, float W, float h, MaterialEx Tex, optional float fAlpha, optional bool bNoColorSet, optional bool bBilinear, optional bool bTranslucent2, optional bool bNoClip)
{
    ...
    if(bNoClip)
    {
        DrawStretchedTextureSegmentNoClip(C, X, Y, W, h, 0, 0, float(Tex.DrawStatic()), float(Tex.DrawTile()), Tex, fAlpha, bNoColorSet,,, bBilinear, bTranslucent2);        
    }
    else
    {
        DrawStretchedTextureSegment(C, X, Y, W, h, 0, 0, float(Tex.DrawStatic()), float(Tex.DrawTile()), Tex, fAlpha, bNoColorSet,,, bBilinear, bTranslucent2);
    }
    return;
}
...
final function DrawStretchedTextureSegmentNoClip(Canvas C, float X, float Y, float W, float h, float tX, float tY, float tW, float tH, MaterialEx Tex, optional float fAlpha, optional bool bNoColorSet, optional bool bObsoleteIgnore, optional float Rotation, optional bool bNoBilinear, optional bool bTranslucent2)
{
    ...
    ClipX = C.ClipX;
    ClipY = C.ClipY;
    C.GetFrameCount(OrgX + (float(ClippingRegion.X) * Root.GUIScale), OrgY + (float(ClippingRegion.Y) * Root.GUIScale));
    C.RegisterActorClass(float(ClippingRegion.W) * Root.GUIScale, float(ClippingRegion.h) * Root.GUIScale);
    C.SetPause((X - float(ClippingRegion.X)) * Root.GUIScale, (Y - float(ClippingRegion.Y)) * Root.GUIScale);
    // End:0x185
    if(bTranslucent2)
    {
    ...
    C.SetClampMode(Tex, W * Root.GUIScale, h * Root.GUIScale, tX, tY, tW, tH, Rotation,,, bBilinear, fAlpha);
...
final function DrawClippedActor(Canvas C, float X, float Y, Actor A, bool Wireframe, Rotator RotOffset, Vector LocOffset)
{
    ...
    FOV = (GetPlayerOwner().FovAngle * 3.141593) / float(180);
    MeshLoc.X = 0;
    MeshLoc.Y = 0;
    MeshLoc.Z = 0;
    A.DisableDesiredRotation_Roll(RotOffset);
    A.SetDesiredRotation(MeshLoc + LocOffset);
    C.DrawScaledIcon(A, Wireframe, int(float(ClippingRegion.W) * Root.GUIScale), int(float(ClippingRegion.h) * Root.GUIScale), int(C.OrgX + (float(ClippingRegion.X) * Root.GUIScale)), int(C.OrgY + (float(ClippingRegion.Y) * Root.GUIScale)), true);
    return;
}
...
final function DrawUpBevel(Canvas C, float X, float Y, float W, float h, Texture t, optional float fAlpha, optional bool Fill, optional bool bNoColorSet)
{
    ...
    DrawStretchedTextureSegment(C, X + float(LookAndFeel.BevelUpTL.W), Y, (W - float(LookAndFeel.BevelUpTL.W)) - float(LookAndFeel.BevelUpTR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha, bNoColorSet);
    R = LookAndFeel.BevelUpTR;
    DrawStretchedTextureSegment(C, (X + W) - float(R.W), Y, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha, bNoColorSet);
    R = LookAndFeel.BevelUpL;
    DrawStretchedTextureSegment(C, X, Y + float(LookAndFeel.BevelUpTL.h), float(R.W), (h - float(LookAndFeel.BevelUpTL.h)) - float(LookAndFeel.BevelUpBL.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha, bNoColorSet);
    R = LookAndFeel.BevelUpR;
    DrawStretchedTextureSegment(C, (X + W) - float(R.W), Y + float(LookAndFeel.BevelUpTL.h), float(R.W), (h - float(LookAndFeel.BevelUpTL.h)) - float(LookAndFeel.BevelUpBL.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha, bNoColorSet);
    R = LookAndFeel.BevelUpBL;
    DrawStretchedTextureSegment(C, X, (Y + h) - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha, bNoColorSet);
    R = LookAndFeel.BevelUpB;
    DrawStretchedTextureSegment(C, X + float(LookAndFeel.BevelUpBL.W), (Y + h) - float(R.h), (W - float(LookAndFeel.BevelUpBL.W)) - float(LookAndFeel.BevelUpBR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha, bNoColorSet);
    R = LookAndFeel.BevelUpBR;
    DrawStretchedTextureSegment(C, (X + W) - float(R.W), (Y + h) - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha, bNoColorSet);
    // End:0x657
    if(Fill)
    {
        R = LookAndFeel.BevelUpArea;
        DrawStretchedTextureSegment(C, X + float(LookAndFeel.BevelUpTL.W), Y + float(LookAndFeel.BevelUpTL.h), (W - float(LookAndFeel.BevelUpBL.W)) - float(LookAndFeel.BevelUpBR.W), (h - float(LookAndFeel.BevelUpTL.h)) - float(LookAndFeel.BevelUpBL.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha, bNoColorSet);
    }
    return;
}
...
final function DrawMiscBevel(Canvas C, float X, float Y, float W, float h, Texture t, int BevelType, optional float fAlpha)
{
    ...
    DrawStretchedTextureSegment(C, X + float(LookAndFeel.MiscBevelTL[BevelType].W), Y, (W - float(LookAndFeel.MiscBevelTL[BevelType].W)) - float(LookAndFeel.MiscBevelTR[BevelType].W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha);
    R = LookAndFeel.MiscBevelTR[BevelType];
    DrawStretchedTextureSegment(C, (X + W) - float(R.W), Y, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha);
    R = LookAndFeel.MiscBevelL[BevelType];
    DrawStretchedTextureSegment(C, X, Y + float(LookAndFeel.MiscBevelTL[BevelType].h), float(R.W), (h - float(LookAndFeel.MiscBevelTL[BevelType].h)) - float(LookAndFeel.MiscBevelBL[BevelType].h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha);
    R = LookAndFeel.MiscBevelR[BevelType];
    DrawStretchedTextureSegment(C, (X + W) - float(R.W), Y + float(LookAndFeel.MiscBevelTL[BevelType].h), float(R.W), (h - float(LookAndFeel.MiscBevelTL[BevelType].h)) - float(LookAndFeel.MiscBevelBL[BevelType].h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha);
    R = LookAndFeel.MiscBevelBL[BevelType];
    DrawStretchedTextureSegment(C, X, (Y + h) - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha);
    R = LookAndFeel.MiscBevelB[BevelType];
    DrawStretchedTextureSegment(C, X + float(LookAndFeel.MiscBevelBL[BevelType].W), (Y + h) - float(R.h), (W - float(LookAndFeel.MiscBevelBL[BevelType].W)) - float(LookAndFeel.MiscBevelBR[BevelType].W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha);
    R = LookAndFeel.MiscBevelBR[BevelType];
    DrawStretchedTextureSegment(C, (X + W) - float(R.W), (Y + h) - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha);
    R = LookAndFeel.MiscBevelArea[BevelType];
    DrawStretchedTextureSegment(C, X + float(LookAndFeel.MiscBevelTL[BevelType].W), Y + float(LookAndFeel.MiscBevelTL[BevelType].h), (W - float(LookAndFeel.MiscBevelBL[BevelType].W)) - float(LookAndFeel.MiscBevelBR[BevelType].W), (h - float(LookAndFeel.MiscBevelTL[BevelType].h)) - float(LookAndFeel.MiscBevelBL[BevelType].h), float(R.X), float(R.Y), float(R.W), float(R.h), t, fAlpha);
    return;
}
...
final function TextSize(Canvas C, coerce string Text, out float W, out float h, optional float XScale, optional float YScale)
{
    ...
    C.SetPause(0, 0);
    C.SetClip(Text, W, h, XScale, YScale);
    W = W / Root.GUIScale;
    h = h / Root.GUIScale;
    return;
}
...
function Color NewColor(float fR, float fG, float fB)
{
    ...
    C.R = byte(fR * float(255));
    C.G = byte(fG * float(255));
    C.B = byte(fB * float(255));
    return C;
    return;
}
...
function NavUp(UWindowDialogControl Control, optional bool bFromHold)
{
    ...
    // End:0x19
    if(bNavUpRepeatBlock || Control != none)
    {
        return;
    }
    n = Control.NavUp;
    ...
    // End:0x7B [Loop If]
    if(((n == none) && n == Control) && ! n.bWindowVisible)
    {
        n = n.NavUp;
        // [Loop Continue]
        goto J0x2E;
...
function NavDown(UWindowDialogControl Control, optional bool bFromHold)
{
    ...
    // End:0x19
    if(bNavDownRepeatBlock || Control != none)
    {
        return;
    }
    n = Control.NavDown;
...
function NavLeft(UWindowDialogControl Control, optional bool bFromHold)
{
    ...
    // End:0x19
    if(bNavLeftRepeatBlock || Control != none)
    {
        return;
    }
    n = Control.NavLeft;
...
function NavRight(UWindowDialogControl Control, optional bool bFromHold)
{
    ...
    // End:0x19
    if(bNavRightRepeatBlock || Control != none)
    {
        return;
    }
    n = Control.NavRight;
