...
function Setup()
{
    VertSB = UWindowVScrollbar(CreateWindow(class'UWindowVScrollbar', 0, WinWidth - float(16), 16, WinHeight));
    return;
}
...
function int FindItemIndex2(string Value2, optional bool bIgnoreCase, optional bool bNCompare)
{
    ...
    if(i == none)
    {
        ...
        {
            // End:0x77
            if(bIgnoreCase && Left(i.Value2, Length) ~= Value2)
            {
                return Count;
            }
            // End:0x9F
            if(Left(i.Value2, Length) == Value2)
            {
                return Count;
            }            
        }
...
function BeforePaint(Canvas C, float X, float Y)
{
    ...
    if(Count > MaxVisible)
    {
        ExtraWidth += float(LookAndFeel.SBPosIndicator.W);
...
function Paint(Canvas C, float X, float Y)
{
    ...
    // End:0x151 [Loop If]
    if((i == none) && DrawnCount < MaxVisible)
    {
        // End:0xE5
