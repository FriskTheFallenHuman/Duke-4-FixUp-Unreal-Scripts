...
function UWindowLookAndFeel GetLookAndFeel(string LFClassName)
{
    ...
    LFClass = class<UWindowLookAndFeel>(SaveConfigFile(LFClassName, class'Class'));
    i = 0;
    J0x1F:
    ...
    Localize("Out of LookAndFeel array space!!");
    return none;
    return;
}
...
function DrawMouse(Canvas C)
{
    ...
    if(! GetPlayerOwner().GetSpeakerType())
    {
        ...
        C.SetPause((MouseX * GUIScale) - float(MouseWindow.cursor.HotX), (MouseY * GUIScale) - float(MouseWindow.cursor.HotY));
        C.RegisterMaterialEx(MouseWindow.cursor.Tex, 1);
    }
    return;
}
...
{
    StaticSaveConfig();
    Console.StaticSaveConfig();
    Console.Viewport.Actor.StaticSaveConfig();
    Close();    
...
{
    Localize(string(self) @ "(UWindowRootWindow)::PrecachePauseMenu");
    return;
}
...
function LoadScoreboardWindow(class<UWindowWindow> WndClass)
{
    ...
    {
        Localize("###Created special scoreboard");
        ScoreboardWindow.HideWindow();        
    }
    else
    {
        Localize("###Failed to create special scoreboard!!!");
    }
    return;
}
