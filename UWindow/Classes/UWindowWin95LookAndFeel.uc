...
function FW_DrawWindowFrame(UWindowFramedWindow W, Canvas C)
{
    ...
    R = FrameT;
    W.DrawStretchedTextureSegment(C, float(FrameTL.W), 0, (W.WinWidth - float(FrameTL.W)) - float(FrameTR.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t);
    R = FrameTR;
    W.DrawStretchedTextureSegment(C, W.WinWidth - float(R.W), 0, float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t);
    // End:0x20A
    if(W.bStatusBar)
    {
        temp = FrameSBL;        
    ...
    R = FrameL;
    W.DrawStretchedTextureSegment(C, 0, float(FrameTL.h), float(R.W), (W.WinHeight - float(FrameTL.h)) - float(temp.h), float(R.X), float(R.Y), float(R.W), float(R.h), t);
    R = FrameR;
    ...
    W.DrawStretchedTextureSegment(C, 0, W.WinHeight - float(R.h), float(R.W), float(R.h), float(R.X), float(R.Y), float(R.W), float(R.h), t);
    // End:0x4C1
    if(W.bStatusBar)
    {
        R = FrameSB;
    ...
    if(W.bStatusBar)
    {
        ...
        C.DrawColor.G = 0;
        C.DrawColor.B = 0;
        W.ClipText(C, 6, W.WinHeight - float(13), W.StatusBarText);
        C.DrawColor.R = 255;
        C.DrawColor.G = 255;
        C.DrawColor.B = 255;
    }
...
function ComboList_DrawBackground(UWindowComboList W, Canvas C)
{
    ...
    W.DrawStretchedTexture(C, 4, 0, W.WinWidth - float(8), 4, Texture'MenuT');
    W.DrawClippedTexture(C, W.WinWidth - float(4), 0, Texture'MenuTR');
    W.DrawClippedTexture(C, 0, W.WinHeight - float(4), Texture'MenuBL');
    W.DrawStretchedTexture(C, 4, W.WinHeight - float(4), W.WinWidth - float(8), 4, Texture'MenuB');
    W.DrawClippedTexture(C, W.WinWidth - float(4), W.WinHeight - float(4), Texture'MenuBR');
    W.DrawStretchedTexture(C, 0, 4, 4, W.WinHeight - float(8), Texture'MenuL');
    W.DrawStretchedTexture(C, W.WinWidth - float(4), 4, 4, W.WinHeight - float(8), Texture'MenuR');
    W.DrawStretchedTexture(C, 4, 4, W.WinWidth - float(8), W.WinHeight - float(8), Texture'MenuArea');
    return;
}
...
function Tab_SetupRightButton(UWindowTabControlRightButton W)
{
    ...
    W.WinWidth = float(SBPosIndicator.W);
    W.WinHeight = float(SBPosIndicator.h);
    W.WinTop = float(SBPosIndicator.h) - W.WinHeight;
...
{
    W.DrawStretchedTexture(C, 16, 0, W.WinWidth - float(32), 16, Texture'MenuBar');
    return;
}
...
function Menu_DrawMenuBarItem(UWindowMenuBar B, UWindowMenuBarItem i, float X, float Y, float W, float h, Canvas C)
{
    ...
    if(B.Selected != i)
    {
        ...
        B.DrawClippedTexture(C, (X + W) - float(1), 1, Texture'MenuHighlightR');
        B.DrawStretchedTexture(C, X + float(1), 1, W - float(2), 16, Texture'MenuHighlightM');
    }
    C.Font = C.TallFont;
    C.DrawColor.R = 0;
    C.DrawColor.G = 0;
    C.DrawColor.B = 0;
    B.ClipText(C, X + float(B.Spacing / 2), 2, i.Caption, true);
    return;
}
...
{
    W.DrawClippedTexture(C, 0, 0, Texture'MenuTL');
    W.DrawStretchedTexture(C, 2, 0, W.WinWidth - float(4), 2, Texture'MenuT');
    W.DrawClippedTexture(C, W.WinWidth - float(2), 0, Texture'MenuTR');
    W.DrawClippedTexture(C, 0, W.WinHeight - float(2), Texture'MenuBL');
    W.DrawStretchedTexture(C, 2, W.WinHeight - float(2), W.WinWidth - float(4), 2, Texture'MenuB');
    W.DrawClippedTexture(C, W.WinWidth - float(2), W.WinHeight - float(2), Texture'MenuBR');
    W.DrawStretchedTexture(C, 0, 2, 2, W.WinHeight - float(4), Texture'MenuL');
    W.DrawStretchedTexture(C, W.WinWidth - float(2), 2, 2, W.WinHeight - float(4), Texture'MenuR');
    W.DrawStretchedTexture(C, 2, 2, W.WinWidth - float(4), W.WinHeight - float(4), Texture'MenuArea');
    return;
}
...
function Menu_DrawPulldownMenuItem(UWindowPulldownMenu m, UWindowPulldownMenuItem Item, Canvas C, float X, float Y, float W, float h, bool bSelected)
{
    ...
    {
        m.DrawClippedTexture(C, (X + W) - float(9), Y + float(3), Texture'MenuSubArrow');
    }
