...
function UWindowPageControlPage AddPage(string Caption, class<UWindowPageWindow> PageClass, optional name ObjectName)
{
    ...
    P.Page = UWindowPageWindow(CreateWindow(PageClass, 0, TabArea.WinHeight - float(LookAndFeel.TabSelectedM.h - LookAndFeel.TabUnselectedM.h), WinWidth, WinHeight - (TabArea.WinHeight - float(LookAndFeel.TabSelectedM.h - LookAndFeel.TabUnselectedM.h)),,, ObjectName));
    P.Page.OwnerTab = P;
    GetPlayerOwner().BroadcastMessage("AddPage" @ string(P));
    // End:0x135
    if(P == SelectedTab)
    {
        P.Page.HideWindow();        
