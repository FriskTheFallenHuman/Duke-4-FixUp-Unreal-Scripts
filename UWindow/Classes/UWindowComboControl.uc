...
{
    super.Created();
    EditBox = UWindowEditBox(CreateWindow(class'UWindowEditBox', 0, 0, WinWidth - float(LookAndFeel.ComboBtnDown.W), WinHeight));
    EditBox.NotifyOwner = self;
    EditBoxWidth = WinWidth / float(2);
    EditBox.bTransient = true;
    Button = UWindowComboButton(CreateWindow(class'UWindowComboButton', WinWidth - float(LookAndFeel.ComboBtnDown.W), 0, float(LookAndFeel.ComboBtnDown.W), float(LookAndFeel.ComboBtnDown.h)));
    Button.Owner = self;
...
function SetButtons(bool bInButtons)
{
    ...
    if(bInButtons)
    {
        LeftButton = UWindowComboLeftButton(CreateWindow(class'UWindowComboLeftButton', WinWidth - float(LookAndFeel.SBLeftUp.W), 0, float(LookAndFeel.SBLeftUp.W), float(LookAndFeel.SBLeftUp.h)));
        RightButton = UWindowComboRightButton(CreateWindow(class'UWindowComboRightButton', WinWidth - float(LookAndFeel.SBRightUp.W), 0, float(LookAndFeel.SBRightUp.W), float(LookAndFeel.SBRightUp.h)));        
    }
    else
    {
        LeftButton = none;
