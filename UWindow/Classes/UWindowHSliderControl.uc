...
function BeforePaint(Canvas C, float X, float Y)
{
    ...
    switch(Align)
    {
        ...
        // End:0xCD
        case 2:
            SliderDrawX = (WinWidth - SliderWidth) / float(2);
    ...
    if(bMouseDown && GetLevel().TimeSeconds > (LastSlideTime + 0.1))
    {
        ...
        // End:0x1A9
        if((X >= (TrackStart - (TrackWidth / float(2)))) && X <= ((TrackStart - (TrackWidth / float(2))) + TrackWidth))
        {
            bSliding = true;
            GrabOffset = X - (TrackStart - (TrackWidth / float(2)));
            Root.CaptureMouse();
        }
        // End:0x205
        if((X < (TrackStart - (TrackWidth / float(2)))) && X > SliderDrawX)
        {
            // End:0x1F6
            if(Step != float(0))
            {
        ...
        // End:0x26F
        if((X > ((TrackStart - (TrackWidth / float(2))) + TrackWidth)) && X < (SliderDrawX + SliderWidth))
        {
            // End:0x260
            if(Step != float(0))
            {
...
{
    LookAndFeel.HSlider_Draw(self, C, ParentWindow.ChildInFocus != self);
    return;
}
