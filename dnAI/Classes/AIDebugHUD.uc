...
simulated function DrawAIHUD(Canvas C)
{
    ...
    else
    {
        ...
        TmpString = TmpString $ "  --  DANGER  --  DANGER  --  DANGER --  DANGER --  DANGER --";
    }
    ...
    DrawString(C, "Disp:");
    // End:0x791
    if(int(m_aTarget.PercentageMoved * 100) < 50)
    {
        C.DrawColor = RedColor;
    }
    DrawString(C, "Moved:");
    ...
    // End:0x812 [Loop If]
    if((i < string(m_aTarget.PathCache)) && m_aTarget.PathCache[i].Valid)
    {
        ++ i;
        ++ NumPathCalls;
        // [Loop Continue]
    ...
    if(m_aTarget.IsGuardingActor(GuardedActor, InnerRange, OuterRange))
    {
        ...
        GuardString = ((((string(GuardedActor.Name) $ " (") $ string(InnerRange)) $ ", ") $ string(OuterRange)) $ ")";        
    }
    else
    {
        // End:0xA06
        ...
        {
            DrawString(C, "Guarding:");
            GuardString = ((((("(" $ string(GuardedPos)) $ ") (") $ string(InnerRange)) $ ", ") $ string(OuterRange)) $ ")";
        }
    }
    DrawString(C, "");
    ...
    DrawString(C, "Ammo:");
    // End:0xB4E
    if(m_aTarget.TKObj == none)
    {
        DrawString(C, "TKObj:");
    }
    // End:0xB7A
    if(m_aTarget.CarriedActor == none)
    {
        DrawString(C, "Carrying:");
    }
    // End:0xBA2
    if(m_aTarget.Prop == none)
    {
        DrawString(C, "Prop:");
    }
    // End:0xBD1
    if(m_aTarget.DesiredProp == none)
    {
    ...
    C.GetScreenXYNoClip("TimeSinceContact:  ", xw, yh);
    sx += xw;
    StartColumn(int(sx), int(sy), int(yh));
    ...
    {
        DrawString(C, string(DynamicLoadObject(class'EAIExecutive', int(CurExecutive))) $ " SUSPENDED");        
    }
    else
    {
        DrawString(C, string(DynamicLoadObject(class'EAIExecutive', int(CurExecutive))));
    }
    ...
    DrawString(C, string(DynamicLoadObject(class'EAIOpStatus', int(m_aTarget.GetCurrentOpStatus()))));
    ...
    {
        C.DrawColor = RedColor;
        TmpString = string(m_aTarget.IsXbox()) $ " - DIED";
        DrawString(C, TmpString);
        C.DrawColor = LightBlueColor;        
    }
    else
    {
        DrawString(C, string(m_aTarget.IsXbox()));
    }
    // End:0xEF6
    if(m_aTarget.IsNotFinal('GotoX'))
    {
    ...
    {
        DrawString(C, ((((string(DynamicLoadObject(class'EAIWeapManMode', int(m_aTarget.WeaponManager.Mode))) $ " (") $ string(m_aTarget.WeaponManager.RefreshScanGate - Level.GameTimeSeconds)) $ "/") $ string(m_aTarget.WeaponManager.ScanGate - Level.GameTimeSeconds)) $ ")");        
    }
    else
    {
        DrawString(C, string(DynamicLoadObject(class'EAIWeapManMode', int(m_aTarget.WeaponManager.Mode))));
    }
    DrawString(C, string(DynamicLoadObject(class'EAIAttackConstraint', int(m_aTarget.AttackConstraint))));
    DrawString(C, "");
    ...
    DrawString(C, string(DynamicLoadObject(class'EPhysics', int(m_aTarget.Physics))));
    DrawString(C, string(m_aTarget.Velocity));
    DrawString(C, string(VSize(m_aTarget.Velocity)));
    DrawString(C, string(m_aTarget.Location - m_aTarget.OldLocation));
    // End:0x11EE
    if(int(m_aTarget.PercentageMoved * 100) < 50)
    {
        C.DrawColor = RedColor;
    }
    DrawString(C, string(int(m_aTarget.PercentageMoved * 100)) $ "%");
    C.DrawColor = LightBlueColor;
    // End:0x124C
    if(NumPathCalls > 1)
    {
    ...
    TmpString = "####################";
    TmpString = Left(TmpString, NumPathCalls);
    TmpString = (string(NumPathCalls) $ " ") $ TmpString;
    DrawString(C, TmpString);
    ...
    DrawString(C, string(m_aTarget.SetAimTargetActor()));
    DrawActor(C, m_aTarget.SetAimTargetPoint());
    DrawString(C, string(m_aTarget.LineOfSightTo()));
    DrawActor(C, m_aTarget.StopWaiting());
    GetVisibilityPoint(m_aTarget.LineOfSightTo(), NewColorBytes(255, 0, 0), 1E-05);
    // End:0x13BF
    ...
    {
        TmpString = "";
        i = m_aTarget.WeaponAimerGetNumTargets() - 1;
        J0x14EF:

        // End:0x1534 [Loop If]
        if(i >= 0)
        {
            TmpString = ((TmpString $ "(") $ m_aTarget.WeaponAimerGetTargetString(i)) $ ") ";
            -- i;
            // [Loop Continue]
            goto J0x14EF;
        }
        DrawString(C, TmpString);
        TmpString = "" $ string(DynamicLoadObject(class'EAIAim', int(m_aTarget.GetWeaponAimerState())));
        TmpString = (((TmpString $ " ") $ string(m_aTarget.WeaponAimerInfo.AngleToTarget)) $ " - ") $ string(m_aTarget.WeaponAimerInfo.DeltaAngleToTarget);
        // End:0x15DC
    ...
    {
        TmpString = "";
        i = m_aTarget.HeadAimerGetNumTargets() - 1;
        J0x1620:

        // End:0x1665 [Loop If]
        if(i >= 0)
        {
            TmpString = ((TmpString $ "(") $ m_aTarget.HeadAimerGetTargetString(i)) $ ") ";
            -- i;
            // [Loop Continue]
            goto J0x1620;
        }
    ...
    else
    {
        ...
        TmpString = (string(LoadedAmmo) $ "/") $ string(TotalAmmo);
        DrawString(C, TmpString);
    }
    // End:0x1762
    if(m_aTarget.TKObj == none)
    ...
    // End:0x18F9
    if(m_aTarget.AmbientSound == none)
    {
        DrawString(C, "");
    ...
    StartColumn(int(sx), int(sy), int(yh));
    C.DrawColor = WhiteColor;
    DrawString(C, (((string(DynamicLoadObject(class'EAIAnimController', int(m_aTarget.AnimCtrl.m_eController))) @ "-") @ string(m_aTarget.AnimationController)) @ "|") @ string(m_aTarget.AnimCtrl.m_oController));
    DrawString(C, "--------------------");
    // End:0x1A06
    ...
    if(int(CurExecutive) == int(1))
    {
        ...
        {
            // End:0x1C3A
            if(ScriptIndex == (NumScripts - 1))
            {
                C.DrawColor = WhiteColor;
                DrawString(C, "> ");
                C.DrawColor = GrayColor;
                C.GetScreenXYNoClip("> ", xw, yh);
                -- m_iCurLine;
                m_iStartX += int(xw);
    ...
    C.GetScreenXYNoClip("Weapon_Anim:     ", xw, yh);
    sx += xw;
    StartColumn(int(sx), int(sy), int(yh));
    C.DrawColor = LightBlueColor;
    DrawString(C, "");
    ...
    DrawString(C, "ActionPoint: " $ string(m_aTarget.ActionPoint));
    DrawString(C, "Popup: " $ string(m_aTarget.PopUp));
    ...
    DrawString(C, "");
    // End:0x2334
    if(m_aTarget.PopUp == none)
    {
        DrawString(C, "------------------------");
        DrawString(C, "ActionPoint: " $ string(m_aTarget.ActionPoint));
        DrawString(C, "Popup: " $ string(m_aTarget.PopUp));
        DrawString(C, "------------------------");
        DrawString(C, "Popup DamageTaken: " $ string(m_aTarget.PopUpState.DamageTaken));
        DrawString(C, "Popup ShotsFired: " $ string(m_aTarget.PopUpState.ShotsFired));
        C.DrawColor = RedColor;
        DrawString(C, "Popup Mode: " $ string(m_aTarget.CurrentOpMode));
        C.DrawColor = GrayColor;
    }
    DrawString(C, "");
    DrawString(C, "------------------------");
    ...
    // End:0x244A [Loop If]
    if((i < string(m_aTarget.AIDebugMessageQueue)) && i < 10)
    {
        // End:0x2406
        if((m_aTarget.AIDebugMessageQueue[i].TimeExpire > float(0)) && m_aTarget.AIDebugMessageQueue[i].TimeExpire < Level.GameTimeSeconds)
        {
    ...
    // End:0x27BD
    if(m_aTarget.AnimCtrl.m_oController == none)
    {
        DrawString(C, "------------------------");
        DrawString(C, "");
        DrawString(C, (("WeaponBone Pitch: " $ string(m_aTarget.WeaponBoneAimConstraints.MinViewPitchOffset)) $ "/") $ string(m_aTarget.WeaponBoneAimConstraints.MaxViewPitchOffset));
        DrawString(C, (("WeaponBone Yaw:   " $ string(m_aTarget.WeaponBoneAimConstraints.MinViewYawOffset)) $ "/") $ string(m_aTarget.WeaponBoneAimConstraints.MaxViewPitchOffset));
        DrawString(C, (("HeadAimer Pitch: " $ string(m_aTarget.HeadBoneAimConstraints.MinViewPitchOffset)) $ "/") $ string(m_aTarget.HeadBoneAimConstraints.MaxViewPitchOffset));
        DrawString(C, (("HeadAimer Yaw:   " $ string(m_aTarget.HeadBoneAimConstraints.MinViewYawOffset)) $ "/") $ string(m_aTarget.HeadBoneAimConstraints.MaxViewPitchOffset));
        DrawString(C, "");
        DrawString(C, "------------------------");
        i = 0;
        J0x2622:
        ...
        // End:0x26D4 [Loop If]
        if(i < string(m_aTarget.BoneAim))
        {
            DrawString(C, ((((((("Bone #" $ string(i)) $ ":") @ string(m_aTarget.BoneAim[i].BoneName)) @ "(") @ string(m_aTarget.BoneAim[i].Pitch)) $ ",") @ string(m_aTarget.BoneAim[i].Yaw)) @ ")");
            ++ i;
            // [Loop Continue]
            goto J0x2622;
        }
        ...
        // End:0x27BD [Loop If]
        if(i < string(m_aTarget.BoneHeadAim))
        {
            DrawString(C, ((((((("Bone #" $ string(i)) $ ":") @ string(m_aTarget.BoneHeadAim[i].BoneName)) @ "(") @ string(m_aTarget.BoneHeadAim[i].Pitch)) $ ",") @ string(m_aTarget.BoneHeadAim[i].Yaw)) @ ")");
            ++ i;
            // [Loop Continue]
            goto J0x270B;
        }
    ...
    DrawString(C, "CanProcFire: " $ string(m_aTarget.CanProcFire));
    DrawString(C, "CanPopupAim: " $ string(m_aTarget.CanPopupAim));
    // End:0x29AC
    if(m_aTarget.Weapon == none)
    {
        DrawString(C, "ShotCount: " $ string(m_aTarget.WeaponManager.ShotCount));
        DrawString(C, "CanFireWeapon: " $ string(m_aTarget.CanFireWeapon()));
        DrawString(C, "IsOutOfLoadedAmmo: " $ string(m_aTarget.Weapon.IsOutOfLoadedAmmo()));
        DrawString(C, "bWeaponFireLockout: " $ string(m_aTarget.Weapon.bWeaponFireLockout));
    }
    // End:0x2A18
    ...
    DrawString(C, "------------Guard----");
    DrawString(C, "InnerRange: " $ string(m_aTarget.GuardInf.InnerRange));
    DrawString(C, "OuterRange: " $ string(m_aTarget.GuardInf.OuterRange));
    DrawString(C, "------------SJump----");
    DrawString(C, "JumpParms.Mode: " $ string(m_aTarget.JumpParms.Mode));
    return;
}
