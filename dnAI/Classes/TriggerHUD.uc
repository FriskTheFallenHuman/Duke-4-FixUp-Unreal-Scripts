...
simulated function DrawAIHUD(Canvas C)
{
    ...
    C.GetScreenXYNoClip("Trigger HUD", xw, yh);
    ...
    C.SetPause(float(int(float(C.SizeX) * 0.005)), float(int(float(C.SizeY) * 0.055)));
...
final function PopulateTriggerList()
{
    ...
    // End:0x1D2
    foreach RotateVectorAroundAxis(class'TestingTrigger', t)
    {
        t.GetZoneLastRenderTime(true);
        // End:0x1BA
        if(NameForString(t.Group, 'None'))
        {
            i = 0;
            J0x4C:
