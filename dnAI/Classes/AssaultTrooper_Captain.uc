...
{
    AllowQuickTeleports = HandleIntModifier(bAllowQuickTeleportsModifier, AllowQuickTeleports);
    return;
}
...
function UpdateShrinkScale(float ShrinkSizeScale)
{
    ...
    // End:0x40
    if(JetPackEffect[0] == none)
    {
        JetPackEffect[0].RemoveActorColor(ShrinkSizeScale * JetPackEffect[0].default.DrawScale);
    }
    // End:0x75
    if(JetPackEffect[1] == none)
    {
        JetPackEffect[1].RemoveActorColor(ShrinkSizeScale * JetPackEffect[1].default.DrawScale);
    }
    return;
}
...
event PostBeginPlay()
{
    ...
    SetupWeapon();
    SetupWeaponFX();
    Destroy(2, false, 'RndStuff');
    MinRoarHealth = int(Health * 0.8);
    HaywireSmokeParticleSystem = SoftParticleSystem(FindMountedActor('HaywireSmokeFX', class'SoftParticleSystem'));
    // End:0x6C
    if(HaywireSmokeParticleSystem == none)
    {
        HaywireSmokeParticleSystem.Enabled = false;
    }
    TeleportParms.StartLight = EmptyTouchClasses(class'AssaultTrooper_Captain_TeleportLight', self,, Location);
    TeleportParms.EndLight = EmptyTouchClasses(class'AssaultTrooper_Captain_TeleportLight', self,, Location);
    return;
}
...
event Destroyed()
{
    ...
    if(JetPackEffect[0] == none)
    {
        JetPackEffect[0].RemoveTouchClass();
        JetPackEffect[0] = none;
    }
    // End:0x52
    if(JetPackEffect[1] == none)
    {
        JetPackEffect[1].RemoveTouchClass();
        JetPackEffect[1] = none;
    }
    // End:0x6B
    if(MuzzleFlash == none)
    {
        MuzzleFlash.RemoveTouchClass();
    }
    // End:0x84
    if(MuzzleLight == none)
    {
        MuzzleLight.RemoveTouchClass();
    }
    // End:0x9D
    if(MyWeapon == none)
    {
...
function RndStuff()
{
    ...
    if(DoJetPackWobble && int(AnimCtrl.m_eController) == int(8))
    {
        ...
        if(! IsNotFinal('GotoX'))
        {
            Velocity.X = Velocity.X + ((FRand() - 0.5) * 30);
            Velocity.Y = Velocity.Y + ((FRand() - 0.5) * 30);
            Velocity.Z = Velocity.Z + ((FRand() - 0.5) * 30);
        }
    }
    Destroy(0.1 + (FRand() * 0.2), false, 'RndStuff');
    return;
}
...
event FireWeapon()
{
    ...
    // End:0x80
    if((Globals == none) && Globals.AssaultCaptainCanFireRealShotNow(self))
    {
        // End:0x7D
...
{
    return float(MinBulletDamage + Rand(RndBulletDamage));
    return;
}
...
{
    MyWeapon = EmptyTouchClasses(WeaponClass, self);
    // End:0x50
    if(MyWeapon == none)
    {
...
{
    MuzzleLight = EmptyTouchClasses(MuzzleLightClass, MyWeapon);
    // End:0x97
    if(MuzzleLight == none)
    {
        ...
        MuzzleLight.MountOrigin = Vect(1, 0, 0);
        MuzzleLight.MoveActor(MyWeapon);
    }
    MuzzleFlash = EmptyTouchClasses(MuzzleFlashClass, MyWeapon);
    // End:0x11C
    if(MuzzleFlash == none)
    {
        MuzzleFlash.MountType = 2;
        MuzzleFlash.MountMeshItem = MyWeapon.MuzzleBones[0];
        MuzzleFlash.MountOrigin = Vect(1, 0, 0);
        MuzzleFlash.MoveActor(MyWeapon);
    }
    return;
}
...
function StartWeaponFX()
{
    ...
    if(MuzzleLight == none)
    {
        MuzzleLight.RemoveActorColor(DrawScale);
        MuzzleLight.Flash();
    }
    // End:0x60
    if(MuzzleFlash == none)
...
animevent simulated function JetStart(optional EventInfo AnimEventInfo)
{
    ...
    if(JetPackEffect[0] != none)
    {
        JetPackEffect[0] = EmptyTouchClasses(class'dnAssaultTrooperJetpackEffect');
        // End:0x75
        if(JetPackEffect[0] == none)
        {
            JetPackEffect[0].GetZoneLastRenderTime(false);
            JetPackEffect[0].TickStyle = 2;
            JetPackEffect[0].SetPhysics(JetPackEffectMount[0], self);
        }
    }
    // End:0xDC
    if(JetPackEffect[1] != none)
    {
        JetPackEffect[1] = EmptyTouchClasses(class'dnAssaultTrooperJetpackEffect');
        // End:0xDC
        if(JetPackEffect[1] == none)
        {
            JetPackEffect[1].GetZoneLastRenderTime(false);
            JetPackEffect[1].TickStyle = 2;
            JetPackEffect[1].SetPhysics(JetPackEffectMount[1], self);
        }
    }
...
animevent simulated function JetStop(optional EventInfo AnimEventInfo)
{
    ...
    if(JetPackEffect[0] == none)
    {
        JetPackEffect[0].GetZoneLastRenderTime(true);
        JetPackEffect[0].TickStyle = 0;
        JetPackEffect[0].GetGravity();
    }
    // End:0x82
    if(JetPackEffect[1] == none)
    {
        JetPackEffect[1].GetZoneLastRenderTime(true);
        JetPackEffect[1].TickStyle = 0;
        JetPackEffect[1].GetGravity();
    }
    SetAnimFrame(none);
    // End:0x9A
    if(IsFlying())
...
function StartBackFire()
{
    ...
    // End:0x0E
    if(JetPackExplode == none)
    {
        return;
    }
    JetpackSmoke = EmptyTouchClasses(JetPackSmokeTrailFXClass, self);
    // End:0x96
    if(JetpackSmoke == none)
    {
        JetpackSmoke.MountType = 2;
        JetpackSmoke.MountMeshItem = 'spine2';
        JetpackSmoke.MountOrigin = Vect(-5, 0, 0);
        JetpackSmoke.MoveActor(self, false, false, false, true, false);
        JetpackSmoke.DestroyOnDismount = false;
    }
    JetPackExplode = EmptyTouchClasses(JetPackBackFireFXClass, self);
    // End:0x112
    if(JetPackExplode == none)
    {
        JetPackExplode.MountType = 2;
        JetPackExplode.MountMeshItem = 'spine2';
        JetPackExplode.MountOrigin = Vect(-5, 0, 0);
        JetPackExplode.MoveActor(self, false, false, false, true, false);
        BackFire();
    }
    return;
}
...
function BackFire()
{
    ...
    Destroy(0.4 + (FRand() * 0.1), false, 'BackFire');
    return;
}
...
event Engine.BaseAI.EAICode COND_ShouldRoarInsteadOfPain()
{
    ...
    // End:0x32
    if((Health + DamageInfo.Damage) < float(MinRoarHealth))
    {
        return 45;
    }
    // End:0x3E
    ...
    // End:0x84
    if((int(AnimCtrl.m_eController) != int(1)) && int(AnimCtrl.m_eController) != int(5))
    {
        return 39;
    }
    // End:0x9C
    ...
    // End:0xC3
    if((int(CheckCondition(41)) != int(0)) || int(CheckCondition(59)) == int(0))
    {
        return 0;
    }
    return 1;
...
event Engine.BaseAI.EAICode COND_ShouldRollInsteadOfPain()
{
    ...
    // End:0xA9
    if(DamageInfo.Valid && FRand() < 0.5)
    {
        // End:0x8D
        if((FRand() < 0.5) && int(CheckCondition(73)) == int(0))
        {
            DodgeSector = 2;
            return 0;            
        }
        ...
        {
            // End:0xA9
            if(int(CheckCondition(74)) == int(0))
            {
                DodgeSector = 3;
                return 0;
            }
...
event Engine.BaseAI.EAICode COND_ShouldTakeOff()
{
    ...
    HeightDelta = PrimaryTarget.GetPos().Z - Location.Z;
    HeightDelta += (CollisionHeight - PrimaryTargetActor.CollisionHeight);
    // End:0x90
    if(HeightDelta < 100)
    {
        return 21;
...
event Engine.BaseAI.EAICode COND_NeedToLand()
{
    ...
    MyOp = GetCurrentOp();
    HeightDelta = PrimaryTargetActor.Location.Z - Location.Z;
    HeightDelta += (CollisionHeight - PrimaryTargetActor.CollisionHeight);
    // End:0xE9
    if(((((((int(MyOp) == int(85)) || int(MyOp) == int(145)) || int(MyOp) == int(86)) || int(MyOp) == int(17)) || int(MyOp) == int(34)) && HeightDelta <= 10) && ! InAir(80))
    {
        return 0;
    }
    return 1;
...
{
    // End:0x50
    if(((int(CheckCondition(56)) == int(0)) && (PrimaryTarget.GetTimeSinceSeen() > TryGotoAttackTime) || int(CheckCondition(25)) == int(0)) && SetGoal(9))
    {
        return 0;
    }
    return 1;
...
event Engine.BaseAI.EAICode COND_GotoAttackInPosition()
{
    ...
    else
    {
        ...
        {
            // End:0x85
            if((Level.GameTimeSeconds - StartContactTime) >= 0.6)
            {
                StartContactTime = -99;
                return 0;
            }
...
function Engine.BaseAI.EAICode COND_CrouchForSmallEnemy()
{
    ...
    // End:0x43
    if(PrimaryTargetActor.Location.Z > (Location.Z + (CollisionHeight * 0.3)))
    {
        return 1;
    }
    return 0;
...
event NotifyOpStarted(Engine.BaseAI.EAIOp NewOp, Engine.BaseAI.EAIOp OldOp)
{
    ...
    if(int(NewOp) == int(80))
    {
        ...
        StartBackFire();
        Destroy(0.15 + (FRand() * 0.15), false, 'BackFire');
        // End:0x5E
        if(HaywireSmokeParticleSystem == none)
        {
    ...
    if(int(NewOp) == int(138))
    {
        ...
        Destroy(1.5, false, 'TimeoutTeleportAnim');        
    }
    else
    {
        bNoDamage = false;
...
function NotifyTeleportInProgress()
{
    ...
    if(i >= 0)
    {
        ...
        if(TripMines[i] == none)
        {
            TripMines[i].GetZoneLastRenderTime(true);
        }
        -- i;
        // [Loop Continue]
        goto J0x13;
...
event NotifyOpEnded(Engine.BaseAI.EAIOp Op, Engine.BaseAI.EAIOpStatus Status)
{
    ...
    if(int(Op) == int(138))
    {
        ...
        if(i >= 0)
        {
            ...
            if(TripMines[i] == none)
            {
                TripMines[i].GetZoneLastRenderTime(false);
        ...
        {
            // End:0x110
            if((DeathAnimChance == 0) || FRand() > DeathAnimChance)
            {
            ...
            else
            {
                ...
                {
                    bForceHaywire = true;
                    HaywireParms.HitNormal = VRand();
                    HaywireParms.HitNormal.Z = Abs(HaywireParms.HitNormal.Z);
                    Code = ForceDieOp(80);                    
                }
                else
                {
...
{
    // End:0x1C
    if(FlyGate > Level.GameTimeSeconds)
    {
        return 87;
    }
    // End:0x2B
...
{
    // End:0x1C
    if(LandGate > Level.GameTimeSeconds)
    {
        return 87;
    }
    // End:0x2B
...
{
    // End:0x1C
    if(JumpGate > Level.GameTimeSeconds)
    {
        return 87;
    }
...
event Engine.BaseAI.EAICode COND_ShouldTeleportToAP()
{
    ...
    // End:0x2B
    if(TeleportGate > Level.GameTimeSeconds)
    {
        return 87;
    }
    // End:0x3A
    if(FRand() > ChanceTeleport_ActionPoint)
    {
        return 25;
    }
...
function bool CheckSpotForTeleport(Vector Destination)
{
    ...
    TraceStart = Destination + Vect(0, 0, - CollisionHeight - 1);
    TraceEnd = Destination + Vect(0, 0, CollisionHeight - 1);
    Extent = Vect(CollisionRadius, CollisionRadius, 1);
    ...
    HitActor = AllActors(TraceStart, TraceEnd, Flags, Result, Extent);
    // End:0x10F
    if(HitActor != none)
    {
        // End:0x10F
...
function Engine.BaseAI.EAICode FindQuickTeleportSpot()
{
    ...
    // End:0x33
    if((! PrimaryTarget.IsValidTarget() || PrimaryTargetActor != none) || ! AllowQuickTeleports)
    {
        return 4;
    }
    TeleportParms.bForceLandOnFinish = false;
    TeleportParms.bForceJetpackOnFinish = false;
    Dest = PrimaryTargetActor.Location - (Vector(PrimaryTargetActor.Rotation) * ((PrimaryTargetActor.CollisionRadius + CollisionRadius) * 2));
    // End:0x101
    ...
    // End:0x234
    if((LastActionPoint == none) && int(FindTeleportAP(LastActionPoint)) == int(0))
    {
        // End:0x204
...
function NotifyOtherTouched(Actor TouchedActor, Actor TouchingActor)
{
    ...
    // End:0xC8
    if((((ChanceTeleport_Reaction > FRand()) && TKShield == none) && TouchedActor != TKShield) && int(GetExecutive()) == int(2))
    {
        Proj = Projectile(TouchingActor);
        // End:0xC8
        if((((Proj == none) && Proj.Instigator == none) && Proj.Instigator.bIsPlayerPawn) && ! IsA(Proj.Class, class'LaserMine'))
        {
            IncomingProjectile = Proj;
            TraceFire(1E-05, false, 'PerformQuickTeleport', true);
        }
    }
    return;
}
...
{
    DamageSinceTeleport += float(nDamage);
    // End:0x37
    if((ChanceTeleport_Reaction > FRand()) && DamageSinceTeleport > float(15))
    {
        TraceFire(1E-05, false, 'PerformQuickTeleport', true);
    }
    return;
...
{
    // End:0x92
    if(((((Health > 0) && int(CurrentOp) != int(138)) && ! HasTripMineAttached()) && int(FrozenState) == int(0)) && int(GetExecutive()) == int(2))
    {
        // End:0x92
        if(int(FindQuickTeleportSpot()) == int(0))
        {
            CanUseOp(138);
            CurrentOpMustFinish = false;
            bAcceptMines = false;
...
final simulated function KnockOffHelmet()
{
    ...
    {
        Helmet.Destroyed();
        Helmet.SetHealth(50 * - Vector(Rotation), Helmet.Location, 'None');
    }
    return;
}
...
{
    // End:0x37
    if(bHeadKnockedOff && SetScaleModifier() == none)
    {
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.RegisterMaterialClass(WeaponClass);
    PrecacheIndex.RegisterMaterialClass(MuzzleLightClass);
    PrecacheIndex.RegisterMaterialClass(MuzzleFlashClass);
    PrecacheIndex.RegisterMaterialClass(TracerClass);
    PrecacheIndex.RegisterMaterialClass(WaterTracerClass);
    PrecacheIndex.RegisterMaterialClass(class'dnAssaultTrooperJetpackEffect');
    PrecacheIndex.RegisterMaterialClass(TeleportParms.StartParticleSystem);
    PrecacheIndex.RegisterMaterialClass(TeleportParms.EndParticleSystem);
    PrecacheIndex.RegisterAnimationControllerEntry(TeleportParms.StartOverlay);
    PrecacheIndex.RegisterMaterialClass(class'AssaultTrooper_Captain_TeleportLight');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, FiringSound);
    PrecacheIndex.InitAnimationControllerEx(VoicePack, TeleportParms.StartSound);
    PrecacheIndex.InitAnimationControllerEx(VoicePack, TeleportParms.EndSound);
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_JetPackBoost');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_JetpackOn');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_JetPackOff');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_JetpackHaywire');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, HaywireParms.ExplosionSoundName);
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_Melee');
    // End:0x2DC
    if(PrecacheIndex.MapName ~= "Map02")
    {
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_ArmWhoosh');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_ClawSwipe');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_HollowMetalImpact');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_BigValve');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_MetalGibExplosion');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'SCR_ThumpMetal');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_MetalImpact');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'SCR_RipFloor');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'SCR_RipFloor');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_SplitDoorNudge');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_SplitDoorRoar');
        PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_ElevatorOpen');
    }
