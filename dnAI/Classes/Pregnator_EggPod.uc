...
function EggPodDied()
{
    ...
    if(! WasLaunched)
    {
        MoveToward(Location + (Normal(PodVel >> Rotation) * 15));
    }
    return;
}
...
simulated function SpawnCorpse()
{
    ...
    // End:0x58
    if(ForceGib && MyCorpse == none)
    {
        MyCorpse.HurtRadiusActivity(none, 200, Vect(0, 0, 1), CollisionRadius, -1.1, class'ExplosionDamage', Location);
    }
    return;
}
...
{
    PodFallAnim = CompositeNames(NewPodFallAnim $ Orientation);
    PodLandAnim = CompositeNames(NewPodLandAnim $ Orientation);
    return;
}
...
function ForceAnim(Engine.BaseAI.EAIAnimXType AnimType, name AnimName, Actor TargetActor)
{
    ...
    FullAnimName = CompositeNames(string(AnimName) $ Orientation);
    SetAnimPresets(AnimType, FullAnimName, true, -1, TargetActor, Vect(0, 0, 0), 0);
    CanUseOp(6);
    ExecuteOp(6);
    return;
}
...
animevent simulated function LeftEggPod(optional EventInfo AnimEventInfo)
{
    ...
    DoExitWound(class'IKSystemInfo_Walker');
    return;
}
