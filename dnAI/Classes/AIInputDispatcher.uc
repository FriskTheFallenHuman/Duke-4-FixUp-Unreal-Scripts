...
function bool VerifySelf()
{
    ...
    {
        InvalidAlert(((("Warning: " $ string(self)) $ " had 0 AIInputEvents and realized his life wasn't worth living. ") $ string(self)) $ " has left the game with -1 frags.");
        return false;
    }
    i = 0;
    J0x9B:
    ...
    // End:0xDA [Loop If]
    if(i < string(AIInputEvents))
    {
        // End:0xD0
        if(AIInputEvents[i].Delay != 0)
        {
            ValidLoop = true;
            // [Explicit Break]
            goto J0xDA;
...
function StartDispatching()
{
    ...
    if(i < string(AIInputEvents))
    {
        CurrentDelay += Abs(AIInputEvents[i].Delay);
...
final function Dispatch()
{
    ...
    // End:0x26
    if((Index == (string(AIInputEvents) - 1)) && ! Loop)
    {
        EndDispatching();
    }
    cont = Dispatching;
    ...
    if((cont && Dispatching) && CurrentIndex == Index)
    {
        ++ Index;
        // End:0xD5
        if(Loop && Index == string(AIInputEvents))
        {
            StartDispatching();
        }
    }
