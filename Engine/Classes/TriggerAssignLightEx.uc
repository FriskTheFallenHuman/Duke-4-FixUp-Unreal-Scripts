...
function DoAssign(Actor A)
{
    ...
    if(l == none)
    {
        ...
        l.LightRadius = HandleVectModifier(LightRadiusModifier, l.LightRadius, NewLightRadius);
        l.LightRadius3D = HandleRotModifier(LightRadius3DModifier, l.LightRadius3D, NewLightRadius3D);
        l.LightColor = check(LightColorModifier, l.LightColor, NewLightColor);
        // End:0xFF
        if(AssignLightStyle)
        {
        ...
        if(TL == none)
        {
            TL.OriginalColor = check(LightColorOriginalModifier, TL.OriginalColor, NewLightColorOriginal);
            TL.TurnOffColor = check(TurnOffColorModifier, TL.TurnOffColor, NewTurnOffColor);
        }
    }
    return;
}
...
final function SetupTicking()
{
    ...
    FinalRedScaleString = Caps(FinalRedScaleString);
    FinalGreenScaleString = Caps(FinalGreenScaleString);
    FinalBlueScaleString = Caps(FinalBlueScaleString);
    // End:0x7E
    if(FinalScaleTotalTime > 0)
    {
        TimePerCharRed = float(Len(FinalRedScaleString)) / FinalScaleTotalTime;
        TimePerCharGreen = float(Len(FinalGreenScaleString)) / FinalScaleTotalTime;
        TimePerCharBlue = float(Len(FinalBlueScaleString)) / FinalScaleTotalTime;        
    }
    else
    {
        TimePerCharRed = 0;
...
final function CompletedLightString()
{
    ...
    if(bRestoreLights)
    {
        ...
        // End:0x6C [Loop If]
        if(i < string(TrackedLights))
        {
            // End:0x62
            if(TrackedLights[i].LightActor == none)
            {
    ...
    bActive = false;
    // End:0x9F
    if(NameForString(CompleteEvent, 'None'))
    {
...
event Tick(float DeltaTime)
{
    ...
    WorkingColor.R = StrGetChar(FinalRedScaleString, int(ElapsedTime * TimePerCharRed)) * ;
    i = string(TrackedLights) - 1;
    J0xE4:
