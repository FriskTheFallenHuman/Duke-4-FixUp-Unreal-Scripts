...
function bool VerifySelf()
{
    ...
    {
        InvalidAlert(((("Warning: " $ string(self)) $ " had 0 OutEvents and realized his life wasn't worth living. ") $ string(self)) $ " has left the game with -1 frags.");
        return false;
    }
    // End:0xE8
    if(RepeatPattern)
...
final function name PickOutEvent()
{
    ...
    TestValue = Rand(string(OutEvents));
    // End:0x25
    if(! OnlyPickOnce)
    {
    ...
    // End:0x6E [Loop If]
    if((i < string(OutEvents)) && bool(OutEventUsed[TestValue]))
    {
        ++ i;
        ++ TestValue;
        // End:0x6B
        if(TestValue == string(OutEvents))
        {
            TestValue = 0;
        }
        // [Loop Continue]
    ...
    {
        ClearUsedList();
        return FoundOutEvent(Rand(string(OutEvents)));
    }
    RemoveTouchClass();
    return;
}
...
final function name PickPatternEvent()
{
    ++ PatternIndex;
    // End:0x1E
    if(PatternIndex == string(OutEvents))
    {
        PatternIndex = 0;
    }
