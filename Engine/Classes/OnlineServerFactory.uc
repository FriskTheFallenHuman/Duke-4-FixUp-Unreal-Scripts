...
function Exit()
{
    Localize("OnlineServerFactory: Interrupting Query and Clearing Servers");
    InterruptQuery();
    LockMutex();
    Servers.Empty();
    UnlockMutex();
...
{
    Localize(("OnlineServerFactory: Starting Query (" $ string(DynamicLoadObject(Enum'EAgentSearchConstraint', int(SearchConstraint)))) $ ")");
...
function Restart()
{
    Localize("OnlineServerFactory: Restarting");
    Exit();
    Start(-1, -1);
    return;
}
...
function Resort()
{
    Localize(("OnlineServerFactory: Sorting Servers(" $ string(DynamicLoadObject(Enum'EAgentBrowserSort', int(SortState)))) $ ")");
    SortServers();
    return;
}
