...
simulated event PostVerifySelf()
{
    ...
    GetPointRegion('ForceLightOff', strForceOffTag);
    GetPointRegion('TurnLightOn', TurnOnTag);
    GetPointRegion('TurnLightOff', TurnOffTag);
    return;
}
...
{
    return ((float(Rand(100)) / 100) * (fMaxIntensityChangeTime - fMinIntensityChangeTime)) + fMinIntensityChangeTime;
    return;
}
...
simulated function Update(float DeltaTime)
{
    ...
    switch(ToggleStyle)
    {
        ...
        case 1:
            ...
            LightColorV = VLerp(Scalar, StartStateColor, EndStateColor);
            // End:0x183
            break;
        // End:0x120
        case 2:
            ...
            {
                bUpdated = true;
                Scalar = Lerp(FRand(), fFlickerMinBrightness, fFlickerMaxBrightness);
                LightColorV = VLerp(Scalar, StartStateColor, EndStateColor);
        ...
        case 3:
            ...
            {
                bUpdated = true;
                Scalar = (FRand() * StateTimeElapsed) / fStateChangeTime;
                LightColorV = VLerp(Scalar, StartStateColor, EndStateColor);
                NextIntensityChangeTime = StateTimeElapsed + (GetIntensityChangeTime());
            }
            // End:0x183
...
{
    LightColor.R = byte(LightVector.X * 255);
    LightColor.G = byte(LightVector.Y * 255);
    LightColor.B = byte(LightVector.Z * 255);
    return;
}
...
simulated function MaybeStartForceOffTimer()
{
    ...
    if(fForceOffTime > 0)
    {
        TraceFire(fForceOffTime, false, 'TurnOff');
    }
    return;
}
