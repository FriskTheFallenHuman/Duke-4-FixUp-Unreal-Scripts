...
function DoAssign(Actor A)
{
    ...
    if(V == none)
    {
        V.Priority = HandleFloatModifier(NewPriorityMod, V.Priority, NewPriority);
        V.bWaterVolume = HandleIntModifier(NewbWaterVolume, V.bWaterVolume);
        V.bKillVolume = HandleIntModifier(NewbKillVolume, V.bKillVolume);
        V.bKillAI = HandleIntModifier(NewbKillAI, V.bKillAI);
        V.bMoveProjectiles = HandleIntModifier(NewbMoveProjectiles, V.bMoveProjectiles);
        V.GroundFriction = HandleVectModifier(NewGroundFrictionMod, V.GroundFriction, NewGroundFriction);
        V.FluidFriction = HandleVectModifier(NewFluidFrictionMod, V.FluidFriction, NewFluidFriction);
        V.Gravity = HandleRotModifier(NewGravityMod, V.Gravity, NewGravity);
        V.TerminalVelocity = HandleVectModifier(NewTerminalVelocityMod, V.TerminalVelocity, NewTerminalVelocity);
        V.KLinearDampingScale = HandleVectModifier(NewKLinearDampingScaleMod, V.KLinearDampingScale, NewKLinearDampingScale);
        V.KAngularDampingScale = HandleVectModifier(NewKAngularDampingScaleMod, V.KAngularDampingScale, NewKAngularDampingScale);
        V.PhysicsTimeScale = HandleVectModifier(NewPhysicsTimeScaleMod, V.PhysicsTimeScale, NewPhysicsTimeScale);
        V.VolumePlayerEvent = HandleStringModifier(NewVolumePlayerEventMod, V.VolumePlayerEvent, NewVolumePlayerEvent);
        V.VolumePlayerExitEvent = HandleStringModifier(NewVolumePlayerExitEventMod, V.VolumePlayerExitEvent, NewVolumePlayerExitEvent);
        V.ActorTimeScale = HandleVectModifier(NewActorTimeScaleMod, V.ActorTimeScale, NewActorTimeScale);
        V.HUDEffectFadeTime = HandleVectModifier(NewHUDEffectFadeTimeMod, V.HUDEffectFadeTime, NewHUDEffectFadeTime);
        V.bVolumeWalkMaterialOnly = HandleIntModifier(NewbVolumeWalkMaterialOnly, V.bVolumeWalkMaterialOnly);
        V.bVolumeHitMaterialOnly = HandleIntModifier(NewbVolumeHitMaterialOnly, V.bVolumeHitMaterialOnly);
        V.VolumeFlotsamRotation = HandleNameModifier(NewVolumeFlotsamRotationMod, V.VolumeFlotsamRotation, NewVolumeFlotsamRotation);
        V.bReverbVolume = HandleIntModifier(NewbReverbVolume, V.bReverbVolume);
        V.bUseExtendedReverb = HandleIntModifier(NewbUseExtendedReverb, V.bUseExtendedReverb);
        V.ReverbTime = HandleVectModifier(ReverbTimeModifier, V.ReverbTime, ReverbTime);
        V.ReverbPreDelay = HandleVectModifier(ReverbPreDelayModifier, V.ReverbPreDelay, ReverbPreDelay);
        V.ReverbDamping = HandleVectModifier(ReverbDampingModifier, V.ReverbDamping, ReverbDamping);
        V.ReverbDryLevel = HandleVectModifier(ReverbDryLevelModifier, V.ReverbDryLevel, ReverbDryLevel);
        V.ReverbWetLevel = HandleVectModifier(ReverbWetLevelModifier, V.ReverbWetLevel, ReverbWetLevel);
        // End:0x4A5
        ...
        V.bRelativeVolumeVelocity = HandleIntModifier(NewbRelativeVolumeVelocity, V.bRelativeVolumeVelocity);
        V.dnPathSystem_FindPathPointTo(HandleRotModifier(NewVolumeVelocityMod, V.VolumeVelocity_Orig, NewVolumeVelocity));
    }
    return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    if(NewHUDEffectMaterialMod)
    {
        PrecacheIndex.RegisterAnimationControllerEntry(NewHUDEffectMaterial);
    }
    // End:0x41
    if(NewVolumeMaterialMod)
    {
        PrecacheIndex.SetAnimState(NewVolumeMaterial);
    }
    return;
}
