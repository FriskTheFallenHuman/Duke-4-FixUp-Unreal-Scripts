...
function DoAssign(Actor A)
{
    ...
    if(D == none)
    {
        D.bNeverClose = HandleIntModifier(NewNeverClose, D.bNeverClose);
        D.bLocked = HandleIntModifier(NewLocked, D.bLocked);
        D.bOpeningEventOnlyOnce = HandleIntModifier(NewOpeningEventOnlyOnce, D.bOpeningEventOnlyOnce);
        D.bFullyOpenedEventOnlyOnce = HandleIntModifier(NewFullyOpenedEventOnlyOnce, D.bFullyOpenedEventOnlyOnce);
        D.bClosingEventOnlyOnce = HandleIntModifier(NewClosingEventOnlyOnce, D.bClosingEventOnlyOnce);
        D.bFullyClosedEventOnlyOnce = HandleIntModifier(NewFullyClosedEventOnlyOnce, D.bFullyClosedEventOnlyOnce);
        // End:0x130
        ...
        if(bAssignFriend)
        {
            ...
            // End:0x216
            if(NameForString(NewFriendDoorTag, 'None'))
            {
                // End:0x215
                foreach RotateVectorAroundAxis(class'DoorMoverEx', f, NewFriendDoorTag)
                {
