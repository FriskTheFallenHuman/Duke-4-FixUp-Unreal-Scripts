...
function NotifyGameEnded()
{
    ...
    // End:0x40
    if(CurrentPlaylist.IsInfinite && CurrentPlaylistMapIndex >= string(CurrentPlaylist.Entry))
    {
...
exec function AgentCreateGame(optional int in_PlaylistId, optional bool in_bIsPrivate)
{
    ...
    // End:0x6F [Loop If]
    if(i < string(class'MPMapInfo'.default.Playlist))
    {
        // End:0x65
...
{
    Localize("Refreshing Leaderboard Data");
...
exec function AgentReliableVoIP(bool in_Reliable)
{
    ...
    Localize("AgentReliableVoIP uses reliable packet: " @ string(in_Reliable));
    return;
}
