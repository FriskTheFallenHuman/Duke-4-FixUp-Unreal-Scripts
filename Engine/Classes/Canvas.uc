...
{
    SetClampMode(Tex, XL, YL, (CurX - OrgX) * Scale, (CurY - OrgY) * Scale, XL * Scale, YL * Scale);
    return;
}

final function DrawRect(MaterialEx Tex, float RectX, float RectY)
{
    SetClampMode(Tex, RectX, RectY, 0, 0, float(Tex.DrawStatic()), float(Tex.DrawTile()));
    return;
}

event LocalizationChange(string Language)
{
    Localize(((string(self) @ "::LocalizationChanged(") @ Language) @ ")");
    // End:0x3E
    if(Language == "jpn")
    {
    }
