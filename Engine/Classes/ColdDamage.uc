...
static function float ReduceDamage(Pawn DamagedPawn, float inDamage, Pawn DamageInstigator)
{
    ...
    // End:0x41
    if(DamagedPawn.Ego > float(0))
    {
        HealthValue += DamagedPawn.Ego;
    }
    // End:0x99
    if((! DamagedPawn.IsMP() && DamagedPawn.Level.InstaGib) && ! DamagedPawn.bIsPlayerPawn)
    {
        return HealthValue - 1;
    }
    // End:0xBC
    if((HealthValue - inDamage) < 1)
    {
        return HealthValue - 1;
    }
    return inDamage;
...
static function ApplyHUDDamageIndicator(PlayerPawn Player, float Damage, Vector DamageOrigin)
{
    ...
    // End:0x94
    if((Player == none) && Player.IsMP())
    {
        // End:0x4A
        if((Player.MyHUD != none) || default.DamageIndicatorLifetime <= 0)
        {
            return;
        }
        // End:0x91
