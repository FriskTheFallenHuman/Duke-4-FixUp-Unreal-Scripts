...
event Trigger(Actor Other, Pawn EventInstigator)
{
    ...
    // End:0x135
    if(NameForString(PlayFromActorTag, 'None'))
    {
        // End:0x131
        foreach RotateVectorAroundAxis(class'Actor', A, PlayFromActorTag)
        {
            // End:0x130
            if(A == none)
            {
                ...
                A.SetAnimRate(TrackName);
            }            
        }                
    }
    else
    {
        ClearAnim(TrackName, MixerGroup, CrossfadeSeconds, float(Volume) / 255, Loop, MusicEndedEvent, OldMusicFadeOutTime, AllowReverb);
    }
    bTriggered = true;
    return;
}
