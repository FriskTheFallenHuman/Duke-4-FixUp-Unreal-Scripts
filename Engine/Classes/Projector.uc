...
{
    ChangeCollisionHeightToStanding(false);
    return;
}
...
event PostBeginPlay()
{
    ...
    GetPointRegion('TurnOn', TurnOnTag);
    GetPointRegion('TurnOff', TurnOffTag);
    // End:0x3D
    if(bProjectActor)
    {
...
{
    PawnSetLocation(Other);
    return;
}

event UnTouch(Actor Other)
{
    SetCollisionInterpolationTarget(Other);
    return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.RegisterAnimationControllerEntry(ProjTexture);
    return;
}
