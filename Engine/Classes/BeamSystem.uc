...
{
    GetPointRegion('ResetEventSegments', SE_ResetEventSegments);
    GetPointRegion('ToggleTraceEnable', SE_ToggleTraceStatus);
    GetPointRegion('ToggleEnable', SE_ToggleEnable);
    GetPointRegion('ForceEnable', SE_ForceEnable);
    GetPointRegion('ForceDisable', SE_ForceDisable);
...
simulated function PostBeginPlay()
{
    ...
    BeamSegments.Empty();
    FinalizeSegments();
    WorldBox.Min = Location - Vect(5, 5, 5);
    WorldBox.Max = Location + Vect(5, 5, 5);
    Disable('Tick');
    // End:0xF3
    if((int(BeamType) != int(8)) && DynamicBeamPoints <= float(0))
    {
        // End:0xF3
        if(((int(BeamBrokenWhen) == int(0)) || int(BeamBrokenAction) == int(0)) && SpawnClassOnBeamCollision != none)
        {
            // End:0xF3
            if(((FrequencyVelocity == float(0)) && AmplitudeVelocity == float(0)) && NoiseVelocity == float(0))
            {
...
simulated function ConstructPresetSegments()
{
    ...
    // End:0x3B [Loop If]
    if(i < string(BeamSegments))
    {
        CopySegmentToInternal(BeamSegments[i], Segments[CalculateExtraOffset()]);
        ++ i;
        // [Loop Continue]
        goto J0x07;
    }
...
simulated function int AddSegmentTracer(Vector Loc1, Vector Loc2)
{
    ...
    Distance = VSize(Loc1 - Loc2);
    // End:0x30
    if(Distance <= (TracerLength * TracerRejectScale))
    {
        return -1;
    }
    i = CalculateExtraOffset();
