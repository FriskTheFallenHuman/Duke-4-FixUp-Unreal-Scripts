...
simulated event EnabledStateChange()
{
    ...
    if(Enabled)
    {
        SetParticleSystemHidden(2);        
    }
    else
    {
        SetParticleSystemHidden(3);
    }
    return;
}
...
simulated event Trigger(Actor Other, Pawn Instigator)
{
    ...
    switch(TriggerType)
    {
        ...
        case 4:
            ...
            bPulsed = true;
            Enabled = ! Enabled;
            f = (PulseSeconds + (PulseSecondsVariance * FRand())) - (PulseSecondsVariance / float(2));
            // End:0xCF
            if(f < 0)
            {
                f = 0;
            ...
            Destroy(f, false, 'DelayedActivity');
            GetPropertyText('Trigger');
            // End:0x13B
            if((bAggressivePhysicsDisable || DisableTickWhenEmpty) && int(TickStyle) == int(0))
            {
        ...
        case 8:
            UpdateEnabled = ! UpdateEnabled;
            f = FMax(0, FVar(PulseSeconds, PulseSecondsVariance));
            Destroy(f, false, 'DelayedActivity');
            PulseStartTime = Level.GameTimeSeconds;
            PulseEndTime = PulseStartTime + f;
            GetPropertyText('Trigger');
...
{
    return FClamp(InPitch + (1 - SystemSizeScale), InPitch * 0.5, InPitch * 1.5);
    return;
}
