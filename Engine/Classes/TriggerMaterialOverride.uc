...
simulated event UnTouch(Actor Other)
{
    ...
    if(aPawn == none)
    {
        ...
        // End:0x6B
        foreach aPawn.GetMapName(class'TriggerMaterialOverride', NewMaterialOverride)
        {
...
{
    // End:0x35
    if((OverrideMaterial == none) && FRand() <= OverrideChance)
    {
        Result = GetOverrideMaterial(Original);
...
{
    PrecacheIndex.SetAnimState(OverrideMaterial);
    return;
}
