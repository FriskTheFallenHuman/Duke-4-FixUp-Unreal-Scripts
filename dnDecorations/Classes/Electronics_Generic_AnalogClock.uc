...
function UpdateDisplay(float Hour, float Minute, float Second)
{
    ...
    Hour = (Hour % float(NumHours)) / float(NumHours);
    Minute /= float(NumMinutes);
    Second /= float(NumSeconds);
    ClockHour = int(((Hour + (Minute / float(NumHours))) + ((Second / float(NumMinutes)) / float(NumHours))) * 65535);
    ClockMinute = int((Minute + (Second / float(NumMinutes))) * 65535);
    ClockSecond = int(Second * 65535);
    // End:0xC5
    if(HourHand == none)
    {
...
{
    TimeOverride.Hour = Level.Hour % NumHours;
    TimeOverride.Minute = Level.Minute % NumMinutes;
    TimeOverride.Second = Level.Second % NumSeconds;
    return;
}
...
final function MoveHand(Actor HandActor, int Target, bool bSmoothMotion)
{
    ...
    HandActor.Markers_AddCircle((HandRotationDir * float(Target)) + HandRotationOffset, 0, HandMoveTime,,,,,,, true);
    return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.InitAnimationControllerEx(VoicePack, TickSoundName);
    return;
}
