...
{
    UpdateSeconds = int(Abs(float(UpdateSeconds)));
...
final function StartTicking()
{
    ...
    Destroy(UpdateRate, true, 'ClockTick');
    return;
}
...
final function UpdateTimeOverride()
{
    ...
    // End:0x9B
    if(TimeOverride.Minute >= NumMinutes)
    {
...
{
    Hour = Hour % float(NumHours);
    Minute = Minute % float(NumMinutes);
    Second = Second % float(NumSeconds);
    return;
}
