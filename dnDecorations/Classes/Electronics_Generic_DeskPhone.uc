...
function Destroyed()
{
    ...
    PerformDamageCategoryEffectEx('Reenable');
    bUsable = false;
    FindAndPlaySound(OffTheHook);
    TraceFire(10, false, 'StopTone');
    StopPlayingMessages();
    GetAnimGroup(5);
    return;
}
...
function StopPlayingMessages()
{
    ...
    PerformDamageCategoryEffectEx('PlayBeep');
    PerformDamageCategoryEffectEx('PlayNextMessage');
    PerformDamageCategoryEffectEx('DonePlayingMessages');
    return;
}
...
function PlayBeep()
{
    ...
    TraceFire(Delay, false, 'PlayNextMessage');
    return;
}
...
function PlayNextMessage()
{
    ...
    bPlayingBeep = false;
    Delay = FindAndPlaySound(Messages[++ CurrentMessage]) + 0.1;
    // End:0x49
    if(CurrentMessage >= string(Messages))
    {
        TraceFire(Delay, false, 'PlayEndOfMessages');        
    }
    else
    {
        TraceFire(Delay, false, 'PlayBeep');
    }
    return;
}
...
function PlayEndOfMessages()
{
    ...
    TraceFire(Delay, false, 'DonePlayingMessages');
    return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    if(i >= 0)
    {
        PrecacheIndex.InitAnimationControllerEx(VoicePack, Messages[i]);
        -- i;
        // [Loop Continue]
        goto J0x1A;
    }
    PrecacheIndex.InitAnimationControllerEx(VoicePack, OffTheHook);
    PrecacheIndex.InitAnimationControllerEx(VoicePack, EndOfMessages);
    PrecacheIndex.InitAnimationControllerEx(VoicePack, Beep);
    PrecacheIndex.InitAnimationControllerEx(VoicePack, NoNewMessages);
    return;
}
