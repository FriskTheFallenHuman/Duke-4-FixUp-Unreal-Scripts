...
event PostBeginPlay()
{
    ...
    if(Vent == none)
    {
        Vent.GetOverlayEffectAlpha(SetCallbackTimer());
        i = VisibleCollidingActors() - 1;
        J0x45:

        // End:0x74 [Loop If]
        if(i >= 0)
        {
            Vent.VisibleActors(i, RadiusActors(i));
            -- i;
            // [Loop Continue]
            goto J0x45;
        }
...
{
    // End:0x4F
    if(NameForString(States[CurrentStateIndex].MyAnim.Animation, 'None'))
    {
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'CircularGymVent_Bend');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'CircularGymVent_Break');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Sound_ElevatorNoise');
    PrecacheIndex.SetAnimPairState('VentRip_CircularGymVent_Idle');
    i = string(States) - 1;
    J0x88:
    ...
    if(i >= 0)
    {
        PrecacheIndex.SetChannelGridState(States[i].MyAnim.Animation, class'acCircularGymVent', class'CircularVent_Cover');
        -- i;
        // [Loop Continue]
        goto J0x88;
    }
...
state() AttachUserAnim
{
    event BeginState()
    {
        ...
        if(! bHidden)
        {
            ...
            Vent.GetZoneLastRenderTime(false);
        }
        return;
    }
    stop;
