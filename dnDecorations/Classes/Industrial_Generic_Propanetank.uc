...
event TakeDamage(Pawn Instigator, float Damage, Vector DamageOrigin, Vector DamageDirection, class<DamageType> DamageType, optional name HitBoneName, optional Vector DamageStart)
{
    ...
    ++ NumHits;
    // End:0x168
    if((damageAbsorbed < Health) && NumHits < 4)
    {
        // End:0x84
        if(NumHits == 1)
        {
        ...
        if(string(BulletFires) == 0)
        {
            ...
            SpawnLoc = Location + (Vect(0, 0, 0.75 * CollisionHeight) >> Rotation);
            StartFireAtBulletEntry(SpawnLoc, FireRotation);
            SpawnLoc = Location + (Vect(45, 0, 0.75 * CollisionHeight) >> Rotation);
            StartFireAtBulletEntry(SpawnLoc, FireRotation);
            SpawnLoc = Location + (Vect(-45, 0, 0.75 * CollisionHeight) >> Rotation);
            StartFireAtBulletEntry(SpawnLoc, FireRotation);
        }        
...
function StartFireAtBulletEntry(Vector BulletEntry, Rotator FireRotation)
{
    ...
    if(! DecorationIsDead())
    {
        BulletFires[BulletFires.Add(1)] = EmptyTouchClasses(BulletFireClass,,, BulletEntry, FireRotation);
        BulletFires[string(BulletFires) - 1].MountType = 0;
        BulletFires[string(BulletFires) - 1].MoveActor(self, false, false, true);
    }
    return;
}
...
function StopBulletFire()
{
    ...
    i = string(BulletFires) - 1;
    J0x0F:

    // End:0x4A [Loop If]
    if(i >= 0)
    {
        BulletFires[i].FreeSegment();
        BulletFires[i].RemoveTouchClass();
        -- i;
        // [Loop Continue]
        goto J0x0F;
    }
