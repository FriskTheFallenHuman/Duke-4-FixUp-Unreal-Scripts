...
function PostVerifySelf()
{
    ...
    {
        // End:0x50
        if(NameForString(LatchTag, 'None'))
        {
            Latch.GetAnimationStart(LatchTag);
        }
        // End:0x76
        if(NameForString(LatchMountParentTag, 'None'))
        {
            LatchMountParent = FindActor(class'Actor', LatchMountParentTag);
        }
        // End:0xCE
        if(LatchMountParent == none)
        {
            // End:0xB9
            if(NameForString(LatchMountParentBoneName, 'None'))
            {
                Latch.MountType = 2;
                Latch.MountMeshItem = LatchMountParentBoneName;
            }
            Latch.MoveActor(LatchMountParent,,, true);
        }
    }
...
{
    // End:0x33
    if((Lock == none) && ! Lock.DecorationIsDead())
    {
        Lock.CriticalDamage();
    }
    // End:0x4F
