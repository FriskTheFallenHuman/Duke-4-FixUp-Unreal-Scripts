...
event PostBeginPlay()
{
    ...
    GetPointRegion('TakeControl', TakeControlTag);
    GetPointRegion('ReleaseControl', ReleaseControlTag);
    // End:0x3B
    if(bStartInControl)
    {
        TakeControl(false);
...
simulated function bool CanBeUsedBy(Pawn TestPawn)
{
    ...
    if(ret)
    {
        ...
        if(Player == none)
        {
            ...
            playerDirection.Z = 0;
            doorDirection = Location - Player.Location;
            dotProd = playerDirection Dot doorDirection;
            // End:0x9C
            if(dotProd < 0)
            {
                return false;
...
{
    // End:0x3D
    if((Vector(DefaultRotation) Dot Normal(AttacheeCameraLoc - Vect(Location.X, Location.Y, AttacheeCameraLoc.Z))) < 0)
    {
        return true;
    }
    return false;
...
simulated function AttachPawnSuccess(Pawn Attachee, optional bool bForced)
{
    ...
    bFlip = ShouldFlipForUser(Attachee.SetDestinationActor());
    ReleaseControl();
    // End:0x3F
    if(bFlip)
    {
        DisableDesiredRotation_Roll(DefaultRotation >> Rot(0, 32768, 0));        
    }
    else
    {
        DisableDesiredRotation_Roll(DefaultRotation);
    }
    TakeControl(bFlip);
    // End:0x6C
...
{
    // End:0x40
    if((Door == none) && NameForString(Door.PortalSurfaceTag, 'None'))
    {
        TraceActors(Door.PortalSurfaceTag, bEnable);
    }
    return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.InitAnimationControllerEx(VoicePack, PryLoopSoundName);
    PrecacheIndex.SetAnimPairState(UserGridAnim);
    PrecacheIndex.SetChannelEventState(Mesh, MyGridAnim);
    return;
}
...
state() idle
{
    ...
    function FullyRelaxed()
    {
        ...
        SetGlobalAnimRate(States[CurrentStateIndex].MyAnim.Animation);
        return;
    }
    ...
    function UpdateUserLift()
    {
        ...
        // End:0x75
        if(PartiallyOpenPercent > 0)
        {
            UserLiftPct = 0.5 * (UserLiftVal + 1);
            // End:0x6A
            if((UserLiftPct >= PartiallyOpenPercent) && LastLiftPercent < PartiallyOpenPercent)
            {
        ...
        UpdateAnimGrids();
        SoundPitch = byte(float(64) * (1 + (0.15 * (UserLiftVal + 1))));
        // End:0xE3
        if(UserLiftVal == -1)
        {
            SoundVolume = byte(Max(0, int(SoundVolume) - int((4 * Level.TimeDeltaSeconds) * 255)));            
        }
        else
        {
            SoundVolume = byte(Min(255, int(SoundVolume) + int((4 * Level.TimeDeltaSeconds) * 255)));
        }
        return;
    }
    stop;
...
state() FinishOpenDoor
{
    ...
    event Tick(float DeltaSeconds)
    {
        ...
        {
            SoundVolume = byte(Max(0, int(SoundVolume) - int((4 * DeltaSeconds) * 255)));
            // End:0x4D
