/*******************************************************************************
 * Biology_Generic_GrowPod generated by Eliot.UELib using UE Explorer.exe.
...
function PodAction(Actor Other)
{
    ...
    // End:0x102
    if(! IsMP() || ! DukeMultiPlayer(Other).IsDrivingVehicle())
    {
        // End:0x102
        if(Other.bIsPawn && ! CoolingDown)
        {
            ...
            if(P.bFullyShrunk)
            {
                P.GetConstraintCount(LinVel);
                LinVel.Z = 0;
                P.KPushGeneratedSimpleState(LinVel);
                P.ReadyToPlay();
                P.ExpandPawn();
                ParticleBurstSystem.Trigger(none, none);
                CoolingDown = true;
                ParticleSystem.Enabled = false;
                Destroy(ParticleCoolDown, false, 'TurnParticlesOn');
...
simulated event AnimEndEx(SAnimEndInfo AnimEndInfo)
{
    ...
    // End:0x30
    if(AnimEndInfo.AnimName != 'Activate')
    {
    ...
    {
        // End:0x52
        if(AnimEndInfo.AnimName != 'Deactivate')
        {
