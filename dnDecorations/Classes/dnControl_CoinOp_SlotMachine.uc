...
event PostBeginPlay()
{
    ...
    VisibleActors(MyInfo.default.PayoutSectionID, MyInfo.default.PayoutTextureNone);
    InitialiseRotors();
...
final simulated function SwapToStaticTumblers()
{
    ...
    if(i < 3)
    {
        Symbol = int(Rotors[i].Panner.ShiftY / float(RotorTextureV / MyInfo.default.RotorSymbols));
        VisibleActors(MyInfo.default.RotorSectionID[i], MyInfo.default.SymbolTextures[Symbol]);
        ++ i;
        // [Loop Continue]
        goto J0x07;
    }
...
final simulated function SwapToPanningTumblers()
{
    ...
    if(i < 3)
    {
        VisibleActors(MyInfo.default.RotorSectionID[i], Rotors[i].Panner);
        ++ i;
        // [Loop Continue]
        goto J0x07;
    }
...
event Tick(float DeltaTime)
{
    ...
    // End:0x3B
    if(((User != none) && ! Active) && BlinksRemaining <= 0)
    {
        TickStyle = 0;
        return;
    }
    ...
    if(BlinksRemaining > 0)
    {
        ...
        {
            -- BlinksRemaining;
            BlinkTimeRemaining += 0.25;
            tempTexture = Texture(RadiusActors(MyInfo.default.PayoutSectionID));
            VisibleActors(MyInfo.default.PayoutSectionID, PreviousTexture);
    ...
    if(Active)
    {
        ...
        else
        {
            ...
            {
                // End:0x19E
                if((TimeSpinning >= (MyInfo.default.InitialSpinTime + (MyInfo.default.AdditionalRotorSpinTime * float(2)))) && Rotors[2].Velocity != 0)
                {
                    stoppingRotor = 2;
                }
            }
        ...
        if(i < 3)
        {
            ...
            vPanThisFrame = int(float(Rotors[i].Velocity) * DeltaTime);
            // End:0x34B
            if(stoppingRotor == i)
            {
                temp = (RotorTextureV / MyInfo.default.RotorSymbols) - (vPan % (RotorTextureV / MyInfo.default.RotorSymbols));
                // End:0x34B
                if(temp <= vPanThisFrame)
                {
                    R1 = (vPan + temp) / (RotorTextureV / MyInfo.default.RotorSymbols);
                    R1 = (R1 + MyInfo.default.RotorSymbolOffset[i]) % 8;
                    switch(i)
                    {
                        // End:0x2B4
                        case 0:
        ...
        if(Payout)
        {
            ...
            SwapToStaticTumblers();
            SetAnimFrame(none);
            R1 = int(GetRotorV(0)) / (RotorTextureV / MyInfo.default.RotorSymbols);
            r2 = int(GetRotorV(1)) / (RotorTextureV / MyInfo.default.RotorSymbols);
            r3 = int(GetRotorV(2)) / (RotorTextureV / MyInfo.default.RotorSymbols);
            R1 = (R1 + MyInfo.default.RotorSymbolOffset[0]) % 8;
            r2 = (r2 + MyInfo.default.RotorSymbolOffset[1]) % 8;
            r3 = (r3 + MyInfo.default.RotorSymbolOffset[2]) % 8;
            R1 = MapRotorSymbolToSymbolID(0, R1);
...
function PressStartButton()
{
    ...
    VisibleActors(MyInfo.default.PayoutSectionID, MyInfo.default.PayoutTextureBlank);
    i = 0;
    J0x41:
    ...
    if(i < 3)
    {
        Rotors[i].Velocity = ((MyInfo.default.RotorMaxVelocity / 4) * 3) + Rand(MyInfo.default.RotorMaxVelocity / 4);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                // End:0x402 [Loop If]
                                if((Rotors[0].DesiredSymbol == Rotors[Rand(2) + 1].DesiredSymbol) || Rotors[0].DesiredSymbol == 3)
                                {
                                    Rotors[0].DesiredSymbol = Rand(6);
                                    doOver = true;
                                    // [Loop Continue]
                                    goto J0x3AE;
                                }
...
function InitialiseRotors()
{
    ...
    if(i < 3)
    {
        ...
        Rotors[i].Panner.Material = RadiusActors(MyInfo.default.RotorSectionID[i]);
        VisibleActors(MyInfo.default.RotorSectionID[i], Rotors[i].Panner);
        Rotors[i].Velocity = 0;
        Rotors[i].DesiredSymbol = 0;
        RotorTextureV = Rotors[i].Panner.DrawTile();
        SetRotorV(i, Rand(MyInfo.default.RotorSymbols) * (RotorTextureV / MyInfo.default.RotorSymbols));
        ++ i;
        // [Loop Continue]
        goto J0x07;
    }
    ...
    if(i < 8)
    {
        ...
        if(Panner == none)
        {
            Panner.ShiftY = float(i * (RotorTextureV / MyInfo.default.RotorSymbols));
        }
        ++ i;
        // [Loop Continue]
        goto J0x17D;
...
function Win(int amount)
{
    ...
    Destroy((1 + float(amount / 100)) + FRand(), false, 'StopPayoutSound');
    // End:0x53
    if(DukePlayer(LastUser) == none)
    {
    ...
    VisibleActors(MyInfo.default.PayoutSectionID, NewPayoutTexture);
    BlinksRemaining = MyInfo.default.PayoutBlinks;
    BlinkTimeRemaining = 0.4;
    PreviousTexture = MyInfo.default.PayoutTextureBlank;
    return;
...
function DebugPrintSymbol(int iRotorID, int SymbolID)
{
    switch(SymbolID)
    {
        // End:0x27
        case 0:
            BroadcastLog(string(iRotorID) $ " -- BAR");
            // End:0xD4
            break;
        // End:0x4A
        case 1:
            BroadcastLog(string(iRotorID) $ " -- BAR BAR");
            // End:0xD4
            break;
        // End:0x6B
        case 2:
            BroadcastLog(string(iRotorID) $ " -- BELL");
            // End:0xD4
            break;
        // End:0x8E
        case 3:
            BroadcastLog(string(iRotorID) $ " -- CHERRY");
            // End:0xD4
            break;
        // End:0xB0
        case 4:
            BroadcastLog(string(iRotorID) $ " -- SEVEN");
            // End:0xD4
            break;
        // End:0xD1
        case 5:
            BroadcastLog(string(iRotorID) $ " -- NUKE");
            // End:0xD4
            break;
        // End:0xFFFF
        default:
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    if(i < 30)
    {
        PrecacheIndex.RegisterAnimation(MyInfo.default.PayoutTextures[i]);
        ++ i;
        // [Loop Continue]
        goto J0x12;
    }
    PrecacheIndex.RegisterAnimation(MyInfo.default.PayoutTextureNone);
    PrecacheIndex.RegisterAnimation(MyInfo.default.PayoutTextureBlank);
    i = 0;
    J0x89:
    ...
    if(i < 8)
    {
        PrecacheIndex.RegisterAnimationControllerEntry(MyInfo.default.SymbolTextures[i]);
        ++ i;
        // [Loop Continue]
        goto J0x89;
    }
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'CoinOp_InsertCoin');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'SlotMachine_PullLever');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'SlotMachine_Spin');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'SlotMachine_WheelStop');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'SlotMachine_Win');
    PrecacheIndex.InitAnimationControllerEx(class'VoicePack_Duke', 'BodyMotion_Short');
    PrecacheIndex.InitAnimationControllerEx(class'VoicePack_Duke', 'CoinFlip');
    PrecacheIndex.InitAnimationControllerEx(class'VoicePack_Duke', 'CoinNudge');
    PrecacheIndex.InitAnimationControllerEx(class'VoicePack_Duke', 'CoinRoll');
    PrecacheIndex.InitAnimationControllerEx(class'VoicePack_Duke', 'SlamMachine');
