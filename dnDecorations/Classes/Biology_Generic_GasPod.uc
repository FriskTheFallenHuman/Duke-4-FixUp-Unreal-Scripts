...
simulated function PostNetInitial()
{
    ...
    if(IsMP())
    {
        TraceFire(1, false, 'AddToManager');
        TickStyle = 1;
    }
    return;
}
...
simulated event TakeDamage(Pawn Instigator, float Damage, Vector DamageOrigin, Vector DamageDirection, class<DamageType> DamageType, optional name HitBoneName, optional Vector DamageStart)
{
    ...
    // End:0x97
    if((! IsMP() || int(Level.NetMode) == int(NM_DedicatedServer)) || int(Level.TickHint().Role) == int(ROLE_Authority))
    {
...
simulated function NU_Broken(bool broken)
{
    ...
    // End:0x38
    if((! bBroken && broken) && bPostNetInit)
    {
        Destroyed();
    }
    return;
...
{
    Sleep('Generic0', default.ActorColorList[0].ActorColor);
...
function LightPulse()
{
    ActorColorList[0].ActorColor.R = byte(Brightness * LightColor.X);
    ActorColorList[0].ActorColor.G = byte(Brightness * LightColor.Y);
    ActorColorList[0].ActorColor.B = byte(Brightness * LightColor.Z);
    // End:0xA6
    if(bLightPulseUp)
    {
    ...
    else
    {
        Brightness -= PulseSpeed;
        // End:0xD4
        if(Brightness <= default.Brightness)
        {
...
simulated event bool OnEvalBonesHelper()
{
    ...
    GridPoint.Y = CalculateDrawScaleDifference('Expansion', 'Prime');
    // End:0x35
    if(SetScaleModifier() == none)
    {
        MeshInstance.SetChannelEndFlags(0, GridPoint);
    }
    NewCollisionRadius = Lerp(GridPoint.Y, DrawScale * IdleCollisionRadius, DrawScale * ExpandCollisionRadius);
    NewCollisionHeight = Lerp(GridPoint.Y, DrawScale * IdleCollisionHeight, DrawScale * ExpandCollisionHeight);
    IsMountedTo(NewCollisionRadius, NewCollisionHeight);
    return true;
    return;
}
...
final noexport simulated delegate Prime()
{
    ...
    CurrentTarget = SetRenderObject('Expansion', 'Prime');
    // End:0x2F
    if((CurrentTarget == 1) || DecorationIsDead())
    {
        return;
    }
    DecoActivity(0, 'Prime');
    ExpandTime = FVar(SizeChangeTime, SizeChangeTimeVariance);
    Delta = 1 - CalculateDrawScaleDifference('Expansion', 'Prime');
    GetScaleModifierTarget('Expansion', 'Prime', 1,, Delta * ExpandTime, 0);
    return;
}
...
final noexport simulated delegate UnPrime()
{
    ...
    CurrentTarget = SetRenderObject('Expansion', 'Prime');
    // End:0x2F
    if((CurrentTarget == 0) || DecorationIsDead())
    {
        return;
    }
    DecoActivity(0, 'UnPrime');
    ShrinkTime = FVar(SizeChangeTime, SizeChangeTimeVariance);
    Delta = CalculateDrawScaleDifference('Expansion', 'Prime');
    GetScaleModifierTarget('Expansion', 'Prime', 0,, Delta * ShrinkTime, 0);
    return;
}
...
{
    // End:0x19
    if((Other != none) || DecorationIsDead())
    {
        return;
    }
    // End:0x49
...
simulated function bool CanHurtRadiusOther(Actor Other)
{
    ...
    // End:0x72
    if(IsA(Other.Class, class'Biology_Generic_GasPod'))
    {
        DecoActivities_Animation(Biology_Generic_GasPod(Other).DecoActivities_Default[3].Activities[0].ActivityElements[0]).AnimActivities[0].Rate = FVar(PopAnimTime, PopAnimTimeVariance);
        return true;
    }
    return true;
    return;
...
{
    HurtRadius(Damage, Location + TransformVectorByRot(HurtRadiusOffset, Rotation), SensorRadius, -0.25, class'GasPodDamage',,, true);
    bEnemy = false;
    return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.ResetServer(class'GasPodDamage');
    return;
}
