/*******************************************************************************
 * Settings_FoodService_Spatula_JumpPad generated by Eliot.UELib using UE Explorer.exe.
...
simulated event PostVerifySelf()
{
    ...
    BeginSenseEvent = CompositeNames(string(Name) $ "_Begin");
    GetPointRegion('BeginSenseObject', BeginSenseEvent);
    MySensor = KBoxSensor(FindMountedActor(, class'KBoxSensor'));
    // End:0xC0
    if(MySensor == none)
    {
        MySensor.BeginSenseEvent = BeginSenseEvent;
        MySensor.ReadText(Vect(10, 6, 5));
        // End:0xC0
        if(NameForString(PhysicsEntityGroup, 'None'))
        {
            MySensor.KSetJointsFrozenPercent(PhysicsEntityGroup);
        }
    }
    return;
}
...
simulated event Tick(float DeltaSeconds)
{
    ...
    // End:0x94
    if((MySensor == none) && TargetPoint == none)
    {
        SensedActors = MySensor.SendText();
        i = 0;
        J0x3F:

        // End:0x94 [Loop If]
        if(i < string(SensedActors))
        {
