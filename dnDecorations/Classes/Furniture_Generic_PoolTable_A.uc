...
{
    GetPointRegion('EightBall', SE_EightBall);
    GetPointRegion('NineBall', SE_NineBall);
    GetPointRegion('WinEgoCapIncrease', SE_WinEgoCapIncrease);
...
function PostVerifySelf()
{
    ...
    if(! bIncludeBalls)
    {
        ...
        // End:0x59 [Loop If]
        if(i < string(MountOnSpawn))
        {
            // End:0x4F
            if(IsA(MountOnSpawn[i].SpawnClass, class'Details_Balls_PoolBall'))
            {
...
function SetupBalls()
{
    ...
    if(bIncludeBalls)
    {
        ...
        if(string(PoolBalls) == 0)
        {
            ...
            if(i < 16)
            {
                PoolBalls[i] = Details_Balls_PoolBall(FindMountedActor(CompositeNames(string(i))));
                ++ i;
                // [Loop Continue]
                goto J0x25;
            }
        ...
        RackLine = Sqrt(Square(2 * BallSize) - Square(BallSize));
        RackTable_EightBall();
    }
    return;
}
...
function ShowComponents()
{
    ...
    if(i < 16)
    {
        PoolBalls[i].SetRotation(18);
        PoolBalls[i].SetDesiredRotation(PoolBallLocations[i]);
        ++ i;
        // [Loop Continue]
        goto J0x13;
    }
...
final function RackTable_EightBall()
{
    ...
    PoolBalls[8].ShowBall(self, RackPoint, 0, 2 * RackLine);
    RowOffset = 4 * RackLine;
    PoolBalls[Rand(7) + 1].ShowBall(self, RackPoint, 4 * BallSize, RowOffset);
    PoolBalls[Rand(7) + 9].ShowBall(self, RackPoint, 4 * - BallSize, RowOffset);
    PoolBalls[GetBall(15)].ShowBall(self, RackPoint, 0, 0);
    PoolBalls[GetBall(15)].ShowBall(self, RackPoint, BallSize, RackLine);
    PoolBalls[GetBall(15)].ShowBall(self, RackPoint, - BallSize, RackLine);
    RowOffset = 2 * RackLine;
    PoolBalls[GetBall(15)].ShowBall(self, RackPoint, 2 * BallSize, RowOffset);
    PoolBalls[GetBall(15)].ShowBall(self, RackPoint, 2 * - BallSize, RowOffset);
    RowOffset = 3 * RackLine;
    PoolBalls[GetBall(15)].ShowBall(self, RackPoint, 3 * BallSize, RowOffset);
...
final function RackTable_NineBall()
{
    ...
    PoolBalls[9].ShowBall(self, RackPoint, 0, 2 * RackLine);
    PoolBalls[GetBall(9)].ShowBall(self, RackPoint, BallSize, RackLine);
    PoolBalls[GetBall(9)].ShowBall(self, RackPoint, - BallSize, RackLine);
    RowOffset = 2 * RackLine;
    PoolBalls[GetBall(9)].ShowBall(self, RackPoint, 2 * BallSize, RowOffset);
    PoolBalls[GetBall(9)].ShowBall(self, RackPoint, 2 * - BallSize, RowOffset);
    RowOffset = 3 * RackLine;
    PoolBalls[GetBall(9)].ShowBall(self, RackPoint, BallSize, RowOffset);
    PoolBalls[GetBall(9)].ShowBall(self, RackPoint, - BallSize, RowOffset);
    PoolBalls[GetBall(9)].ShowBall(self, RackPoint, 0, 4 * RackLine);
    return;
}
...
final function TriggerFunc_WinEgoCapIncrease()
{
    ...
    Duke = DukePlayer(Level.TickHint());
    // End:0x36
    if(Duke == none)
    {
