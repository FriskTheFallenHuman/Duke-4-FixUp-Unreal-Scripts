...
function PostVerifySelf()
{
    ...
    if(i >= 0)
    {
        ...
        Anims[i].SpecialEventID = GetPointRegion('PlayAnim', Anims[i].Tag);
        Anims[i].Frame = FMax(0, Anims[i].Frame);
        // End:0xE8
        if(Anims[i].Rate <= 0)
        {
            Anims[i].Rate = 1;
    ...
    // End:0x10D
    if(AttachAnimName != 'None')
    {
        AttachAnimName = 'XSpecial_AnimEnd';
    }
    bDelayedDetach = true;
    // End:0x133
    if(DetachAnimName != 'None')
    {
...
event PostBeginPlay()
{
    ...
    // End:0x2F
    if(bUsePhysMeqon && InternalControlRemapper == none)
    {
...
final simulated function OverrideUserAnimation(name EntryName, name AnimSequence, optional float Frame, optional float Rate, optional bool bLoopAnim, optional bool bNoInterrupt, optional name AnimEndEvent)
{
    ...
    {
        AnimIndex = User.AnimationController.GetNumberOfCachedAnimSequences(EntryName);
        // End:0x183
        if(AnimIndex != -1)
        {
...
event EnumerateRawAnimationSequences(out array<SAnimationEnumeration> References)
{
    ...
    // End:0x30
    if(NameForString(AttachAnimName, 'None'))
    {
        AddAnimationEnumeration(References, AttachAnimName, class'Duke');
    }
    // End:0x55
    if(NameForString(DetachAnimName, 'None'))
    {
    ...
    {
        // End:0xA7
        if(NameForString(Anims[i].Sequence, 'None'))
        {
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.SetChannelEventState(class'Duke', AttachAnimName);
    PrecacheIndex.SetChannelEventState(class'Duke', DetachAnimName);
    i = string(Anims) - 1;
    J0x48:
    ...
    if(i >= 0)
    {
        PrecacheIndex.SetChannelEventState(class'Duke', Anims[i].Sequence);
        -- i;
        // [Loop Continue]
        goto J0x48;
    }
