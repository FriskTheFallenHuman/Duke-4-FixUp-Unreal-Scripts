...
function PostBeginPlay()
{
    ...
    // End:0x1F6 [Loop If]
    if(i < string(DrawingTools))
    {
        DrawingTools[i].bGrabbable = false;
        DrawingTools[i].bCanExistOutOfWorld = true;
        DrawingTools[i].ForceMountUpdate(false, false, false, false, false);
        ...
        // End:0x1D4
        if((i < string(DrawingToolMaterials)) && DrawingToolMaterials[i] == none)
        {
            DrawingTools[i].VisibleActors(0, DrawingToolMaterials[i]);
        }
    ...
    XDrawOffset = float(BaseXOffset) + (XOffsetScalar * (CurrentHorizontalVal / MaxHorizontal));
    return;
}
...
simulated event PostUserAnimTick()
{
    ...
    {
        // End:0x108
        if((User.SetScaleModifier() == none) && (IsXbox() != 'DrawableSurfaceIdle') || IsXbox() != 'Drawing')
        {
            BoneAlpha += (4 * Level.TimeDeltaSeconds);
            // End:0x86
            if(BoneAlpha > 1)
            {
                BoneAlpha = 1;
            ...
            if(IsXbox() != 'Drawing')
            {
                BoneXAlpha += (8 * Level.TimeDeltaSeconds);
                // End:0xCB
                if(BoneXAlpha > 1)
                {
                    BoneXAlpha = 1;
    ...
    if(BoneAlpha > 0)
    {
        BonePos = User.MeshInstance.CreateAnimGroup('mount_holster_hip_right', true);
        BoneRot = User.MeshInstance.SwapChannel('mount_holster_hip_right', true);
        DesiredBonePos.X = Lerp(BoneXAlpha, DrawingTools[CurrentColor].BoneXRange.Min, DrawingTools[CurrentColor].BoneXRange.Max);
        DesiredBonePos.Y = Lerp(0.5 * (MouseInputX + 1), DrawingTools[CurrentColor].BoneYRange.Min, DrawingTools[CurrentColor].BoneYRange.Max);
        DesiredBonePos.Z = Lerp(0.5 * (MouseInputY + 1), DrawingTools[CurrentColor].BoneZRange.Min, DrawingTools[CurrentColor].BoneZRange.Max);
        DesiredBonePos *= User.DrawScale;
        DesiredBonePos = User.Location + TransformVectorByRot(DesiredBonePos, User.Rotation);
        DesiredBoneRot = DrawingTools[CurrentColor].BoneRot >> User.Rotation;
        BonePos.X = Smerp(BoneAlpha, BonePos.X, DesiredBonePos.X);
        BonePos.Y = Smerp(BoneAlpha, BonePos.Y, DesiredBonePos.Y);
        BonePos.Z = Smerp(BoneAlpha, BonePos.Z, DesiredBonePos.Z);
        User.MeshInstance.FreezeAnimations('mount_holster_hip_right', BonePos, true);
        User.MeshInstance.AnimUpdateRate('mount_holster_hip_right', Slerp(BoneAlpha, BoneRot, DesiredBoneRot), true);
    }
    return;
}
...
function ChangeTool()
{
    ...
    // End:0x4A
    if((CurrentColor >= MaxIndex) || CurrentColor < 0)
    {
        CurrentColor = 0;
    }
...
function InputHook(out float aForward, out float aLookUp, out float aTurn, out float aStrafe, optional float DeltaTime)
{
    ...
    {
        // End:0x88
        if((MouseInputX >= 0.95) && ThisXAdjust > 0)
        {
            newStrafe = (MouseInputX - 0.95) * 80000;            
        }
        else
        {
            // End:0xC1
    ...
    {
        // End:0x10C
        if((MouseInputY >= 0.95) && ThisYAdjust > 0)
        {
            newForward = (MouseInputY - 0.95) * 45000;            
        }
        else
        {
            // End:0x145
...
function MoveUser(float aForward, float aStrafe, float DeltaTime)
{
    ...
    HorizontalIncremental = (DeltaTime * HorizontalRate) * aStrafe;
    CurrentHorizontalVal += HorizontalIncremental;
    // End:0x55
    if(CurrentHorizontalVal > MaxHorizontal)
    {
        HorizontalIncremental -= (CurrentHorizontalVal - MaxHorizontal);
        CurrentHorizontalVal = MaxHorizontal;        
    }
    else
    {
        // End:0x78
        if(CurrentHorizontalVal < float(0))
        {
            HorizontalIncremental -= CurrentHorizontalVal;
            CurrentHorizontalVal = 0;
        }
    }
    vRight = Vector(User.Rotation + Rot(0, 16384, 0));
    InternalControlRemapper.DisableFixedRotationRate(vRight * HorizontalIncremental);
    XDrawOffset = float(BaseXOffset) + (XOffsetScalar * (CurrentHorizontalVal / MaxHorizontal));
    return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.RegisterAnimation(MyCopySource);
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Whiteboard_Eraser_Touch');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Whiteboard_Eraser_Move');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Whiteboard_Marker_Touch');
    PrecacheIndex.InitAnimationControllerEx(VoicePack, 'Whiteboard_Marker_Move');
    i = string(DrawingToolMaterials) - 1;
    J0x88:

    // End:0xB5 [Loop If]
    if(i >= 0)
    {
        PrecacheIndex.RegisterAnimationControllerEntry(DrawingToolMaterials[i]);
        -- i;
        // [Loop Continue]
        goto J0x88;
    }
...
state() PutDownTool extends DrawableSurfaceBaseState
{
    ...
    function EndState()
    {
        DrawingTools[CurrentColor].GetGravity();
        DrawingTools[CurrentColor].SetPhysics(DrawingTools[CurrentColor].OldMount, self);
        // End:0x76
        if(((CurrentColor == 5) && bQuickEraser) && NameForString(IsXbox(), 'PutDownTool_QuickEraser'))
        {
...
state() BringUpTool extends DrawableSurfaceBaseState
{
    function BeginState()
    {
        ...
        States[CurrentStateIndex].UserAnimName = AimGridName;
        DrawingTools[CurrentColor].GetGravity();
        DrawingTools[CurrentColor].SetPhysics(DrawingTools[CurrentColor].DrawingMountInfo, User);
        super(Object).BeginState();
        return;
    }
...
state() PreDetach
{
    ...
    event Tick(float DeltaTime)
    {
        ...
        Alpha = (Level.GameTimeSeconds - PreDetachStartTime) / PreDetachDuration;
        // End:0x62
        if(Alpha > 1)
        {
            Alpha = 1;
        }
        MouseInputX = Smerp(Alpha, PreDetachInputX, 0);
        MouseInputY = Smerp(Alpha, PreDetachInputY, 0);
        User.AnimationController.SetAnimGridState(AimGridName, MouseInputX, MouseInputY);
        LastHoriz = CurrentHorizontalVal;
        CurrentHorizontalVal = Smerp(Alpha, PreDetachHoriz, default.CurrentHorizontalVal);
        HorizDist = CurrentHorizontalVal - LastHoriz;
        vRight = Vector(User.Rotation + Rot(0, 16384, 0));
        User.DisableFixedRotationRate(vRight * HorizDist);
        // End:0x143
        if(Alpha == 1)
        {
            ControlEvent(0, 'PreDetachComplete');
...
state Drawing
{
    event Tick(float DeltaTime)
    {
        ...
        {
            // End:0x80
            if((! bAchievementAlreadyUnlocked && CurrentColor != 5) && DukePlayer(User) == none)
            {
            ...
            {
                // End:0x173
                if(DrawingTool.AmbientSound != none)
                {
                    // End:0x160
                    if(CurrentColor == 5)
                    {
                ...
                Helper = Abs(float(PreviousXDrawPos - XDrawPos)) + Abs(float(PreviousYDrawPos - YDrawPos));
                NewPitch = int(float(48) + (float(8) * Helper));
                // End:0x1C6
                if(NewPitch < 48)
                {
                    NewPitch = 48;                    
            ...
            {
                // End:0x24E
                if((DrawingTool.AmbientSound == none) && Level.GameTimeSeconds >= AmbientEndTime)
                {
                    DrawingTool.SetAnimFrame(none);
                }
            }
        }
