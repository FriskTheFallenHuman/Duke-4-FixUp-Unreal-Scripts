...
event PostBeginPlay()
{
    ...
    {
        // End:0x44
        if(StationTriggers[i].Tag != 'None')
        {
            StationTriggers.Remove(i, 1);
            // [Explicit Continue]
            goto J0x6D;
        }
        StationTriggers[i].SpecialEventID = GetPointRegion('ChangeChannel', StationTriggers[i].Tag);
        J0x6D:
...
{
    // End:0x2D
    if((Radio == none) && Radio.CurrFreq == 0)
    {
        ChangeChannel();
    }
    super.ForcePowerOn();
