...
{
    VisibleActors(0, BallMaterials[MaterialIndex]);
    return;
}
...
final function ShowBall(Actor Table, Vector Position, float YOffset, float xOffset)
{
    InitBall();
    MountType = 0;
    MountOrigin = Vect(Position.X + xOffset, Position.Y + YOffset, Position.Z);
    MountAngles = Rot(Rand(65536), Rand(65536), 0);
    MoveActor(Table);
    GetGravity();
    DecoActivity(0, 'ShowBall');
    KGetInteractingActors();
    KGetCollidingActors();
    TickStyle = 3;
    Destroy(1, false, 'NormalTicking');
    return;
}
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    if(i < 16)
    {
        PrecacheIndex.RegisterAnimationControllerEntry(BallMaterials[i]);
        ++ i;
        // [Loop Continue]
        goto J0x12;
    }
