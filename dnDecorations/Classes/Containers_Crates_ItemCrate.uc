/*******************************************************************************
 * Containers_Crates_ItemCrate generated by Eliot.UELib using UE Explorer.exe.
 * Eliot.UELib ? 2009-2022 Eliot van Uytfanghe. All rights reserved.
 * http://eliotvu.com
 *
 * All rights belong to their respective owners.
 *******************************************************************************/
class Containers_Crates_ItemCrate extends Containers_Crates
    collapsecategories;

var float SpeakRadius;

simulated event TakeDamage(Pawn Instigator, float Damage, Vector DamageOrigin, Vector DamageDirection, class<DamageType> DamageType, optional name HitBoneName, optional Vector DamageStart)
{
    local bool bWasAlive;

    bWasAlive = ! bCriticalDamage;
    super(dnDecoration).TakeDamage(Instigator, Damage, DamageOrigin, DamageDirection, DamageType, HitBoneName, DamageStart);
    // End:0xB1
    if(bWasAlive && bCriticalDamage)
    {
        // End:0xB1
        if((! IsMP() && Instigator == none) && Instigator.bIsPlayerPawn)
        {
            // End:0xB1
            if(VSize(Location - Instigator.Location) <= SpeakRadius)
            {
                Instigator.FindSoundAndSpeak('GotAmmo');
            }
        }
    }
    return;
}

event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    super(dnDecoration).RegisterPrecacheComponents(PrecacheIndex);
    PrecacheIndex.InitAnimationControllerEx(class'DukePlayer'.default.VoicePack, 'GotAmmo');
    return;
}

defaultproperties
{
    SpeakRadius=200
    DestroyedActivities(0)=none
    begin object name=DA_Sound_ItemCrate_Destroyed class=DecoActivities_Sound
        SoundNames(0)=Destruct_Wood
    object end
    // Reference: DecoActivities_Sound'Containers_Crates_ItemCrate.DA_Sound_ItemCrate_Destroyed'
    DestroyedActivities(1)=DA_Sound_ItemCrate_Destroyed
    HealthPrefab=5
    begin object name=MP_ItemCrate_BottomGib class=MotionPrefab
        bUseDamageInfo=false
        bUseParentVelocity=false
        bUseParentRotationRate=false
    object end
    // Reference: MotionPrefab'Containers_Crates_ItemCrate.MP_ItemCrate_BottomGib'
    SpawnedPickupMotionPrefab=MP_ItemCrate_BottomGib
    Health=1
    HealthCap=1
    SpawnOnDestroyed(0)=(SpawnClass='Containers_Crates_ItemCrate_Gib_Wood_Top_F',RenderObject=none,DrawScale=0,DrawScaleVariance=0,DrawScale3D=(X=0,Y=0,Z=0),SpawnChance=0,SpawnCopies=0,SpawnCopiesVariance=0,bIgnorePawnAirCushion=true,bDontScaleByDrawScale=false,bScaleByDrawScaleNonDefault=false,bNoCollision=false,bFindSpot=false,bIgnoreParentRotation=false,bTakeParentCollisionSize=false,bTakeParentMounting=false,bTakeParentActorColors=false,bTakeParentSkins=false,Offset=(X=-11.012,Y=-10.014,Z=11.3),OffsetVariance=(X=0,Y=0,Z=0),Rotation=(Pitch=0,Yaw=0,Roll=0),RotationVariance=(Pitch=0,Yaw=0,Roll=0),BoneName=None,MotionInfo=MotionPrefab'Containers_Crates_ItemCrate.MP_ItemCrate_Gibs')
    SpawnOnDestroyed(1)=(SpawnClass='Containers_Crates_ItemCrate_Gib_Wood_Left',RenderObject=none,DrawScale=0,DrawScaleVariance=0,DrawScale3D=(X=0,Y=0,Z=0),SpawnChance=0,SpawnCopies=0,SpawnCopiesVariance=0,bIgnorePawnAirCushion=true,bDontScaleByDrawScale=false,bScaleByDrawScaleNonDefault=false,bNoCollision=false,bFindSpot=false,bIgnoreParentRotation=false,bTakeParentCollisionSize=false,bTakeParentMounting=false,bTakeParentActorColors=false,bTakeParentSkins=false,Offset=(X=0.012,Y=-15.512,Z=2.045),OffsetVariance=(X=0,Y=0,Z=0),Rotation=(Pitch=0,Yaw=0,Roll=0),RotationVariance=(Pitch=0,Yaw=0,Roll=0),BoneName=None,MotionInfo=MotionPrefab'Containers_Crates_ItemCrate.MP_ItemCrate_Gibs')
    SpawnOnDestroyed(2)=(SpawnClass='Containers_Crates_ItemCrate_Gib_Wood_Right',RenderObject=none,DrawScale=0,DrawScaleVariance=0,DrawScale3D=(X=0,Y=0,Z=0),SpawnChance=0,SpawnCopies=0,SpawnCopiesVariance=0,bIgnorePawnAirCushion=true,bDontScaleByDrawScale=false,bScaleByDrawScaleNonDefault=false,bNoCollision=false,bFindSpot=false,bIgnoreParentRotation=false,bTakeParentCollisionSize=false,bTakeParentMounting=false,bTakeParentActorColors=false,bTakeParentSkins=false,Offset=(X=7.005,Y=14.971,Z=4.038),OffsetVariance=(X=0,Y=0,Z=0),Rotation=(Pitch=0,Yaw=0,Roll=0),RotationVariance=(Pitch=0,Yaw=0,Roll=0),BoneName=None,MotionInfo=MotionPrefab'Containers_Crates_ItemCrate.MP_ItemCrate_Gibs')
    SpawnOnDestroyed(3)=(SpawnClass='Containers_Crates_ItemCrate_Gib_Wood_Front',RenderObject=none,DrawScale=0,DrawScaleVariance=0,DrawScale3D=(X=0,Y=0,Z=0),SpawnChance=0,SpawnCopies=0,SpawnCopiesVariance=0,bIgnorePawnAirCushion=true,bDontScaleByDrawScale=false,bScaleByDrawScaleNonDefault=false,bNoCollision=false,bFindSpot=false,bIgnoreParentRotation=false,bTakeParentCollisionSize=false,bTakeParentMounting=false,bTakeParentActorColors=false,bTakeParentSkins=false,Offset=(X=11.5,Y=1.987,Z=4.042),OffsetVariance=(X=0,Y=0,Z=0),Rotation=(Pitch=0,Yaw=0,Roll=0),RotationVariance=(Pitch=0,Yaw=0,Roll=0),BoneName=None,MotionInfo=MotionPrefab'Containers_Crates_ItemCrate.MP_ItemCrate_Gibs')
    SpawnOnDestroyed(4)=(SpawnClass='Containers_Crates_ItemCrate_Gib_Wood_Back',RenderObject=none,DrawScale=0,DrawScaleVariance=0,DrawScale3D=(X=0,Y=0,Z=0),SpawnChance=0,SpawnCopies=0,SpawnCopiesVariance=0,bIgnorePawnAirCushion=true,bDontScaleByDrawScale=false,bScaleByDrawScaleNonDefault=false,bNoCollision=false,bFindSpot=false,bIgnoreParentRotation=false,bTakeParentCollisionSize=false,bTakeParentMounting=false,bTakeParentActorColors=false,bTakeParentSkins=false,Offset=(X=-11.494,Y=5.992,Z=4.045),OffsetVariance=(X=0,Y=0,Z=0),Rotation=(Pitch=0,Yaw=0,Roll=0),RotationVariance=(Pitch=0,Yaw=0,Roll=0),BoneName=None,MotionInfo=MotionPrefab'Containers_Crates_ItemCrate.MP_ItemCrate_Gibs')
    bTickOnlyWhenPhysicsAwake=true
    PhysicsMaterial='dnMaterial.dnPhysicsMaterial_Wood_Solid'
    PhysicsSoundOverrides(0)=(SoundType=0,OtherMaterialTypes=none,OtherMassTypes=none,Sounds=('a_impact.KarmaWood.WoodHollowMed01','a_impact.KarmaWood.WoodHollowMed02','a_impact.KarmaWood.WoodHollowMed03','a_impact.KarmaWood.WoodHollowMed04'),SoundInfo=(InputRange=(Min=32,Max=512),OutputPitchRange=(Min=0.8,Max=1),OutputVolumeRange=(Min=0.5,Max=1)),bDisableSoundInWater=false)
    PhysicsSoundOverrides(1)=(SoundType=2,OtherMaterialTypes=none,OtherMassTypes=none,Sounds=('a_impact.KarmaWood.WoodSolid_SlideConcrete01'),SoundInfo=(InputRange=(Min=4,Max=500),OutputPitchRange=(Min=0.8,Max=1),OutputVolumeRange=(Min=0.25,Max=1)),bDisableSoundInWater=false)
    PhysicsSoundOverrides(2)=(SoundType=1,OtherMaterialTypes=none,OtherMassTypes=none,Sounds=none,SoundInfo=(InputRange=(Min=1,Max=5),OutputPitchRange=(Min=0,Max=0),OutputVolumeRange=(Min=0,Max=0)),bDisableSoundInWater=false)
    Physics=18
    CollisionRadius=16
    CollisionHeight=12
    StaticMesh='sm_class_decorations.Crate.ammoCrate'
}