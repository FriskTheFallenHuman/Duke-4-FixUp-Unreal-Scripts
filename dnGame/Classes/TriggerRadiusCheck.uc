...
function PostBeginPlay()
{
    ...
    if(bEnabled)
    {
        Destroy(0.0001, false, 'EnableTicking');        
    }
    else
    {
        Destroy(0.0001, false, 'DisableTicking');
    }
    return;
}
...
function Tick(float DeltaTime)
{
    ...
    // End:0x2F
    if((TestActors[0] != none) || TestActors[1] != none)
    {
        EnableTicking();
    }
    // End:0x55
    ...
    InternalFrameCounter += DeltaTime;
    // End:0x133
    if(InternalFrameCounter > FrameCheck)
    {
        // End:0x94
        if(FrameCheck != 0)
        {
            InternalFrameCounter = InternalFrameCounter % FrameCheck;            
        }
        else
        {
            InternalFrameCounter = 0;
        }
        CurrentRadiusSquared = VSizeSquared(TestActors[0].Location - TestActors[1].Location);
        // End:0xFB
        if((CurrentRadiusSquared >= RadiusSquared) && PreviousRadiusSquared < RadiusSquared)
        {
...
final function EnableTicking()
{
    ...
    if((TestActors[0] != none) || TestActors[1] != none)
    {
        ...
        if(bDebug)
        {
            BroadcastLog(((((((((string(self) $ ".SetupInitialState(): couldn't find actors for tags '") $ string(ActorTags[0])) $ "' (") $ string(TestActors[0])) $ ") and '") $ string(ActorTags[1])) $ "' (") $ string(TestActors[1])) $ ").");
        }        
    }
    else
    {
