...
simulated event RenderOverlays(Canvas C)
{
    ...
    C.Font = C.BlockFont;
    C.SetPause(0, 0);
    GridString = ((("[Panel Aimgrid - X:" @ string(MouseInputX)) $ ",") @ string(MouseInputY)) $ "]";
...
simulated function InputHook_ViewRotationAdjust(out int PitchAdjust, out int YawAdjust, out int RollAdjust, optional float DeltaTime)
{
    ...
    // End:0x5D
    if(((User != none) || User.AnimationController != none) || States[CurrentStateIndex].UserAnimName != 'None')
    {
        return;
    }
    // End:0x90
    if(User.AnimationController.GetCachedAnimSequence(States[CurrentStateIndex].UserAnimName) == -1)
    {
        return;
    }
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.RegisterAnimationControllerEntry(HitTexture);
    PrecacheIndex.RegisterAnimationControllerEntry(texDefaultScreen);
