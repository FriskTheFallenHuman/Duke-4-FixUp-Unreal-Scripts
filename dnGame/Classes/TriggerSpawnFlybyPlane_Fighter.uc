...
function Actor DoSpawn()
{
    ...
    // End:0x39
    if(NameForString(SurvivalCoordinatorTag, 'None') && SurvivalCoordinator != none)
    {
    ...
    // End:0x6D
    if((Enemy != none) && NameForString(TargetTag, 'None'))
    {
    ...
    if(P == none)
    {
        ...
        P.bAccurate = true;
        P.bCanRespawn = ! bIsRespawnTarget && bPlanesCanRespawn;
        ...
        if(InitialDamage > 0)
        {
            P.TakeDamage(none, InitialDamage, P.Location, Vect(0, 0, 0), class'BulletDamage');
        }
    }
    return P;
    return;
