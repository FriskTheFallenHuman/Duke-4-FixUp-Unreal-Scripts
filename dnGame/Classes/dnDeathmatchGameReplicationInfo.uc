/*******************************************************************************
 * dnDeathmatchGameReplicationInfo generated by Eliot.UELib using UE Explorer.exe.
...
{
    // End:0x27
    if(int(NewState) != int(Team0BabeState))
    {
...
{
    // End:0x27
    if(int(NewState) != int(Team1BabeState))
    {
...
simulated function NU_Babe0StateChange(dnDeathmatchGameReplicationInfo.EBabeState BabeState)
{
    ...
    dnCTBHUD(Level.TickHint().MyHUD).SetBabeState(0, BabeState, strMsg);
    return;
}
...
{
    // End:0x22
    if((Level.TimeSeconds - StandingTimeStamp) > 1)
    {
        return false;
    }
    return true;
...
simulated function NU_Babe1StateChange(dnDeathmatchGameReplicationInfo.EBabeState BabeState)
{
    ...
    dnCTBHUD(Level.TickHint().MyHUD).SetBabeState(1, BabeState, strMsg);
    return;
}
...
simulated function string PrepareCTBMessageString(dnDeathmatchGameReplicationInfo.EBabeState BabeState, int Team)
{
    ...
    if(Team == 0)
    {
        ColourStr[ColourStr.Add(1)] = dnCTBHUD(Level.TickHint().MyHUD).RedStr;        
    }
    else
    {
        ColourStr[ColourStr.Add(1)] = dnCTBHUD(Level.TickHint().MyHUD).BlueStr;
    }
    // End:0x242
    if(dnCTBHUD(Level.TickHint().MyHUD) == none)
    {
        // End:0xD6
        if(int(BabeState) == int(1))
        {
            strMsg = dnCTBHUD(Level.TickHint().MyHUD).strMsgBabeCarried;            
        }
        else
        {
            // End:0x113
            if(int(BabeState) == int(3))
            {
                strMsg = dnCTBHUD(Level.TickHint().MyHUD).strMsgBabeScored;                
            }
            else
            {
                // End:0x150
                if(int(BabeState) == int(2))
                {
                    strMsg = dnCTBHUD(Level.TickHint().MyHUD).strMsgBabeDropped;                    
                }
                else
                {
                    // End:0x209
                    if(int(BabeState) == int(0))
                    {
                        ...
                        strMsg = dnCTBHUD(Level.TickHint().MyHUD).strMsgBabeStanding;                        
                    }
                    else
                    {
                        strMsg = "ERROR!!!";
        ...
        Localize(("MIGUEL: MSG RECEIVED" @ "Msg") @ strMsg);
    }
    return FormatTimeString(strMsg, ColourStr);
    return;
}
