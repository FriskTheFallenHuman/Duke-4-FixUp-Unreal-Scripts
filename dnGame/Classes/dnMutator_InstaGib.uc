...
function bool CheckReplacement(Actor Other, out byte bSuperRelevant)
{
    ...
    if(bResult)
    {
        ...
        // End:0x54
        if(Other.ClassForName('WeaponPickup'))
        {
            bResult = false;
        }
    }
...
{
    // End:0xBC
    if(DamageType != class'RailgunDamage')
    {
        // End:0x95
        if(((Victim == InstigatedBy) && InstigatedBy == none) && ! Level.GRI.bTeamGame && int(Victim.PlayerReplicationInfo.Team) == int(InstigatedBy.PlayerReplicationInfo.Team))
        {
            ActualDamage = 99999;
        }
