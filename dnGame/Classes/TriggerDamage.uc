...
final function TriggerDamageActor(Actor HurtActor, Pawn EventInstigator)
{
    ...
    if(HurtActor.bIsPawn)
    {
        ...
        // End:0x76
        if((HurtControl == none) && HurtControl.DamageTrigger == none)
        {
    ...
    HurtActor.TakeDamage(Instigator, FVar(DamageAmount, DamageVariance), HurtActor.Location, Vect(0, 0, 1), DamageTypeClass,, Location);
    // End:0x146
    if(DontSpawnDestroyTrash && RA == none)
    {
...
{
    PrecacheIndex.ResetServer(DamageTypeClass);
    return;
}
