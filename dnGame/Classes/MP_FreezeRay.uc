...
final simulated function DisableFreezeBeam()
{
    ...
    TraceFire(RechargeFrequency, true, 'RechargeFreezeAmmo');
    return;
}
...
simulated event Tick(float DeltaTime)
{
    ...
    if((int(WeaponState) == int(4)) || int(WeaponState) == int(5))
    {
        ...
        // End:0xA5
        if(! bFreezeBeamActive && ! Instigator.IsFrozen())
        {
            EnableFreezeBeam();
            bFreezeBeamActive = true;
        }        
    ...
    if(bFreezeBeamActive)
    {
        ...
        if(HitActor == none)
        {
            ...
            if(int(WeaponState) == int(4))
            {
                ...
                HitActor.TakeDamage(Instigator, (DeltaTime * DamagePerSecond) * Instigator.DrawScale, TraceHitResult.Location, Vector(FreezeBeamDir), class'ColdDamage', 'None', Start);
...
simulated event ScriptGetActorColor()
{
    ...
    if(int(Instigator.PlayerReplicationInfo.Team) == 0)
    {
        ...
        SirenLightEx.LightColor.B = byte(CalculateDrawScaleDifference('SirenPulse', 'SirenPulse'));
        SirenLightEx.LightColor.G = byte(float(SirenLightEx.LightColor.B) * 0.11764);
        Sleep('Generic1', NewColorBytes(0, 30, 255));        
    }
    else
    {
...
simulated function OnDestroyed()
{
    ...
    if(FreezeImpactEffect == none)
    {
        FreezeImpactEffect.RemoveTouchClass();
    }
    return;
}
