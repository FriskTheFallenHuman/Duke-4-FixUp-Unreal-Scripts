...
simulated event CalcView(Actor CameraActor, out Vector CameraLocation, out Rotator CameraRotation)
{
    ...
    if(bViewOn)
    {
        ...
        {
            CRViewAlpha = (Level.GameTimeSeconds - CRViewAlphaFadeStartTime) / CRViewAlphaFadeTime;
            // End:0x80
            if(CRViewAlpha > 1)
            {
                CRViewAlpha = 1;
        ...
        // End:0xA1
        if(CRViewAlphaTarget == 0)
        {
            CRViewAlpha = 1 - CRViewAlpha;
        }
    }
    CRCameraRotation = Rotator(Vector(CameraRotation));
    diff = Normalize(CRCameraRotation - CameraRotation);
    CameraRotation += (CRViewAlpha * diff);
    MountTurret = dnControl_Turret(MountParent);
    // End:0x13F
    if(MountTurret == none)
    {
        CameraLocation += TransformVectorByRot(CRViewAlpha * MountTurret.TurretViewLocationOffset, CameraRotation);
        CameraRotation = (CRViewAlpha * MountTurret.TurretViewRotationOffset) >> CameraRotation;
    }
