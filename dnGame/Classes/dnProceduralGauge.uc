...
event PostBeginPlay()
{
    ...
    bStartup = true;
    StepAngle = (EndAngle - StartAngle) / MaxValue;
...
event Tick(float DeltaTime)
{
    ...
    {
        bStartup = false;
        AngleDelta = float(DesiredAngle - Angle);        
    }
    else
    {
        diff = float(DesiredAngle - Angle);
        AngleDelta = diff * SpringStrength;
        // End:0x79
        if(TimeScaleStrength != 0)
        {
            AngleDelta *= (TimeScaleStrength * DeltaTime);
        }
        AngleDelta = FClamp(AngleDelta, - MaxAngleDelta, MaxAngleDelta);
    }
    return;
}
...
{
    SetScaleModifier().AnimUpdateRate(NeedleBoneName, Rot(0, 0, int(- AngleDelta)), false, true);
...
{
    GaugeState = Clamp(NewGaugeState, 0, MaxValue);
    DesiredAngle = StartAngle + (GaugeState * StepAngle);
    return;
}
