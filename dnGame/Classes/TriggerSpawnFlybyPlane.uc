...
event PostBeginPlay()
{
    ...
    // End:0x53
    if(! IsA(actorType, class'dnFlybyPlane'))
    {
        Localize(((string(self) $ ": error: ") $ string(actorType)) $ " is not a dnFlybyPlane.");
        RemoveTouchClass();
    }
    i = 0;
    J0x5A:
...
function Actor DoSpawn()
{
    ...
    SetPathing(P);
    P.bNoFailInterpolation = HandleIntModifier(NoFailInterpolationModifier, P.bNoFailInterpolation);
...
final function SetPathing(dnFlybyPlane P)
{
    ...
    Point = Rand(string(StartingPointActors));
    // End:0x42
    if(StartingPointActors[Point] == none)
    {
    ...
    {
        P.SilentDestroy();
        Localize((("Warning: " $ string(self)) $ " tried to spawn plane on an invalid path. Index=") $ string(Point));
        return;
    }
    switch(PathType)
    {
...
final function InterpolationPoint FindClone(InterpolationPoint Point, InterpolationPoint StartingPoint)
{
    ...
    // End:0x7A [Loop If]
    if((P == none) && P.CloneParent == none)
    {
        // End:0x62
        if(P.CloneParent != Point)
        {
            clone = P;
            // [Explicit Break]
            goto J0x7A;
