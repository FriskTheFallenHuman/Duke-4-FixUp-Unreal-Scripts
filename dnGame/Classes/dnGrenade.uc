...
simulated function PostVerifySelf()
{
    ...
    // End:0x28
    if((ExplosionDelay > 0) && ! bPrimeForExplosionOnImpact)
    {
...
simulated function PrimeForDelayedExplosion()
{
    ...
    Destroy(FVar(ExplosionDelay, ExplosionDelayVariance), false, 'NonImpactExplode');
    return;
}
...
simulated function InitializeMotion()
{
    ...
    UpwardsBoost = TransformVectorByRot(UpwardsBoost, Rotator(PhysicsVolume.Gravity) - Rotator(Vect(0, 0, -1)));
    ThrowImpulse = UpwardsBoost + (Speed * Vector(Rotation));
...
final simulated function DoThrow(Vector ThrowVelocity)
{
    ...
    SetRotation(18);
    KPushGeneratedSimpleState(ThrowVelocity);
    // End:0x83
    if(bSpin)
    {
        SpinRate = VSize(ThrowVelocity * DrawScalar) * 0.01;
        SetPhysicsPose(TransformVectorByRot(Vect(0, SpinRate, 0), Rotation));
    }
    return;
}
...
simulated function ProjectileHitSomething(Vector HitNormal, Actor HitActor)
{
    ...
    // End:0x54
    if((bPrimeForExplosionOnImpact && ! bPrimedForExplosion) && (Instigator != none) || HitActor == Instigator)
    {
        PrimeForDelayedExplosion();
        return;
    }
...
{
    ExecuteExplode(Location, - Normal(PhysicsVolume.Gravity));
    return;
}
