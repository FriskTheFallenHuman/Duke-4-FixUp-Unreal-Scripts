...
final function DamageMyEnemy(Vector startLocation, Vector endLocation, bool bFakedShotValid)
{
    ...
    {
        // End:0x6C
        if((Enemy == none) && bFakedShotValid)
        {
            Enemy.TakeDamage(none, GetBaseDamage(Enemy), Location, Enemy.Location - startLocation, TraceDamageType,, Location);
        }        
    }
    else
    {
        ...
        {
            ShouldTouch(startLocation, startLocation + (Normal(endLocation - startLocation) * ShootingInfo.ShotLength), TraceDamageType);
        }
    }
    return;
}
...
{
    return VVar(InitialEnd, ShootingInfo.ShotTargetVariance);
    return;
}
...
{
    return FVar(ShootingInfo.ShotDamage, ShootingInfo.ShotDamageVariance);
    return;
}
