...
final function AddUserPower()
{
    ...
    Alpha = GetPCTTap();
    MinTapInterval = FMax(2 * Level.TimeDeltaSeconds, Lerp(Alpha, StartTapInterval, EndTapInterval) * Level.TimeDilation);
    // End:0xD1
    if(PlayerUser.bIJumpPressed)
    {
        PlayerUser.bIJumpPressed = false;
        TimeSinceLastTap = Level.GameTimeSeconds - LastTapTime;
        // End:0xBC
        if((TimeSinceLastTap <= MinTapInterval) || Alpha == 0)
        {
            bSuccessfulTap = true;
            UserStraining(false);
        }
...
event RegisterPrecacheComponents(PrecacheIndex PrecacheIndex)
{
    ...
    PrecacheIndex.InitAnimationControllerEx(class'VoicePack_Duke', UserGruntSoundName);
    return;
}
...
state() idle
{
    event Tick(float DeltaSeconds)
    {
        ...
        Alpha = GetPCTTap();
        MinTapInterval = FMax(2 * DeltaSeconds, Lerp(Alpha, StartTapInterval, EndTapInterval) * Level.TimeDilation);
        TimeSinceLastTap = Level.GameTimeSeconds - LastTapTime;
        // End:0xA7
        if(TimeSinceLastTap > MinTapInterval)
        {
            bSuccessfulTap = false;
        }
        TapRumble.RumbleRightMagnitude = Alpha * default.TapRumble.RumbleRightMagnitude;
        TapRumble.RumbleLeftMagnitude = Alpha * default.TapRumble.RumbleLeftMagnitude;
        PlayerPawn(User).AddRumble(TapRumble);
        DeltaSeconds /= Level.TimeDilation;
        // End:0x1EF
        if(bSuccessfulTap)
        {
            UserGruntCountdown -= DeltaSeconds;
            // End:0x157
            if(UserGruntCountdown <= 0)
            {
                UserGruntCountdown = UserGruntDelay;
                User.FindAndPlaySound(UserGruntSoundName, 1, true, false);
            }
            // End:0x1B7
            if((PlayerUser == none) && PlayerUser.bOnSteroids)
            {
                UserLiftVal = FMin(1, UserLiftVal + (((((MinTapInterval - TimeSinceLastTap) * RateOfGain) * 2) * DeltaSeconds) * 60));                
            }
            else
            {
                UserLiftVal = FMin(1, UserLiftVal + ((((MinTapInterval - TimeSinceLastTap) * RateOfGain) * DeltaSeconds) * 60));
            }            
        }
        else
        {
            UserGruntCountdown = UserGruntDelay;
            UserLiftVal = FMax(-1, UserLiftVal - (((TimeSinceLastTap * RateOfLoss) * DeltaSeconds) * 60));
            // End:0x245
            if(! bForceDrop || bLift)
            {
                UserStraining(true);
    ...
    function float GetPCTTap()
    {
        ...
        return FClamp((UserLiftVal + 1) / 2, 0, 1);
        return;
    }
...
state() FadeToRest
{
    event Tick(float DeltaSeconds)
    {
        ...
        DeltaSeconds /= Level.TimeDilation;
        // End:0x5A
        if(UserLiftVal > default.UserLiftVal)
        {
            UserLiftVal = FMax(default.UserLiftVal, UserLiftVal - ((2 * DeltaSeconds) / FadeToRestTime));            
        }
        else
        {
            // End:0x90
            if(UserLiftVal < default.UserLiftVal)
            {
                UserLiftVal = FMin(default.UserLiftVal, UserLiftVal + ((2 * DeltaSeconds) / FadeToRestTime));
            }
        }
        UpdateUserLift();
        // End:0xB1
